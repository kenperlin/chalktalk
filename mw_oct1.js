...
,'      highp float rval = pow(v1 * v2, 7.0);' 
,'      return rval; '
,'   }' 
,'   void main(void) {'
,'      float numrows = 7.;'
...
,'      float mtime = time * 0.65;'
,'      float trb1 = turbulence(vec3(mndx*42., mndy*32., floor(time*37.0)));'
,'      float trb2 = turbulence(vec3(mndx*23., mndy*23., floor(time*18.0)));'
,'      float sin1 = sin((tx + mtime) * 2.0 * 3.14159 * 2.0);' 
,'      //'
,'      float cycle_time = 10.0;'
,'      float cycle_one = 3.0;'
,'      float cycle_two = 6.0;'
,'      float modtime = mod(time, cycle_time);'
,'      float cycle_num = 0.0;'
,'      if (modtime <= cycle_one) cycle_num = 1.0;'
,'      if (modtime >= cycle_one && modtime <= cycle_two) cycle_num = 2.0;'
,'      if (modtime >= cycle_two) cycle_num = 3.0;'
,'      //'
,'      vec3 lt_clr = vec3(0.05, 0.1, 0.05);'
,'      vec3 nolt_clr = vec3(0.2, 0.05, 0.05);'
...
,'      vec3 spot_clr = vec3(0.0, 0.5, 0.5);'
,'      //'
...
,'      lt_amp = spot_amp;'
,'      if (cycle_num == 1.0) lt_amp *= step(0.2, trb1);'
,'      if (cycle_num == 2.0) lt_amp *= step(0.2, trb2);'
,'      if (cycle_num == 3.0) lt_amp *= max(0., sin1);'
,'      // lt_amp += spot_amp * 0.05;'
,'      // spot_amp *= step(0.2, trb1);'
,'      // lt_amp = (spot_amp > 0.0) ? 0.0 : 1.0;'
,'      '
,'      // line_amp = step(0.975, max(mod(ndx, 1.0), mod(ndy, 1.0)));'
,'      //'
,'      out_color += lt_amp * spot_clr;'
,'      out_color += nolt_clr * (spot_amp * 0.1);'
