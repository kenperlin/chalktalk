...
eval : function(V, ni, nj, z) {
   ...
   for (let m = 0 ; m < nm ; m++)
      if (tag[m]) {
         tag[m] = P.length / 3;
	 let n = m >> 6,
	     ai = m    & 1, aj = m>>1 & 1, ak = m>>2 & 1,
	     bi = m>>3 & 1, bj = m>>4 & 1, bk = m>>5 & 1,
             A = V[n + ai + aj * dj + ak * dk],
             B = V[n + bi + bj * dj + bk * dk],
             t = (z - A) / (B - A);
         P.push((n2i(n) + ai + t * (bi - ai)) / ni,
                (n2j(n) + aj + t * (bj - aj)) / ni,
                (n2k(n) + ak + t * (bk - ak)) / ni);
      }
   ...
   for (let m = 0 ; m < T.length ; m += 3) {
      let a = 3 * T[m], b = 3 * T[m+1], c = 3 * T[m+2],
          n = floor(ni*P[a]) + floor(ni*P[a+1]) * dj + floor(ni*P[a+2]) * dk,
