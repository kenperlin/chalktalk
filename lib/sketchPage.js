...
///////// SAVING / LOADING THE PAGE TO / FROM THE SERVER //////////
      ...
      serialize : function () {
         ...
         for (n = 0 ; n < nsk() ; n++) {
            if ((out = sk(n).out_DEPRECATED_PORT_SYSTEM[0]) !== undefined)
...
///////////////////// EVENT HANDLERS /////////////////////////
      ...
      mouseDrag : function(x, y, z) {
         ...
         if (outPort >= 0 && isDef(outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort]) && ! this.click) {
            if (isNumeric(outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort])) {
               ...
               if (this.portValueDragMode == "portValueDragY") {
                  ...
                  outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort] += incr * outSketch.defaultValueIncr_DEPRECATED_PORT_SYSTEM[outPort];
               }
            }
            return;
         }
      ...
      mouseUp : function(x, y, z) {
         ...
         if (this.portValueDragMode !== undefined) {
            if (this.portValueDragMode == "portValueDragX") {
               outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort] *= x > this.xDown ? 0.1 : 10;
               outSketch.defaultValueIncr_DEPRECATED_PORT_SYSTEM[outPort] *= x > this.xDown ? 0.1 : 10;
            }
         ...
         if (this.isFocusOnLink && bgClickCount != 1) {
            ...
            // CAUSES ALL INSTANCES OF THAT VARIABLE TO BE REPLACED BY ITS VALUE.
               ...
               codeTextArea.value = variableToValue(codeTextArea.value,
                                                    "xyz".substring(j, j+1),
                                                    roundedString(codeSketch.inValue_DEPRECATED_PORT_SYSTEM[j]));
               linkAtCursor.remove();
            }
            return;
         }
      ...
      keyDown : function(key) {
         ...
         if (outPort >= 0 && isDef(outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort])) {
            isTextMode = true;
         ...
         // If the key is the designated tool tips toggle key,
         // toggle whether tool tips are shown or hidden
         if (key == 32) {
         ...
         if (isTextMode) {
            ...
            default:
               ...
               if (this.isPortValueTextMode !== undefined) {
                  val = "" + outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort];
                  if (letter == 'del') {
                     if (val.length > 0) {
                     ...
                     val += letter;
                  outSketch.defaultValue_DEPRECATED_PORT_SYSTEM[outPort] = isNumeric(val) ? val : val.length == 0 ? 0 : val;
               }
               else {
...
//////////////////////////////////////////////////////////////
      ...
      // Create a link between two sketches.
      //
      // inSketch: The sketch receiving input.
      // inPort: The index of the port it should receive input at.
      // outSketch: The sketch sending out the output.
      // outPort: The index of the output port it's using (currently values > 0 deprecated).
      createLink : function(inSketch, inPort, outSketch, outPort) {
         // Don't connect any two sketches that are incompatible
         if (!inSketch.canAcceptInputOfType(outSketch.getOutputTypename(), inPort)) {
            return false;
         }

         // AVOID CREATING DUPLICATE LINKS.

         if (inPort >= inSketch.in_DEPRECATED_PORT_SYSTEM.length
            || inSketch.in_DEPRECATED_PORT_SYSTEM[inPort] === undefined
            || inSketch.in_DEPRECATED_PORT_SYSTEM[inPort].a != outSketch
            || inSketch.in_DEPRECATED_PORT_SYSTEM[inPort].i != outPort)
         {
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            ...
            if (sketch.parent == null)
               for (var i = 0 ; i < sketch.portName_DEPRECATED_PORT_SYSTEM.length ; i++)
                  if (sketch.sketchProgress == 1 && isDef(sketch.portName_DEPRECATED_PORT_SYSTEM[i])) {
                     var str = sketch.portName_DEPRECATED_PORT_SYSTEM[i];
                     var A = sketch.portXY_DEPRECATED_PORT_SYSTEM(i);
                     var tw = max(portHeight, textWidth(str) + 10);
                     var px = A[0] - tw/2;
                     var py = A[1] - portHeight/2;
                     var pw = tw;
                     var ph = portHeight;
                     sketch.portBounds_DEPRECATED_PORT_SYSTEM[i] = [px, py, px + pw, py + ph];
                  }
         }
         _g.font = saveFont;
      },
