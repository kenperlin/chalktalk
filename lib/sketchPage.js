...
// MOST USER INTERACTION IS MEDIATED BY THE CURRENT SKETCH PAGE.
   ...
   function SketchPage() {
      ...
      SK = this; // EASIER TESTING
      this.timeL = new TimeLine(TIME_LINE_CAP);
   }

  // // capped timeline, double-ended circular queue
   
   
   // use powers of two sizes only
   function TimeLine(CONTAINER_SIZE) {
   ...
   // TimeLine.prototype = {
   //    // save current state
   //    saveState : function(saveStateFunc) {
   //       var next = (this.curr + 1) & this.MOD;
   //       // console.log("C", this.curr, "S", this.start, "N", next);
   //       // check whether to "forget" earliest past state
   //       if (next == this.start) {
   //          this.states[this.start] = undefined;
   //          this.start = (this.start + 1) & this.MOD;
   //       }

   //       this.states[this.curr] = saveStateFunc();
   //       // update write index
   //       this.curr = next;
   ...
   //       // console.log("-----------------------------");
   //       // console.log("AT INDEX" + ((this.curr - 1 + this.SIZE) & this.MOD));
   ...
   //    _getCurState : function() {
   //       return this.states[((this.curr - 1 + this.SIZE) & this.MOD)];      
   //    }
   //    ,

   //    // rewind
   //    restoreRW : function(toRestore, restoreFunc) {
   //       // no-op
   //       var prev = (this.curr - 1 + this.SIZE) & this.MOD;
   //       // console.log("C", this.curr, "S", this.start, "P", prev);
   //       if (prev == this.start || this.curr == this.start) {
   //          return;
   //       }
   //       this.curr = prev;
   //       restoreFunc(toRestore, this._getCurState());
   ...
   //    // fast-forward
   //    restoreFF : function(toRestore, restoreFunc) {
   //       // no-op
   //       // console.log("C", this.curr, "S", this.start, "F", this.frontier);
...
///////// SAVING / LOADING THE PAGE TO / FROM THE SERVER //////////
      ...
      saveState : function() {
         ...
         this.timeL.saveState(function() {
            ...
            return [sketchCopies, _g.panX, _g.panY, sketchPageState];
         });
      },

      _restoreSketchPage : function(page, data) {
         console.log(data[0]);
         page.sketches = data[0].slice();
         _g.panX = data[1];
         _g.panY = data[2];

         // var sketchPageState = data[3];
