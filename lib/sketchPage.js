...
///////// SAVING / LOADING THE PAGE TO / FROM THE SERVER //////////
      ...
      savePage : function(name) {
         ...
         server.set('state/sketchPage_' + name, [this.sketches, linksData, _g.panX, _g.panY]);
         this._linksFromData(linksData);
      },

      loadPage : function(name) {
         var that = this;
         server.get('state/sketchPage_' + name, function(data) {
	    that._sketchesFromData(data[0]);
            that._linksFromData   (data[1]);
            _g.panX = data[2];
            _g.panY = data[3];
         });
      },
      ...
      _linksFromData : function(d) {
         for (var n = 0 ; n < d.length ; n++)
            new SketchLink(sk(d[n][0]), 0, sk(d[n][1]), d[n][2], d[n][3]);
      },
