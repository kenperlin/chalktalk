...
// DRAW ALL THE SKETCHES.
	    ...
	    sketch = sk();
            ...
            if (this.isOnPanStrip) {
               ...
               if (sketch instanceof GeometrySketch) {
                  var x0 = xOnPanStrip(min(sketch.sp[0][0], sketch.sp[1][0]));
                  var y0 = yOnPanStrip(min(sketch.sp[0][1], sketch.sp[1][1]));
                  var x1 = xOnPanStrip(max(sketch.sp[0][0], sketch.sp[1][0]));
                  var y1 = yOnPanStrip(max(sketch.sp[0][1], sketch.sp[1][1]));
                  color(scrimColor(.2));
                  _g.beginPath();
                  _g.moveTo(x0, y0);
                  _g.lineTo(x1, y0);
                  ...
                  _g.beginPath();
                  for (var i = 0 ; i < sketch.sp.length ; i++) {
                     var x = xOnPanStrip(sketch.sp[i][0]);
                     var y = yOnPanStrip(sketch.sp[i][1]);
                     if (sketch.sp[i][2] == 0)
                        _g.moveTo(x, y);
                     else
                        _g.lineTo(x, y);
	 ...
	 if (! isExpertMode && bgClickCount == 1) {
	    ...
	    if (isHover())
               drawHelpMenu();
            else if (time - spacePressedTime > 0.5)
      ...
      overlay : function() {
         ...
         if (! isShowingGlyphs && ! isTextMode) {
            ...
            lineWidth(1);
            textHeight(9);
            var y0 = paletteY(palette.length) - 13;
