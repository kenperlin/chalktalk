...
*/
      ...
      keyUp : function(key) {
         ...
         case 'q':
            ...
            break;
         case 'R':
            isFillPolygonShader = ! isFillPolygonShader;
            break;
         case 'b':
         case 'm':
         case 'r':
...
// DRAW ALL THE SKETCHES.
         ...
         if (isFillPolygonShader) {
	    if (! window.fillShader) {
	       var webgl_context = webgl_canvas.getContext('experimental-webgl');
	       fillShader = new FillShader(webgl_context);
            }
            fillShader.useProgram();
            fillShader.draw([.5,0,0, 0,.5,0, -.5+.3*sin(3*time),0,0, 0,-.5,0,], [1,.5,.5,1]);
         }
