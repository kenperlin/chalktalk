...
var isShowingGif = false;
   ...
   var moveAllSketches_t = 1;
   var moveAllSketches_d = 0;

   function moveAllSketches(dir) {
      moveAllSketches_t = 0;
      moveAllSketches_d = dir * width();
   }
...
// HOT KEYS ARE RECOGNIZED BY THE SKETCH PAGE.
   ...
   var hotKeyMenu = [
      ...
      ['C'  , "casual style"],
      ['F'  , "fog"],
      ['G'  , "toggle greenscreen"],
      ['L'  , "capture reflectance ball"],
      ['M'  , "print model file"],
      ['R'  , "spin the world"],
      ['T'  , "speaker notes"],
      ['U'  , "toggle lightpen"],
      ['a'  , "reload sketchlibs"],
      ['b'  , "emulate mouse press"],
      ['c'  , "toggle camera data"],
      ['d'  , "dark video"],
      ['e'  , "speaker notes"],
      ['f'  , "fuzzy lines"],
      ['g'  , "toggle gifs"],
      ...
      ['v'  , "show/hide video"],
      ['w'  , "show/hide slides"],
      ['x'  , "toggle slides level"],
      ['y'  , "open video links page"],
      ['z'  , "reload sketch"],
      ['Z'  , "disable fullscreen"],
      ['{'  , "shift 1/4 screen left"],
      ['}'  , "shift 1/4 screen right"],
      ['['  , "shift one screen left"],
      [']'  , "shift one screen right"],
      ['.'  , "set lightpen color"],
      ['#'  , "graph paper"],
      ['-'  , "black / white board"],
      ['='  , "show glyphs"],
...
///////////////////// EVENT HANDLERS /////////////////////////
      ...
      mouseUp : function(x, y, z) {
	    ...
	    imageLibrary_selectAction();
         ...
         if (palette.isVisible) {
            // (Toby) temporarily disable so people do not accidentally delete in XR
	         // if (isk()) {
	 ...
	 isSketchCmdMode = false;
      ...
      keyDown : function(key) {
         ...
         switch (letter) {
         case '.':
	    if (lightTracker && lightTracker.setColorFromCornerBegin)
	       lightTracker.setColorFromCornerBegin();
	    break;
         case '3': case 'alt':
            isAltPressed = true;
            return;
	    ...
	    break;
         case 'L':
	    if (videoLayer)
	       window.isCapturingReflectionMap = true;
	    break;
         case 'm':
         case 's':
	 ...
	 case 'spc': // Space bar toggles tool tips.
	    ...
	    break;
         ...
         case PAGE_UP:
	    if (! isShowingGif)
               imageLibrary_previousImage();
	    else if (gifs.length)
	       setGif(gifs[gifIndex = (gifIndex + gifs.length - 1) % gifs.length]);
            break;
         case U_ARROW:
            setPage(pageIndex - 1);
            break;
         ...
         case PAGE_DN:
	    if (! isShowingGif)
               imageLibrary_nextImage();
	    else if (gifs.length)
	       setGif(gifs[gifIndex = (gifIndex + 1) % gifs.length]);
            break;
         case D_ARROW:
            setPage(pageIndex + 1);
            break;
         ...
         case '?':
            ...
            break;
         case '<':
	    if (lightTracker && lightTracker.setThreshold)
	       lightTracker.setThreshold(lightTracker.getThreshold() * 1.1);
	    break;
         case ',':
	    if (lightTracker && lightTracker.setColorMode)
	       lightTracker.setColorMode(lightTracker.getColorMode() + 1);
	    break;
         case '.':
	    if (lightTracker && lightTracker.setColorFromCornerEnd)
	       lightTracker.setColorFromCornerEnd();
	    break;
         case '>':
	    if (lightTracker && lightTracker.setThreshold)
	       lightTracker.setThreshold(lightTracker.getThreshold() / 1.1);
	 ...
	 case 'alt':
	    ...
	    break;
         ...
         case '9':
	    if (window.videoMatteThreshold === undefined)
	       window.videoMatteThreshold = 0;
	    videoMatteThreshold += .1;
            break;
         case '0':
	    if (window.videoMatteThreshold === undefined)
	       window.videoMatteThreshold = 0;
	    ...
	    break;
         case 'C':
            isCasualStyle = ! isCasualStyle;
            break;
         case 'd':
	    isDarkVideo = ! isDarkVideo;
	    break;
         case 'e':
	    ...
	    break;
         case 'F':
            ...
            break;
         case 'g':
	    setGif(isShowingGif = ! isShowingGif && gifs.length ? gifs[gifIndex] : '');
	    break;
         case 'G':
	    if (window.videoMatteState === undefined)
	       window.videoMatteState = 0;
	    videoMatteState = (videoMatteState + 1) % 3;
            break;
         case 'h':
	    this.toggleShowScript();
            break;
	    ...
	    break;
         case 'L':
	    isCapturingReflectionMap = false;
	    break;
         case 'M':
            if (isk() && sk()._model) {
	    ...
	    break;
         ...
         case 't':
            ...
            else if (elem.requestFullScreen   ) elem.mozRequestFullScreen();
...
//////////////////////////////////////////////////////////////
      ...
      animate : function(elapsed) {
         ...
         if (moveAllSketches_t < 1) {
	    let t0 = moveAllSketches_t,
	        t1 = t0 + elapsed,
	        dx = moveAllSketches_d * (sCurve(t1) - sCurve(t0));
	    for (let i = 0 ; i < nsk() ; i++)
	       sk(i).tX += dx / def(sk(i).xyz[2], 1);
            moveAllSketches_t = t1;
	 }
...
// DRAW ALL THE SKETCHES.
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            ...
            window.PUSHED_sketch_zOffset = sketch.zOffset();
	 ...
	 if (window.isCapturingReflectionMap) {
	    color('red');
	    fillOval(width()/2 - 128, height()/2 - 128, 256, 256);
	 }
...
function moveSketchToPage(sketch, dstPage) {
   const curPage = sketchPage;
   const curSketches = curPage.sketches;
   console.assert(sketch == curSketches[curSketches.length - 1]);

   curSketches.pop();

   dstPage.sketches.push(sketch);
}

function cloneSketchAndMoveToPageAtIndex(sketch, dstPageIndex) {
   const clone = cloneSketch(sketch);
   moveCloneToPage(clone, sketchPages[dstPageIndex]);
}

function moveSketchToPageAtIndex(sketch, srcPageIndex, dstPageIndex) {
   const currPageSketches = sketchPages[srcPageIndex].sketches;

   let index = currPageSketches.indexOf(sketch);

   if (index === -1) {
      return;
   }

   currPageSketches.splice(index, 1);

   console.log("SEE THIS");
   console.log("current page: " + pageIndex + "... moving sketch from: " + srcPageIndex + " to " + dstPageIndex);
         ...
         for (var outPort = 0 ; outPort < a.out.length ; outPort++)
            if (isDef(a.out[outPort]))
               for (var k = a.out[outPort].length - 1 ; k >= 0 ; k--) {
                  ...
                  if (link.b == sketch) {
                     console.log("removing a link from other sketch");
   ...
   for (var outPort = 0 ; outPort < sketch.out.length ; outPort++)
      if (isDef(sketch.out[outPort]))
         for (var k = sketch.out[outPort].length - 1 ; k >= 0 ; k--) {
            ...
            console.log("removing a link to other sketch");
            //link.removeFromInSketch();
            link.remove();
         }

   console.log(sketch.in);
   console.log(sketch.out);

   const srcPage = sketchPages[srcPageIndex];
   const dstPage = sketchPages[dstPageIndex];

    // TODO not sure if pan translation necessarily, will verify later
   // const transX = srcPage.panX - dstPage.panX;
   // const transY = srcPage.panY - dstPage.panY;
   // const transZ = srcPage.panZ - dstPage.panZ;
   // sketch.translate(transX / f, transY / f);
   ...
   const f = sketch.mScale();
   sketch.translate(-transX / f, -transY / f);

   sketchPages[dstPageIndex].add(sketch);
}

function repositionSketch(sketch) {
   const mx = This().mouseX;
   const my = This().mouseY;
   const cx = sketch.cx();
   const cy = sketch.cy();
...
function removeLinksOutsideGroup(sketch, groupSketches) {
   for (var inPort = 0 ; inPort < sketch.in.length ; inPort++)
      if (isDef(sketch.in[inPort])) {
         var a = sketch.in[inPort].a;
         for (var outPort = 0 ; outPort < a.out.length ; outPort++)
            if (isDef(a.out[outPort]))
               for (var k = a.out[outPort].length - 1 ; k >= 0 ; k--) {
                  var link = a.out[outPort][k];
                  console.log(link);
                  if (link.b == sketch && (groupSketches.indexOf(link.a) === -1)) {
                     console.log("removing a link from other sketch");
                     //link.removeFromOutSketch();
                     link.remove();
                  }
   ...
   for (var outPort = 0 ; outPort < sketch.out.length ; outPort++)
      if (isDef(sketch.out[outPort]))
         for (var k = sketch.out[outPort].length - 1 ; k >= 0 ; k--) {
            var link = sketch.out[outPort][k];
            if (groupSketches.indexOf(link.b) !== -1) {
               continue;
            }
...
function removeLinksOutsideGroupLoop(groupSketches) {
   for (let i = 0; i < groupSketches.length; i += 1) {
      removeLinksOutsideGroup(groupSketches[i], groupSketches);
   }
}

function moveGroupToPageAtIndex(groupObj, srcPageIndex, dstPageIndex) {
   console.log("moving group of sketches from: " + pageIndex + " to " + dstPageIndex);
   // remove sketches from current page
   const groupSketches = groupObj.sketches;
   const currPageSketches = sketchPages[srcPageIndex].sketches;
   for (let i = 0, bound = groupSketches.length; i < bound; i += 1) {
      let index = currPageSketches.indexOf(groupSketches[i]);
      if (index !== -1) {
         console.log("moving sketch from: " + srcPageIndex + " to " + dstPageIndex);
         currPageSketches.splice(index, 1);
      }      
   }
   ...
   // add group sketches to the destination page
   const dstPage = sketchPages[dstPageIndex];
   for (let i = 0, bound = groupSketches.length; i < bound; i += 1) {
      dstPage.add(groupSketches[i]);
   }

   group.sketchPageIndex = dstPageIndex;
   ...
   // const transZ = srcPage.panZ - dstPage.panZ;
    ...
    // group.translateUnconditionally(transX, transY);
...
function repositionGroup(groupObj) {
   const mx = This().mouseX;
   const my = This().mouseY;
   const cx = (group.xlo + group.xhi) / 2;
   const cy = (group.ylo + group.yhi) / 2;
