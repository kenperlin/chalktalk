...
// UNDER CONSTRUCTION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   ...
   // I'll see
   function skStateRecord(recordLen) {
      // reserve one more space for the empty state     
      this.recordCap = recordLen + 1;
      this.backLogLen = 1;
      this.queue = [];
      for (var i = 0; i < recordLen + 1; i++) {
        this.queue.push([]);
      }
      // set to next index to which we can write
      this.curr   = 1;
      // earliest state
      this.start  = 0;
      // state farthest in the future
      this.future = 0;
   }
   
   // TODO: Should I serialize and keep that for the state, and de-serialize
   // upon state change? Not entirely sure whether this is worth it /
   // where and how often to update state
   skStateRecord.prototype.saveState = function(sp) {
      this.queue[this.curr] = sp.serialize();
      // invalidate future after saving a state
      this.future = this.curr;
      
      var next = (this.curr + 1) % this.recordCap;
      if (this.curr == this.start && this.backLogLen > 0) {
         this.start = next;
      }
      else {
         this.backLogLen++;
   ...
   //
   skStateRecord.prototype.undo = function(sp) {
      // no-op
      if (this.backLogLen == 0 
            || (this.curr - 1) < 0 && this.recordCap - 1 == this.start
            || (this.curr - 1) % this.recordCap == this.start) {
         return;
      }
      this.curr = ((this.curr - 1) < 0) ? this.recordCap - 1 : this.curr - 1;
      this.backLogLen--;
   }
   ...
   //
   skStateRecord.prototype.redo = function(sp) {
      // no-op
      if (((this.curr - 1) < 0 && this.recordCap - 1 == this.future)
              || (this.backLogLen > 0 
                  && ((this.curr - 1) % this.recordCap == this.future))) {
            return;
      }
      this.curr = (this.curr + 1) % this.recordCap;
      this.backLogLen++;
   }
   
   skStateRecord.prototype.restoreState = function(sp) {
      if (this.backLogLen == 0) {
         return;
      }
      
      var state = this.queue[
         ((this.curr - 1) < 0) ? this.recordCap - 1 : this.curr - 1
      ];
      
      sp.unpackSketches(state);
   }
...
// END OF CONSTRUCTION ZONE >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   ...
   function SketchPage() {
      ...
      // test
      this.skStates = new skStateRecord(7);
   }
