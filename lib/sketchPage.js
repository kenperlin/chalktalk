      ...
      keyUp : function(key) {
	    ...
	    break;
         ...
         case 'y':
            ...
            break;
         case 'z':
            window.open('localhost:11235/videos/demo.mp4');
            break;
         case 'Z':
/*
      ...
      skCallback : function(actionName, x, y, z) {
         if (isk() && sk()[actionName] !== undefined) {
            ...
            if (sk().onEnter !== undefined || sk().onExit !== undefined) {
               function isWithinSketch(p) {
	          return p.x >= -1 && p.x < 1 && p.y >= -1 && p.y < 1;
	       }
	       if (sk()._prevCursorPoint === undefined)
	          sk()._prevCursorPoint = newVec3();
               else {
	          var wasWithin = isWithinSketch(sk()._prevCursorPoint);
	          var isWithin  = isWithinSketch(sk()._cursorPoint);
	       ...
	       sk()._prevCursorPoint.copy(sk()._cursorPoint);
            }
         }
      },
