...
///////////////////// EVENT HANDLERS /////////////////////////
	 ...
	 if (linkAtMouseDown != null) {
	    ...
	    if (! isPieMenuPalette) {
               index = findPaletteColorIndex(x, y);
               if (index >= 0)
      ...
      mouseUp : function(x, y, z) {
	    ...
	    return;
         ...
         if (isShowingGlyphs && ! this.isDraggingGlyph) {
	    ...
	    // DELETE ANY LEFT-OVER EMPTY TEXT SKETCHES.
	       ...
	       continue;
            ...
            if (sketch.sketchTrace != null && sketch.sketchState != 'finished') {
               if (sketch.createMesh !== undefined) {
                  ...
                  if (alpha > 0) {
                     ...
                     morphSketchToGlyphSketch(sketch);
                  }
               }
               else
                  morphSketchToGlyphSketch(sketch);
            ...
            if (sketch.hasMotionPath() && skTrue.hasMotionPath() && sketch.colorId == skTrue.colorId) {
               ...
               _g.strokeStyle = 'rgba(' + paletteRGB[sketch.colorId][0] + ',' +
                                          paletteRGB[sketch.colorId][1] + ',' +
                                          paletteRGB[sketch.colorId][2] + ', 0.5)';
	 ...
	 if (! isExpertMode && bgClickCount == 1) {
	    ...
	    case ',':
         ...
         if (this.paletteColorDragXY != null) {
            ...
            fillOval(this.paletteColorDragXY[0] - 12,
                     this.paletteColorDragXY[1] - 12, 24, 24);
         }
      },

      glyphColWidth : function() {
         return height() / this.glyphsPerCol * .89;
      },

      glyphBounds : function(i) {
         var ht = height() / this.glyphsPerCol;
         var x = this.glyphColWidth() * (0.05 + floor(i / this.glyphsPerCol)) - _g.panX;
      ...
      isShowingPalette : function() {
	    ...
	    return true;
         return this.paletteColorDragXY != null ||
	        This().mouseX < paletteWidth() - _g.panX &&
	        This().mouseY < paletteHeight() - _g.panY &&
      ...
      showGlyphs : function() {
         ...
         this.glyphT = this.isDraggingGlyph
                     ? this.iDragged + 0.99
                     : this.glyphsPerCol * (floor( (this.mx + _g.panX) / this.glyphColWidth() ) +
      ...
      showGlyph : function(i, cx, cy) {
         ...
         var selected = (this.nnSelected & 1 << nn) != 0 ||
	                mx >= b[0] && my >= b[1] && mx < b[2] && my < b[3];

         _g.strokeStyle = _g.fillStyle = selected ? defaultPenColor : this.glyphColor();
