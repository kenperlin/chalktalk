...
// DRAW ALL THE SKETCHES.
	 ...
	 if (isStrokeShader) {
	    if (! window.strokeShader)
	       strokeShader = new StrokeShader(gl());
            gl().useProgram(strokeShader.program);
	    gl().frontFace( gl().CW );
	    strokeShader.drawStroke([-.5,  0,0,  .5, 0,0], [1,.5,.5,1], .05, .1);
	    strokeShader.drawStroke([-.5,-.5,0,  .5,.5,0], [.5,1,.5,1], .05, .1);
	    strokeShader.drawStroke([  0,-.5,0,   0,.5,0], [.5,.5,1,1], .05, .1);
         }
