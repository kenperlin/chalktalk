...
////////// EXPERIMENTAL CODE TO REBUILD ALL LINKS ///////////

   var rebuildLinksPhase = 0;

   function rebuildLinks() {

      for (var n = 0 ; n < nsk() ; n++)
         console.log(sk(n).glyph !== undefined ? sk(n).glyph.name : sk(n).glyphName);
      return;

      var data = [], n, out, k, link, index, text, d;
      switch (rebuildLinksPhase) {
      case 0:
         var sketchesData, linksData;

         // CONVERT ALL LINKS TO A DATA ARRAY THAT USES INDICES RATHER THAN POINTERS.
         ...
         for (n = 0 ; n < nsk() ; n++)
            if ((out = sk(n).out[0]) !== undefined)
	          ...
	          data.push([ n , index , link.j , link.s ]);
	          link.remove();
               }

         // NOW THAT WE HAVE REMOVED ALL CIRCULAR REFERENCES,
         // WE CAN SAVE SKETCHES AND LINKS DATA TO THE SERVER IN JSON FORMAT.

         var sketchesString = JSON.stringify(sketchPage.sketches);
         server.setValue('sketches', sketchesString);
         server.setValue('links', JSON.stringify(data));

	 console.log(sketchesString);
      ...
      case 1:
         ...
         server.getValue('sketches');
         server.getValue('links');

         break;

      case 2:

         console.log('reconstituting sketches and links');

         // RECONSTITUTE THE SKETCHES AND LINKS.

	 console.log(server.sketches);

         sketchPage.sketches = JSON.parse(server.sketches);

         data = JSON.parse(server.links);
         for (n = 0 ; n < data.length ; n++) {
	    d = data[n];
            new SketchLink(sk(d[0]), 0, sk(d[1]), d[2], d[3]);
         }

	 break;
      }

      rebuildLinksPhase = (rebuildLinksPhase + 1) % 3;
   }
...
///////////////////// EVENT HANDLERS /////////////////////////
	 ...
	 if (group.mouseUp(x, y))
	    return;

         if (overviewAlpha > 0) {
	    var panX = _g.panX, panY = _g.panY;
	    _g.panX = (width() /2 - (x + _g.panX)) * overviewScale;
	    _g.panY = (height()/2 - (y + _g.panY)) * overviewScale;
	    ...
	    isShowingGlyphs = false;
         ...
         if (glyphChart.isDragging) {
            glyphs[iDragged].toSimpleSketch(This().mouseX, This().mouseY, 1.5);
...
// DRAW ALL THE SKETCHES.
	    ...
	    // DELETE ANY LEFT-OVER EMPTY TEXT SKETCHES.
	       ...
	       continue;
            ...
            if (sketch.hasMotionPath() && skTrue.hasMotionPath() && sketch.colorId == skTrue.colorId) {
               var X = sketch.motionPath[0];
               var Y = sketch.motionPath[1];
               _g.strokeStyle = 'rgba(' + palette.rgb[sketch.colorId][0] + ',' +
                                          palette.rgb[sketch.colorId][1] + ',' +
                                          palette.rgb[sketch.colorId][2] + ', 0.5)';
	 ...
	 if (! isExpertMode && bgClickCount == 1) {
      ...
      panHistoryAdd : function() {
         if (this.panHistoryIndex == this.panHistory.length - 1)
            this.panHistoryIndex++;
         this.panHistory.push([_g.panX, _g.panY]);
      },

      panHistoryScrub : function(dIndex) {
         if (this.panHistory.length > 0) {
            this.panHistoryIndex += dIndex;
            this.panHistoryIndex = max(0, min(this.panHistory.length - 1, this.panHistoryIndex));
	    var xy = this.panHistory[floor(this.panHistoryIndex)];
	    _g.panX = xy[0];
	    _g.panY = xy[1];
         }
      },
