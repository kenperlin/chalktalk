...
// UNDER CONSTRUCTION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   ...
   // trying to build a double-ended circular queue,
   // javascript's shift might not be the greatest, but possibly easier,
   // I'll see
   
   /*
   // use powers of two sizes only
   function StateQueue(CONTAINER_SIZE) {
      // reserve one more space for the empty state     
      this.SIZE = CONTAINER_SIZE; // might or might not save SIZE and MOD separately
      this.MOD = this.size - 1;
      this.queue = [];
      for (var i = 0; i < this.size; i++) {
        this.queue.push(undefined);
      }
      // set to next index outside the timeline to which we can write
      this.curr = 1;
      // index of earliest state
      this.start = 0;
      // set to first index outside the timeline (same as curr if we never undo)
      this.frontier = 1;
   }
   
   // passing a this pointer to the sketchPage is not great, 
   // yes, must learn more javascript
   StateQueue.prototype.saveState = function(sp) {
      var next = (this.curr + 1) & this.MOD;
      // check whether to "forget" earliest past state
      if (next == this.start) {
         this.queue[this.start] = undefined;
         this.start = (this.start + 1) & this.MOD;
      }

      //this.queue[this.curr] = sp.serialize();// // WHAT / HOW TO SAVE?
	  
	  // invalidate future after saving a state
      this.frontier = this.curr;
      // update write index
      this.curr = next;
   }
   
   StateQueue.prototype.restoreState = function(sp) {
      var state = this.queue[
         ((this.curr - 1 + this.SIZE) & this.MOD)
      ];
      /*sp.unpackSketches(state)//; // WHAT / HOW TO RESTORE?
      sp.sketches = [];
   }
   
   //
   // move index to next valid state (in the future)
   //
   StateQueue.prototype.undo = function(sp) {
      // no-op
      var prev = (this.curr - 1 + this.SIZE) & this.MOD;
      if (prev == this.start) {
         return;
      }
      this.curr = prev;
      
      this.restoreState(sp); // ?
   }
   
   //
   // move index to previous valid state (in the past)
   //
   StateQueue.prototype.redo = function(sp) {
      // no-op
      if (this.curr == this.frontier) {
         return;
      }
      this.curr = (this.curr + 1) & this.MOD;
...
// MOST USER INTERACTION IS MEDIATED BY THE CURRENT SKETCH PAGE.
   ...
   TimeLine.prototype = {
      saveState : function(procedure) {
         // console.log(procedure);
         // this.states = this.states.slice(this.idx + 1);
         /*var copy = [[]];
         for (var i = 1; i <= this.idx; i++) {
            copy.push(this.states[i]);
         }
         */

         //this.states = copy;
         
         //console.log("before:", this.states, "len: ", this.states.length, "idx:", this.idx);
         this.states.splice(this.idx + 1);
         this.states.push(procedure());
         
         this.idx++;

         console.log("-----------------------------");
         console.log("AT INDEX" + this.idx);
         ...
         //console.log("after:", this.states, "len: ", this.states.length, "idx: ", this.idx);

      },
      restoreRW : function(toRestore, procedure) {
         ...
         this.idx--;
         procedure(toRestore, this.states[this.idx]);
      ...
      restoreFF : function(toRestore, procedure) {
...
///////// SAVING / LOADING THE PAGE TO / FROM THE SERVER //////////
      ...
      // maybe use the sketchbook to save instead
      saveState : function() {
         console.log("save");

         var skPage = this;
         this.timeL.saveState(function() {
            /*
            var sketchPageState = {};
            sketchPageState.altCmdState                 = skPage.altCmdState;
            sketchPageState.fadeAway                    = skPage.fadeAway;
            sketchPageState.imageLibrary_alpha          = skPage.imageLibrary_alpha;
            sketchPageState.imageLibrary_index          = skPage.imageLibrary_index;
            sketchPageState.isCTObject                  = skPage.isCTObject;
            sketchPageState.isCTRotate                  = skPage.isCTRotate;
            sketchPageState.isFocusOnLink               = skPage.isFocusOnLink;
            sketchPageState.isGraphPaper                = skPage.isGraphPaper;
            sketchPageState.isLocked                    = skPage.isLocked;
            sketchPageState.keyPressed                  = skPage.keyPressed;
            sketchPageState.letterPressed               = skPage.letterPressed;
            sketchPageState.letterPressedTime           = skPage.letterPressedTime;
            sketchPageState.needToStartSketchDragAction = skPage.needToStartSketchDragAction;
            sketchPageState.scaleRate                   = skPage.scaleRate;
            sketchPageState.zoom                        = skPage.zoom;
            //sketchPageState.paletteDragXY               = skPage.palette.dragXY;
            */

            var sketchCopies = [];
            for (var i = 0; i < nsk(); i++) {
               console.log("save individual\n", sk(i));
               saveSketchState(sk(i), i, sketchCopies);
            }
            //return [sketchCopies, _g.panX, _g.panY];
            return sketchCopies;
         });
      },

      _restoreSketchPage : function(page, data) {
         /*
         var sketchPageState = data[0];
         page.altCmdState                 = sketchPageState.altCmdState;
         page.fadeAway                    = sketchPageState.fadeAway;
         page.imageLibrary_alpha          = sketchPageState.imageLibrary_alpha;
         page.imageLibrary_index          = sketchPageState.imageLibrary_index;
         page.isCTObject                  = sketchPageState.isCTObject;
         page.isCTRotate                  = sketchPageState.isCTRotate;
         page.isFocusOnLink               = sketchPageState.isFocusOnLink;
         page.isGraphPaper                = sketchPageState.isGraphPaper;
         page.isLocked                    = sketchPageState.isLocked;
         page.keyPressed                  = sketchPageState.keyPressed;
         page.letterPressed               = sketchPageState.letterPressed;
         page.letterPressedTime           = sketchPageState.letterPressedTime;
         page.needToStartSketchDragAction = sketchPageState.needToStartSketchDragAction;
         page.scaleRate                   = sketchPageState.scaleRate;
         page.zoom                        = sketchPageState.zoom;
         //page.palette.dragXY              = sketchPageState.paletteDragXY;
         

         page.sketches   = data[1];
         _g.panX         = data[2];
         _g.panY         = data[3];
         */
         /*
         page.sketches   = data[0];
         _g.panX         = data[1];
         _g.panY         = data[2];
         */
         page.sketches = data.slice();
      },

      restoreRW : function() {
         console.log("restore RW");
         this.timeL.restoreRW(this, this._restoreSketchPage);
      },

      restoreFF : function() {
         console.log("restore FF");
         this.timeL.restoreFF(this, this._restoreSketchPage);
      },
