...
///////////////////// EVENT HANDLERS /////////////////////////
	 ...
	 if (sketchDragMode != 7 && this.iStretch !== undefined)
	    return;
...
*/
      ...
      keyUp : function(key) {
         ...
         case 'q':
            ...
            break;
         case 'R':
	    isRenderedStrokes = true;
	    for (var n = 0 ; n < 10 ; n++) {
	       var renderedStroke = new RenderedStroke(100);
	       renderer.scene.root.add(renderedStroke.mesh);
	       renderedStroke.mesh.renderedStroke = renderedStroke;
            }
            break;
         case 'b':
         case 'm':
...
//////////////////////////////////////////////////////////////
      ...
      toggleStereo : function() {
         displayStrokes.isStereo = ! displayStrokes.isStereo;
         server.broadcastEvent("toggleStereo");
      },
...
// DRAW ALL THE SKETCHES.
	 ...
	 // NEED TO IMPLEMENT STEREO VIEW OF 3D RENDERED SCENE.

         if (displayStrokes.isStereo) {
            renderer.render(renderer.scene, renderer.camera);
	 }
	 else {
	    if (isRenderedStrokes) {
	       for (var n = 0 ; n < renderer.scene.root.children.length ; n++) {
	          var obj = renderer.scene.root.children[n];
		  if (obj.renderedStroke) {
	             var c = [];
	             for (var i = 0 ; i < 100 ; i++)
	                c.push([20 * n + i/100, i * i / 100/100, 0]);
	             obj.renderedStroke.render(c, [1,1,n/10,1]);
                  }
               }
	    }
