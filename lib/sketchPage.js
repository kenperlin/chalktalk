...
///////////////////// EVENT HANDLERS /////////////////////////
      ...
      mouseDown : function(x, y, z) {
         ...
         if (isShowingGlyphs) {
            ...
            for (var i = 0 ; i < glyphChart.tags.length ; i++) {
               var b = glyphChart.tagBounds(i);
               if (x >= /*(width() - (2 * (width() - b[0])))*/ -width() - 2 * b[0] && x < b[2] && y >= b[1] && y < b[3]) {
                  glyphChart.isSelectingTag = true;
                  glyphChart.iSelectedTag = i;
                  glyphChart.isClicked = true;
                  return;
               }
            }
            return;
      ...
      mouseUp : function(x, y, z) {
         ...
         if (isShowingGlyphs && ! glyphChart.isDragging && ! glyphChart.isSelectingTag) {
            isShowingGlyphs = false;
            glyphChart.disableAllTags();
            return;
         }
         ...
         if (glyphChart.isSelectingTag) {
            glyphChart.isSelectingTag = false;
            return;
         }
