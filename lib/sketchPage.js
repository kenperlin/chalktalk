...
// THE SKETCH BOOK CONTAINS ALL THE SKETCH PAGES.
   ...
   function pointToPixel(src, dst) {
      dst.copy(src).applyMatrix4(pointToPixelMatrix);
      dst.z = 0;
   }
      ...
      mouseDown : function(x, y, z) {
         ...
         if (! isShowingGlyphs) {
            ...
            if (this.isOnPanStrip) {
               this.isPanViewGesture = this.panViewBounds.contains(x, y);
            }
         ...
         if (isk() && sk().isMouseOver) {
            ...
            if (sk().sketchProgress == 1) {
               this.isFocusOnGlyphSketch = ! (sk() instanceof FreehandSketch) || sk().isGroup();
               sk().isPressed = true;
               sk().isClick = true;
               sk().travel = 0;
               sk().xDown = x;
...
*/
         ...
         if (this.isOnPanStrip) {
            ...
            this.panViewBounds = new Bounds(x0, y0, x1, y1);
