...
function Sketch() {
   ...
   // Legacy port system, DEPRECATED
   this.inLabel_DEPRECATED_PORT_SYSTEM = []; // array of incoming link labels
   this.inValue_DEPRECATED_PORT_SYSTEM = []; // array of incoming link values
   this.inValues_DEPRECATED_PORT_SYSTEM = []; // flattened array of inValue
   this.outValue_DEPRECATED_PORT_SYSTEM = []; // array of values

   this.nPorts_DEPRECATED_PORT_SYSTEM = 0;
   this.portName_DEPRECATED_PORT_SYSTEM = [];
   this.portLocation_DEPRECATED_PORT_SYSTEM = [];
   this.portBounds_DEPRECATED_PORT_SYSTEM = [];
   this.defaultValue_DEPRECATED_PORT_SYSTEM = [];
   this.defaultValueIncr_DEPRECATED_PORT_SYSTEM = [];
}
...
Sketch.prototype = {
   ...
   initInstanceVariables: function() {
      // We initialize here any variables that need to have unique objects in every sketch.
      // If we were to do it above in the Sketch constructor, we'd end up with these objects
      // being shared among every class of object, because each class of sketch uses a "new Sketch"
      // object as a shared prototype.
      this.drawing = new DRAWING.Drawing();
      this.sketchTexts = [];
      this.inputs = new InputPortList();

      this.arrows = [];
      this.children = [];

      this.defaultValue_DEPRECATED_PORT_SYSTEM = [];
      this.outValue_DEPRECATED_PORT_SYSTEM = [];
      this.portBounds_DEPRECATED_PORT_SYSTEM = [];
      this.portLocation_DEPRECATED_PORT_SYSTEM = [];
      this.inValue_DEPRECATED_PORT_SYSTEM = [];
   },
   ...
   findLinkedSketches : function() {                             // FIND SKETCHES MUTUALLY
      let C = [ this ], nC = 0;                                  // CONNECTED VIA ARROWS OR LINKS,
      while (C.length > nC) {                                    // STARTING WITH THIS SKETCH.
         nC = C.length;
         for (let n = 0 ; n < C.length ; n++) {
            let s = C[n];
            if (isDef(s.output)) {
               for (let i = 0 ; i < s.output.links.length ; i++) {
                  if (C.indexOf(s.output.links[i].b) < 0) {
                     C.push(s.output.links[i].b);
                  }
               }
            }

            for (let port = 0 ; port < s.inputs.numPorts(); port++) {
               let inLink;
               if (isDef(inLink = s.inputs.link(port))) {
   ...
   modelBeginFrame : function() {
      if (this._isModel) {
	    ...
	    return;
         ...
         this.model().xform(
	    CT.matrixMultiply(CT.matrixTranslated( (this.tX + _g.panX) / w2,
                                                  -(this.tY + _g.panY) / w2,
                                                              _g.panZ  / w2 ),
   ...
   modelEndFrame : function() {
      if (this._called_beginFrame) {
         if (this.color instanceof Array) {
            this.model().applyRecursively('color', this.color);
         }
   ...
   clearPorts_DEPRECATED_PORT_SYSTEM : function() {
      this.nPorts_DEPRECATED_PORT_SYSTEM = 0;
      this.portName_DEPRECATED_PORT_SYSTEM = [];
      this.portLocation_DEPRECATED_PORT_SYSTEM = [];
      this.portBounds_DEPRECATED_PORT_SYSTEM = [];
      this.inValue_DEPRECATED_PORT_SYSTEM = [];
      this.inValues_DEPRECATED_PORT_SYSTEM = [];
      this.outValue_DEPRECATED_PORT_SYSTEM = [];
      this.defaultValue_DEPRECATED_PORT_SYSTEM = [];
   },
   inSketch : function(port) {
      return this.inputs.hasLink(port) ? this.inputs.link(port).a : undefined;
   },
   addPort_DEPRECATED_PORT_SYSTEM : function(name, x, y) {
      this.portName_DEPRECATED_PORT_SYSTEM[this.nPorts_DEPRECATED_PORT_SYSTEM] = name;
      this.portLocation_DEPRECATED_PORT_SYSTEM[this.nPorts_DEPRECATED_PORT_SYSTEM] = [x, y];
      this.nPorts_DEPRECATED_PORT_SYSTEM++;
   },
   setPortLocation_DEPRECATED_PORT_SYSTEM : function(name, x, y) {
      var index = getIndex(this.portName_DEPRECATED_PORT_SYSTEM, name);
      if (index >= 0 && index < this.portLocation_DEPRECATED_PORT_SYSTEM.length) {
         this.portLocation_DEPRECATED_PORT_SYSTEM[index][0] = x;
         this.portLocation_DEPRECATED_PORT_SYSTEM[index][1] = y;
      }
   },
   clone : function() {
   ...
   // Defines an input port for this sketch.
   // The order in which input ports defined matters. The first call to this function will define
   // input port 0 for this sketch, the second will define input port 1, and so on.
   //
   // type: The constructor of the Atypical type this input port should accept. This input port will
   //       be guaranteed to only have a value of this type (if it's not empty/undefined, and
   //       if you haven't added any alternate types). Must be a type that's already been
   //       defined.
   // preprocessingFunction: An optional function of one argument that gets called every time this
   //                        port receives a value. If defined, this.inputs.value(n) returns the
   //                        result of calling this function on the input value instead of the
   //                        raw input value.
   defineInput: function(type, preprocessingFunction) {
      // The "inputs" variable only gets created when a sketch is added to the board.
      // Other sketch objects, such as those used for prototypes, don't have inputs,
      // so we ignore those.
      if (!isDef(this.inputs)) {
         return;
      }

      if (!AT.typeIsDefined(type)) {
         console.error("Type " + type.name + ", used as input "
            + this.inputs.numPorts() + " for " + this.label
            + ", not defined");
         return;
      }
      ...
      if (isDef(preprocessingFunction)) {
         if (!this._validatePreprocessingFunction(preprocessingFunction, this.inputs.numPorts())) {
            preprocessingFunction = undefined;
         }
         else {
            preprocessingFunction = preprocessingFunction.bind(this);
         }
      }

      this._lastPortDefinedWasDynamic = false;
      this.inputs._addPort(new InputPort(type, preprocessingFunction))
   },

   // Defines an alternate input type for this sketch's last-defined input port.
   // For example, doing this as the first port:
   //    defineInput(AT.Float);
   //    defineAlternateInputType(AT.Int);
   // Makes it so that port 0 can accept objects of Float or Int type. Any objects passed in
   // will be passed as-is if their type matches any of the types exactly. If not, they will
   // be converted to the first type that this port accepts that that value can be converted to.
   // (E.g. will first try to convert to Float, then Int).
   // As with the input function, order matters!
   //
   // type: The constructor of the Atypical type you want to add support for. Must be a type
   //       that's already been defined.
   // preprocessingFunction: An optional function of one argument that gets called every time
   //                        this port receives a value of this alternate type. If defined, this.
   //                        inputs.value(n) returns the result of calling this function on the
   //                        input value instead of the raw input value.
   defineAlternateInputType: function(type, preprocessingFunction) {
      // The "inputs" variable only gets created when a sketch is added to the board.
      // Other sketch objects, such as those used for prototypes, don't have inputs,
      // so we ignore those.
      if (!isDef(this.inputs)) {
      ...
      let i = this._lastPortDefinedWasDynamic
         ? this.inputs.numPorts()
         : this.inputs.numPorts() - 1;

      if (!AT.typeIsDefined(type)) {
         console.error("Type " + type.name + ", used as alternate input type for port "
            + i + " of " + this.label + ", not defined");
         return;
      }

      // Validate that at least one port exists
      if (!this.inputs.hasDynamicPort() && this.inputs.numPorts() == 0) {
         console.error("Tried to add an alternate input port type to " + this.label
            + ", but this sketch has no input ports defined.");
         return;
      }

      // Validate preprocessing function
      if (isDef(preprocessingFunction)) {
         if (!this._validatePreprocessingFunction(preprocessingFunction, i)) {
            preprocessingFunction = undefined;
         }
         else {
            preprocessingFunction = preprocessingFunction.bind(this);
      ...
      if (this._lastPortDefinedWasDynamic) {
         this.inputs._dynamicPortTemplate.addAcceptedType(type, preprocessingFunction);
      }
      else {
         this.inputs.port(i).addAcceptedType(type, preprocessingFunction);
      }
   },

   // Call this to define a variable-length set of inputs for a sketch.
   //
   // For example, if you want the first port of a sketch to take in Ints, the second to take in
   // Floats, and the third, fourth, fifth, and so on to take in a String, you can call:
   // this.defineInput(AT.Int);
   // this.defineInput(AT.Float);
   // this.defineAllRemainingInputs(AT.String);
   // And the sketch will be allowed to take in an arbitrary number of String inputs from its
   // third input onwards.
   //
   // This MUST be called as the last input port definition on a sketch. After this is called, you
   // cannot call defineInput(...) anymore.
   //
   // You CAN call defineAlternateInputType(...) after calling this function to add another input
   // type to all variable-length ports.
   //
   // type: The constructor of the Atypical type these input ports should accept.
   //       Must be a type that's already been defined.
   // preprocessingFunction: An optional function of one argument that gets called every time any
   //                        of these ports receive a value. If defined, this.inputs.value(n)
   //                        returns the result of calling this function on the input value
   //                        instead of the raw input value.
   defineAllRemainingInputs: function(type, preprocessingFunction) {
      // The "inputs" variable only gets created when a sketch is added to the board.
      // Other sketch objects, such as those used for prototypes, don't have inputs,
      // so we ignore those.
      if (!isDef(this.inputs)) {
      ...
      // Validate preprocessing function
      if (isDef(preprocessingFunction)) {
         if (!this._validatePreprocessingFunction(preprocessingFunction,
            this.inputs.numPorts()))
         {
            preprocessingFunction = undefined;
         }
      ...
      this._lastPortDefinedWasDynamic = true;
      this.inputs._addDynamicPort(type, preprocessingFunction);
   },

   // Convenience function for validating preprocessing functions and spitting out the correct
   // error message. Returns true if the first argument can be used as a preprocessing function,
   // false if not, and also prints our an error message to the console if validation fails.
   _validatePreprocessingFunction: function(preprocessingFunction, portNumber) {
      if (typeof(preprocessingFunction) !== "function") {
         console.error("Error defining preprocessing function for port " + portNumber
            + " of " + this.label + ", got " + typeof(preprocessingFunction)
            + " instead of function.");
         return false;
      }
      if (preprocessingFunction.length !== 1) {
         console.error("Error defining preprocessing function for port " + portNumber
            + " of " + this.label + ", function must have exactly one argument (has "
            + preprocessingFunction.length + ")");
         return false;
      }
   ...
   // Defines the output port for this sketch.
   //
   // type: The constructor of the Atypical type this output will provide. Must be a type that's
   //       already been defined.
   // outputFunc: A function of no arguments that returns the output value. This value MUST be an
   //             Atypical type object of the same type you declared it to output, otherwise you'll
   //             get unusual behaviour. The only exception is with primitive values, that can
   //             be auto-wrapped in an Atypical type. This function will be called at every tick,
   //             and at any time the output value is needed.
   defineOutput: function(type, outputFunc) {
      if (!AT.typeIsDefined(type)) {
         console.error("Type " + type.name + ", used as output for " + this.label
            + ", not defined");
         return;
      }
      
      this.output = new OutputPort(type, outputFunc.bind(this));
   },

   // Returns the type of data this sketch has been configured to output (as an Atypical type
   // constructor), or null if there is no output type.
   getOutputType: function() {
      if (this.output instanceof OutputPort) {
         // New-style output port with type information
   ...
   // Determines whether this sketch can accept an input value of the given type at the given port.
   //
   // inputValueType: The constructor of the type that you want to check. Must be the
   //                 constructor of an already-defined Atypical type, otherwise returns false.
   // inputPortNumber: (optional) The number (>= 0) of the port on this sketch you want to check
   //                  type compatibility for.
   //                  If not specified, assumes you're talking about the first non-filled port.
   canAcceptInputOfType: function(inputValueType, inputPortNumber) {
      if (!this.inputs) {
         // The "inputs" variable only gets created when a sketch is added to the board.
         // Other sketch objects, such as those used for prototypes, don't have inputs,
         // so we ignore those.
      ...
      if (!AT.typeIsDefined(inputValueType)) {
         return false;
      }

      if (inputPortNumber === undefined) {
         inputPortNumber = this.inputs.firstOpenPort();
      }
      
      if (inputPortNumber < 0 || inputPortNumber >= this.inputs.numPorts()) {
         return this.inputs.hasDynamicPort() ;
      }
      return this.inputs._canAcceptValueOfType(inputPortNumber, inputValueType);
   },
   ...
   delete : function() {
      ...
      if (i >= 0) {
         ...
         if (this.inputs.hasLink(0)) {
            inLink = this.inputs.link(0);
            if (isDef(this.output) && this.output.links.length > 0) {
               outLink = this.output.links[0];
            }
         }
         ...
         for (var inPort = 0 ; inPort < this.inputs.numPorts(); inPort++)
            if (this.inputs.hasLink(inPort)) {
               var a = this.inputs.link(inPort).a;
               if (isDef(a.output)) {
                  for (var k = a.output.links.length - 1 ; k >= 0 ; k--) {
                     var link = a.output.links[k];
                     if (link.b == this)
         ...
         if (isDef(this.output)) {
            for (var k = this.output.links.length - 1 ; k >= 0 ; k--) {
               var link = this.output.links[k];
               link.removeFromInSketch();
            }
         }

         // IF POSSIBLE, LINK DELETED SKETCH'S INPUT TO ITS OUTPUT.
         // But only do this if the two sketches are compatible.
         if (inLink != null && outLink != null
            && outLink.b.canAcceptInputOfType(inLink.a.getOutputType(), outLink.j))
         {
            let ca = inLink.C, cb = outLink.C;
            let start = ca.length > 0 ? ca[0] : [inLink.a.cx(), inLink.a.cy()];
            let mid = mix(ca.length > 0 ? ca[ca.length-1] : [inLink.a.cx(), inLink.a.cy()],
               cb.length > 0 ? cb[0] : [outLink.b.cx(), outLink.b.cy()], .5);
            let end = cb.length > 0 ? cb[cb.length-1] : [outLink.b.cx(), outLink.b.cy()];
            new SketchLink(inLink.a, outLink.b, outLink.j, 2 * computeCurvature(start, mid, end));
         }
      }
   ...
   drawText : function(context) {
      ...
      if (this instanceof FreehandSketch && this.isNullText()
         && this.inputs.hasValue(0))
      {
         var str = "";
         if (this.inputs.hasValue(0)) {
            str = this.inputs.value(0);
         }
         ...
         if (typeof str == 'string') {
            ...
            let lines = str.split("\n");
            var dx = textWidth(lines[0]) / 2;
            if (lines[0].indexOf('[') < 0) {
               var i = lines[0].indexOf('.');
               if (i >= 0)
                  dx = textWidth(lines[0].substring(0, i));
            }

            for (let line = 0; line < lines.length; line++) {
               _g_text(lines[line], [ this.cx() - dx, this.cy() + (.5 + line) * fontHeight ], m);
            }
            context.restore();
         }
         return;
   ...
   evalCode : function(code, x, y, z) {
      ...
      x = def(x, def(this.inValue_DEPRECATED_PORT_SYSTEM[0]));
      y = def(y, def(this.inValue_DEPRECATED_PORT_SYSTEM[1]));
      z = def(z, def(this.inValue_DEPRECATED_PORT_SYSTEM[2]));
   ...
   getInValue_DEPRECATED_PORT_SYSTEM : function(j, dflt) {
      return this.inValues_DEPRECATED_PORT_SYSTEM[j] !== undefined ? valueOf(this.inValues_DEPRECATED_PORT_SYSTEM[j], time) : dflt;
   },
   ...
   isInValueAt_DEPRECATED_PORT_SYSTEM : function(j) {
      return j >= 0 ? isDef(this.inValue_DEPRECATED_PORT_SYSTEM[j]) : false;
   },
   isNullText : function() {
   ...
   portXY_DEPRECATED_PORT_SYSTEM : function(i) {
      if (isDef(this.portLocation_DEPRECATED_PORT_SYSTEM[i])) {
         if (this instanceof FreehandSketch)
            return [this.cx(), this.cy()];
         else {
            m.save();
            this.standardView();
            var p = this.portLocation_DEPRECATED_PORT_SYSTEM[i];
   ...
   outPortIndex_DEPRECATED_PORT_SYSTEM : function(forceCreation) {
      var i = getIndex(this.portName_DEPRECATED_PORT_SYSTEM, 'out');
      if (i == -1 && forceCreation !== undefined) {
         this.addPort_DEPRECATED_PORT_SYSTEM('out', 0, 0);
         i = getIndex(this.portName_DEPRECATED_PORT_SYSTEM, 'out');
      }
      return i;
   },
   ...
   setOutPortValue_DEPRECATED_PORT_SYSTEM : function(value) {
      this.outPortIndex_DEPRECATED_PORT_SYSTEM(true);
      this.setOutValue_DEPRECATED_PORT_SYSTEM('out', value);
   },
   setOutValue_DEPRECATED_PORT_SYSTEM : function(name, value) {
      var j = getIndex(this.portName_DEPRECATED_PORT_SYSTEM, name);
      if (j >= 0)
         this.outValue_DEPRECATED_PORT_SYSTEM[j] = value;
   },
   ...
   useInputColors: function() {
      this.defineInput(AT.Color, function (col) {
         // TODO: consider undeprecating this label or refactoring it
         this.inLabel_DEPRECATED_PORT_SYSTEM[0] = 'color';
...
function FreehandSketch() {
   ...
   // Keep this array up-to-date if the inputs change.
   FreehandSketch.inputTypes = [AT.Expression, AT.String, AT.Unknown];

   this.init = function() {
      this.defineAllRemainingInputs(AT.Expression, function(exp) {
         let result;
         try {
            result = exp.eval();
         }
      ...
      this.defineAlternateInputType(AT.String);
      this.defineAlternateInputType(AT.Unknown, function(unk) {
         return valueToString(unk.value);
      });

      this.defineOutput(AT.Expression, function() {
         if (this.isNullText()) {
            if (this.inputs.hasValue(0)) {
            ...
            if (!this.inputs.hasValue(0) 
               && !this.inputs.hasValue(1) 
               && !this.inputs.hasValue(2))
            {
               // No inputs? Just return the text as an expression.
               return this.text;
            }
            // Otherwise, add variables representing the inputs.
            //
            // With things done this way, if you have e.g. "sin(x)" as your text, and you
            // connect it to something expecting a Function(Float, Float), then:
            // - If you don't have any inputs, the result after conversion should be:
            //   (function(x) { return sin(x); })
            //   With the x argument being provided by the Expression -> Function conversion
            // - If you do have inputs, though, we're going to pre-specify the x, y, and z
            //   variables, so that after conversion it should actually be:
            //   (function() { let x = 4; return sin(x); })()
            //   Which, after conversion, ends up as:
            //   (function(x) { (function() { let x = 4; return sin(x); })() })
            //   I.e. outputting a number.
            let expression = "(function() {";
            if (this.inputs.hasValue(0)) {
               expression += "let x = " + this.inputs.value(0) + ";";
            }
            if (this.inputs.hasValue(1)) {
               expression += "let y = " + this.inputs.value(1) + ";";
            }
            if (this.inputs.hasValue(2)) {
               expression += "let z = " + this.inputs.value(2) + ";";
            }
            expression += "return " + this.text + "})()";
            // TODO: if the first input is an array, consider mapping the function across
            // the values
            return expression;
         }
      });
   ...
   this.mouseDown = function(x, y) {
      ...
      if (this.text.length > 0 && !this.hasInputText())
         return;
   ...
   this.hasInputText = function() {
      return this.inputs.hasValue(0);
   }
...
function NumericSketch() {
   ...
   this.render = function() {
      ...
      if (this.hasInputText()) {
         this.setText(this.getInputText());
         this.value = this.text;
      }
   }
}
