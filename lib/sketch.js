...
function Sketch() {
   ...
   this.isSelectedByUID  = UID_Flags.NULL;
   this.isMouseOverByUID = UID_Flags.NULL;
...
Sketch.prototype = {
   ...
   modelEndFrame : function() {
      if (this._called_beginFrame) {
		 ...
		 // for unity
		 for (let f = 0; f < meshDataSendCallbackQueue.length; f += 1) {
            meshDataSendCallbackQueue[f]();
         }
         meshDataSendCallbackQueue = [];
      }
   },
   modeler : function(init, update) {
      if (! this._modelerObj) {
	 this.is3D = true;
         this._modelerObj = ctScene.add(init());
      }
      if (! this.fadeAway) {
         this.extendBounds([[-1,-1],[1,1]]);
         this._modelerObj.placeOnSketch();
         if (update) {
            this._modelerObj._update = update;
            this._modelerObj._update();
         }
      }
      else
         ctScene.remove(this._modelerObj);
   },
   noReload : function(names) {
      this._noReload = namesArray(names);
   },
   rayFromCamera : function() {
      let C = this._camRay;
      if (C === undefined)
         this._camRay = C = { _mat: new M4() };
      if (C._s === undefined && ! isNaN(this.xhi))
         C._s = 2 * this.scale() / max(this.xhi - this.xlo - 2 * sketchPadding,
                                       this.yhi - this.ylo - 2 * sketchPadding);
      let x = (sketchPage.x - this.cx()) * C._s / this.scale();
      let y = (sketchPage.y - this.cy()) * C._s / this.scale();
      this.standardViewInverse(C._mat);
      C.W = scale(normalize(C._mat.Z()), -1);
      C.V = add(scale(normalize(C._mat.X()), x),
            add(scale(normalize(C._mat.Y()), y),
                scale(C.W, -ctScene.getFL())));
      C.W[3] = 0;
      return C;
   },
   reload : function() {
      let s = this;
      if (s.typeName == 'props_sketch')
         s = s.outSketch(0);
      if (s.typeName) {
         let j = s.typeName.indexOf('_');
         let fileName = s.typeName.substring(0, j) + '.js';
         window._load_sketch = s;
         loadSketches(fileName);
      }
   },
   ...
   drawBoundsTranslated : function(tx, ty) {
      if (this.parent == null) {
         lineWidth(this == sk() ? this.isMouseOver ? 4 : 2 : 0.5);


          drawRoundRect(this.xlo + tx, this.ylo + ty, (this.xhi - this.xlo), (this.yhi - this.ylo), sketchPadding);
      }
   },
   drawBoundsTranslatedSpecifyWidth : function(tx, ty, width) {
      if (this.parent == null) {
         _g.save();
         lineWidth(width);
   ...
   drawText : function(context) {
      ...
      // DISPLAY INPUT VALUE FROM A LINK AS A TEXT STRING.
	    ...
	    for (let line = 0 ; line < nLines ; line++) {
	       let s  = strs[line],
               ...
               if (s.indexOf('[') < 0) {
	              ...
	              ch = s.charAt(i - 1);
	          if (ch == '-' || ch >= '0' && ch <= '9')
                     dx = textWidth(s.substring(0, i));
               }
               _g_text(s, [ this.cx() - dx, this.cy() + (line - (nLines - 1) / 2) * fontHeight ], m);
            }
   ...
   renderWrapper : function(elapsed) {
      ...
      if (this.draw && (this.createMesh !== undefined ? this.glyphTransition < 0.5 : this.sketchProgress < 1)) {
         _g.save();
         _g.globalAlpha = 1 - this.styleTransition;
         this.draw();
         _g.restore();
      }
      if (this.isAfterSketch() || ! this.update) {
         let render_or_update = () => {
	         if (this.update)
	            this.update(elapsed);
            else
               this.render(elapsed);
...
CT.Object.prototype.placeOnSketch = function() {
   if (! isk())
      return this;

   let cx =  ( sk().xlo + sk().xhi - width () ) / width();
   let cy = -( sk().ylo + sk().yhi - height() ) / width();
   let ss = norm(vecDiff(m.transform([-1,0,0]), m.transform([1,0,0])));
   let yy = min(1, 4 * sk().rY * sk().rY);

   return this.identity().translate(cx, cy , -this.getScene().getFL())
                         .rotateX(PI * -sk().rY)
                         .rotateY(PI *  sk().rX * (1 - yy))
                         .rotateZ(PI * -sk().rX * yy)
                         .scale(ss * sk().xyzS() / width());
}
