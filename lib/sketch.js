...
Sketch.prototype = {
   ...
   adjustParamCurves : function() {
      var a, bc, bs, c, i, iterate, n, pcurves, s, strokes, strokeCurves;

      if (this.params && this.curves &&
          this.sketchTrace && this.sketchTrace.length == this.curves.length && ! this._curvesAreAdjusted) {

	 // CLONE THE FREEHAND USER STROKES, THEN TRANSFORM THEM INTO THE COORDS OF THE SKETCH.

         strokes = cloneArray(this.sketchTrace);
         pcurves = buildParamCurves(this.params, this.curves);

         bs = curvesComputeBounds(strokes);
         bc = curvesComputeBounds(pcurves);

         s = sqrt((bc.width * bc.height) / (bs.width * bs.height));

         for (n = 0 ; n < strokes.length ; n++)
            for (i = 0 ; i < strokes[n].length ; i++) {
               strokes[n][i][0] = s * (strokes[n][i][0] - bs.x) + bc.x;
               strokes[n][i][1] = s * (bs.y - strokes[n][i][1]) + bc.y;
            }

         // CREATE A SET OF CURVES THAT MIMICS THE STRUCTURE OF THE PARAM CURVES.

         strokeCurves = [];
         for (n = 0 ; n < pcurves.length ; n++)
            strokeCurves.push(mimicParamCurve(strokes[n], this.params, this.curves[n]));

         // ITERATIVELY CHANGE THE PARAMS SO THE PARAM CURVES TRY TO MATCH THE FREEHAND CURVES.

         for (iterate = 0 ; iterate < 10 ; iterate++) {
            for (n = 0 ; n < pcurves.length ; n++)
               for (i = 0 ; i < pcurves[n].length ; i++)
                  for (a = 0 ; a < 2 ; a++)
                     this.params[this.curves[n][i][a]] = mix(this.params[this.curves[n][i][a]],
		                                             strokeCurves[n][i][a], 0.1);
            if (this.constraints)
         ...
         this._curvesAreAdjusted = true;

	 this.xyz = []; // FORCE A RECOMPUTATION OF "BEST FIT" FOR MORPH TRANSITION FROM FREEHAND SKETCH.
      }
   },
   ...
   modelBeginFrame : function() {
      if (this._isModel) {
         ...
         this.model()
            .color(mix(bg, palette.getRGB(this.colorId), this.getAlpha()))
            .xform(CT.matrixMultiply(CT.matrixTranslated(
