...
function Sketch() {
   ...
   this.inLinks = []; // array of SketchLink
   this.outLinks = []; // array of array of SketchLink
...
Sketch.prototype = {
   ...
   findLinkedSketches : function() {                             // FIND SKETCHES MUTUALLY
      ...
      while (C.length > nC) {                                    // STARTING WITH THIS SKETCH.
         ...
         for (n = 0 ; n < C.length ; n++) {
            s = C[n];
            for (port = 0 ; port < s.outLinks.length ; port++)
               if (isDef(outLinks = s.outLinks[port]))
                  for (i = 0 ; i < outLinks.length ; i++)
                     if (C.indexOf(outLinks[i].b) < 0)
                        C.push(outLinks[i].b);
            for (port = 0 ; port < s.inLinks.length ; port++)
               if (isDef(inLink = s.inLinks[port]))
   ...
   modelEndFrame : function() {
      if (this._called_beginFrame) {
         if (this.inValue_DEPRECATED_PORT_SYSTEM[0] && typeof this.inValue_DEPRECATED_PORT_SYSTEM[0] != 'function')
            this.model().applyRecursively('color', this.inValue_DEPRECATED_PORT_SYSTEM[0]);
   ...
   getOutputType: function() {
      ...
      else if (this.USES_DEPRECATED_PORT_SYSTEM) {
         // Old-style output port with no type information, use the "Unknown" type
         return AT.Unknown;
      }
   },
   ...
   canAcceptInputOfType: function(inputValueType, inputPortNumber) {
      ...
      if (inputPortNumber === undefined) {
         inputPortNumber = this.inLinks.indexOf(undefined);
         if (inputPortNumber < 0) {
            inputPortNumber = this.inLinks.length;
         }
      }
      
      if (this.USES_DEPRECATED_PORT_SYSTEM) {
         // Only "Unknown" typed values can be fed into this.
         // (Values that are plain javascript objects are also considered "Unknown" typed.)
         return AT.canConvert(inputValueType, AT.Unknown);
      }
      else {
         if (inputPortNumber >= this.inputs.numPorts()) {
            return false;
         }
         return this.inputs.port(inputPortNumber)._canAcceptValueOfType(inputValueType);
      }
   },
   ...
   delete : function() {
      ...
      if (i >= 0) {
         ...
         if (this.inLinks.length > 0 && isDef(this.inLinks[0])) {
            inLink = this.inLinks[0];
            var outPortIndex_DEPRECATED_PORT_SYSTEM = this.outPortIndex_DEPRECATED_PORT_SYSTEM();
            if (outPortIndex_DEPRECATED_PORT_SYSTEM >= 0) {
               var outLinks = this.outLinks[outPortIndex_DEPRECATED_PORT_SYSTEM];
         ...
         for (var inPort = 0 ; inPort < this.inLinks.length ; inPort++)
            if (isDef(this.inLinks[inPort])) {
               var a = this.inLinks[inPort].a;
               for (var outPort = 0 ; outPort < a.outLinks.length ; outPort++)
                  if (isDef(a.outLinks[outPort]))
                     for (var k = a.outLinks[outPort].length - 1 ; k >= 0 ; k--) {
                        var link = a.outLinks[outPort][k];
                        if (link.b == this)
         ...
         for (var outPort = 0 ; outPort < this.outLinks.length ; outPort++)
            if (isDef(this.outLinks[outPort]))
               for (var k = this.outLinks[outPort].length - 1 ; k >= 0 ; k--) {
                  var link = this.outLinks[outPort][k];
            ...
            && outLink.b.canAcceptInputOfType(inLink.a.getOutputType(), outLink.j))
         {
            ...
            new SketchLink(inLink.a, inLink.i, outLink.b, outLink.j,
               2 * computeCurvature(start, mid, end));
         }
      }
   ...
   drawText : function(context) {
      ...
      if (this instanceof FreehandSketch && this.isNullText()
         && (isDef(this.inValue_DEPRECATED_PORT_SYSTEM[0])
   ...
   findInLink_DEPRECATED_PORT_SYSTEM : function(inPort) {
      if (this.inLinks.length > inPort && isDef(this.inLinks[inPort])) {
         var a = this.inLinks[inPort].a;
         for (var outPort = 0 ; outPort < a.outLinks.length ; outPort++)
            if (isDef(a.outLinks[outPort]))
               for (var k = a.outLinks[outPort].length - 1 ; k >= 0 ; k--)
                  return a.outLinks[outPort][k];
      }
      return undefined;
   },
   getAlpha : function() {
   ...
   useInputColors_DEPRECATED_PORT_SYSTEM : function() {
      this.afterSketch(function() {
         if (this.inValue_DEPRECATED_PORT_SYSTEM[0] !== undefined) {
            this.inLabel_DEPRECATED_PORT_SYSTEM[0] = 'color';
            this.ambient = this.diffuse = this.inValue_DEPRECATED_PORT_SYSTEM[0];
            if (this.inValue_DEPRECATED_PORT_SYSTEM[1] !== undefined) {
               this.inLabel_DEPRECATED_PORT_SYSTEM[0] = 'ambient';
               this.inLabel_DEPRECATED_PORT_SYSTEM[1] = 'diffuse';
               this.diffuse = this.inValue_DEPRECATED_PORT_SYSTEM[1];
               if (this.inValue_DEPRECATED_PORT_SYSTEM[2] !== undefined) {
                  this.inLabel_DEPRECATED_PORT_SYSTEM[2] = 'specular';
                  this.specular = this.inValue_DEPRECATED_PORT_SYSTEM[2].concat([30]);
               }
            }
         }
      });
...
function FreehandSketch() {
   ...
   this.init = function() {
      let addInput = function() {
         this.defineInput(AT.Expression, function(exp) {
            let result;
            try {
               result = exp.eval();
            }
