...
Sketch.prototype = {
   ...
   clone : function() {
      function isGetterOrSetter(obj, prop) {
         let proto = null;
         if (obj === undefined || 
             obj === null || 
             (proto = Object.getPrototypeOf(obj)) === null) {
            return false;
         }
         ...
         return {
            isGetterOrSetter : (descriptor['get'] !== undefined || 
                   descriptor['set'] !== undefined), 
            descriptor : descriptor
         };
      }
      
      const ignoredArraysDuringCopy = [
         "children",
         "defaultValue",
         "in",
         "inValue",
      ...
      // function cloneObject(obj, selfRef, selfCopy = undefined) {
      //    if (obj === undefined || obj === null) {
      //       return obj;  
      //    }
      //    var dst = Object.create(obj);
      ...
      //       const check = isGetterOrSetter(obj, prop);
      //       if (check.isGetterOrSetter) {
      //          Object.defineProperty(Object.getPrototypeOf(dst), prop, check.descriptor);
      //       }
      //       else if (obj[prop] instanceof Array) {
      //          // IGNORE IN / OUT, LINK ARRAYS, REPLACE WITH EMPTY ARRAYS
      //          if (ignoredArraysDuringCopy.indexOf(prop) > -1) {
      //             dst[prop] = [];
      //          }
      //          else {
      //             dst[prop] = cloneArrayWithSelfRef(
      //                obj[prop], undefined, selfRef, selfCopy
      //             );
      //          }
      //       }
      //       else if (obj[prop] instanceof Clonable) {
      //          dst[prop] = Object.create(obj[prop]);
      //       }
      //       else if (obj[prop] instanceof Object &&
      //           ! (obj[prop] instanceof Function)) {
      //          // IF ENCOUNTERED, 
      //          // ORIGINAL SKETCH'S "this" POINTERS
      //          // NOT COPIED, BUT RATHER REPLACED WITH
      //          // THE POINTER TO THE SKETCH COPY-IN-PROGRESS
      //          if (obj[prop] === selfRef) {
      //             dst[prop] = selfCopy;
      //          }
      //          else {
      ...
      //                if (selfCopy.createMesh) { // MUST BE SHALLOW COPY, DIFFERENTIATE BETWEEN MESH / NON-MESH
      //                   dst[prop] = obj[prop];
      //                }
      //                else {
      //                   // CLONE RECURSIVELY
      //                   dst[prop] = cloneObject(obj[prop], selfRef, selfCopy);
      //                }
      //             }
      //          }
      //       }
      ...
      // function cloneArrayWithSelfRef(src, dst, selfRef, selfCopy) {
      //    if (src === undefined) {
      //       return undefined;
      //    }
      //    if (dst === undefined) {
      ...
      //          dst[i] = cloneArrayWithSelfRef(src[i], dst[i], selfRef, selfCopy);
      //       }
      //       else if (src[i] instanceof Clonable) {
      //          dst[i] = Object.create(obj[prop]);
      //       }
      ...
      //          }
      //          else {
      //             dst[i] = cloneObject(src[i], selfRef, selfCopy);
      //          }
      //       }
      //       else {
      //          dst[i] = src[i];
         ...
         for (var prop in obj) {
            ...
            if (check.isGetterOrSetter) {
               ...
               if (ignoredArraysDuringCopy.indexOf(prop) > -1) {
                  ...
                  dst[prop] = cloneArrayWithSelfRef(obj[prop]);
               }
            }
            else if (obj[prop] instanceof Clonable) {
               // CHECK FOR CIRCULAR REFERENCES
               const idx = findCircularRef(obj[prop]);
               if (idx >= 0) {
                  dst[prop] = cpyRefs[idx];
               }
               else {
                  dst[prop] = Object.create(obj[prop]);
               }
               ...
               if (idx >= 0) {
                  ...
                  const descriptor = Object.getOwnPropertyDescriptor(obj, prop);
                  if (descriptor === undefined || descriptor.writable) {
                     if (srcRefs[0].createMesh) { // MUST BE SHALLOW COPY, DIFFERENTIATE BETWEEN MESH / NON-MESH
                        dst[prop] = obj[prop];
                     }
                     else {
                        dst[prop] = cloneObject(obj[prop]);
                     }
                  }
               }
            }
      ...
      function cloneArrayWithSelfRef(src, dst) {
         if (src === undefined) {
            return undefined;
         }
         if (dst === undefined) {
         ...
         for (var i = 0 ; i < src.length ; i++) {
            if (src[i] instanceof Array) {
               dst[i] = cloneArrayWithSelfRef(src[i], dst[i]);
            }
            else if (src[i] instanceof Clonable) {
               // CHECK FOR CIRCULAR REFERENCES
               const idx = findCircularRef(obj[prop]);
               if (idx >= 0) {
                ...
                ! (src[i] instanceof Function)) {
               // CHECK FOR CIRCULAR REFERENCES
               const idx = findCircularRef(src[i]);
               if (idx >= 0) {
                  dst[i] = cpyRefs[idx];
               }
               else {
