...
function Sketch() {
   ...
   this.in_DEPRECATED_PORT_SYSTEM = []; // array of SketchLink
   ...
   this.out_DEPRECATED_PORT_SYSTEM = []; // array of array of SketchLink
...
Sketch.prototype = {
   ...
   initInstanceVariables: function() {
      ...
      this.defaultValue_DEPRECATED_PORT_SYSTEM = [];
      this.out_DEPRECATED_PORT_SYSTEM = [];
      this.outValue_DEPRECATED_PORT_SYSTEM = [];
      this.portBounds_DEPRECATED_PORT_SYSTEM = [];
      this.portLocation_DEPRECATED_PORT_SYSTEM = [];
      this.in_DEPRECATED_PORT_SYSTEM = [];
   ...
   findLinkedSketches : function() {                             // FIND SKETCHES MUTUALLY
      ...
      while (C.length > nC) {                                    // STARTING WITH THIS SKETCH.
         ...
         for (n = 0 ; n < C.length ; n++) {
            s = C[n];
            for (port = 0 ; port < s.out_DEPRECATED_PORT_SYSTEM.length ; port++)
               if (isDef(outLinks = s.out_DEPRECATED_PORT_SYSTEM[port]))
                  for (i = 0 ; i < outLinks.length ; i++)
                     if (C.indexOf(outLinks[i].b) < 0)
                        C.push(outLinks[i].b);
            for (port = 0 ; port < s.in_DEPRECATED_PORT_SYSTEM.length ; port++)
               if (isDef(inLink = s.in_DEPRECATED_PORT_SYSTEM[port]))
   ...
   //
   // typename: The name of the Atypical type this input port should accept. This input port will
   ...
   //                        raw input value.
   defineInput: function(typename, preprocessingFunction) {
      ...
      if (!AT.typeIsDefined(typename)) {
         console.error("Typename " + typename + ", used as input "
   ...
   //
   // typename: The name of the Atypical type you want to add support for. This input port will.
   ...
   defineAlternateInputType: function(typename, preprocessingFunction) {
      // The "inputs" variable only gets created when a sketch is added to the board.
      // Other sketch objects, such as those used for prototypes, don't have inputs,
      // so we ignore those.
      if (!isDef(this.inputs)) {
      ...
      // Validate typename
      if (!AT.typeIsDefined(typename)) {
         console.error("Typename " + typename + ", used as alternate input type for port "
      ...
      this.inputs.port(i).addAcceptedType(typename, preprocessingFunction);
   },
   ...
   //
   // typename: The name of the Atypical type this output will provide. Must be a type that's
   //           already been defined.
   // outputFunc: A function of no arguments that returns the output value. This value MUST be an
   //             Atypical type object of the same type as the given typename, otherwise you'll
   //             get unusual behaviour. This function will be called at every tick, and at
   //             any time the output value is needed.
   defineOutput: function(typename, outputFunc) {
      if (!AT.typeIsDefined(typename)) {
         console.error("Typename " + typename + ", used as output for " + this.label
            + ", not defined");
         return;
      }
      
      this.output = new OutputPort(typename, outputFunc.bind(this));
   },

   // Returns the type of data this sketch has been configured to output (as an Atypical type name),
   // or null if there is no output type.
   getOutputTypename: function() {
      if (this.output instanceof OutputPort) {
         // New-style output port with type information
         return this.output.typename;
      }
   ...
   //
   // inputValueTypename: The name of the type that you want to check, as a string. Must be the
   //                     name of an already-defined Atypical type, otherwise returns false.
   ...
   canAcceptInputOfType: function(inputValueTypename, inputPortNumber) {
      if (!this.inputs) {
         // The "inputs" variable only gets created when a sketch is added to the board.
         // Other sketch objects, such as those used for prototypes, don't have inputs,
         // so we ignore those.
      ...
      if (typeof(inputValueTypename) !== "string" || !AT.typeIsDefined(inputValueTypename)) {
         return false;
      }

      if (inputPortNumber === undefined) {
         inputPortNumber = this.in_DEPRECATED_PORT_SYSTEM.length;
      }
      ...
      if (this.USES_DEPRECATED_PORT_SYSTEM) {
         ...
         return AT.canConvert(inputValueTypename, "Unknown");
      }
      else {
         if (inputPortNumber >= this.inputs.numPorts()) {
            return false;
         }
         return this.inputs.port(inputPortNumber)._canAcceptValueOfType(inputValueTypename);
      }
   },
   ...
   delete : function() {
      ...
      if (i >= 0) {
         ...
         if (this.in_DEPRECATED_PORT_SYSTEM.length > 0 && isDef(this.in_DEPRECATED_PORT_SYSTEM[0])) {
            inLink = this.in_DEPRECATED_PORT_SYSTEM[0];
            var outPortIndex_DEPRECATED_PORT_SYSTEM = this.outPortIndex_DEPRECATED_PORT_SYSTEM();
            if (outPortIndex_DEPRECATED_PORT_SYSTEM >= 0) {
               var outLinks = this.out_DEPRECATED_PORT_SYSTEM[outPortIndex_DEPRECATED_PORT_SYSTEM];
         ...
         for (var inPort = 0 ; inPort < this.in_DEPRECATED_PORT_SYSTEM.length ; inPort++)
            if (isDef(this.in_DEPRECATED_PORT_SYSTEM[inPort])) {
               var a = this.in_DEPRECATED_PORT_SYSTEM[inPort].a;
               for (var outPort = 0 ; outPort < a.out_DEPRECATED_PORT_SYSTEM.length ; outPort++)
                  if (isDef(a.out_DEPRECATED_PORT_SYSTEM[outPort]))
                     for (var k = a.out_DEPRECATED_PORT_SYSTEM[outPort].length - 1 ; k >= 0 ; k--) {
                        var link = a.out_DEPRECATED_PORT_SYSTEM[outPort][k];
                        if (link.b == this)
         ...
         for (var outPort = 0 ; outPort < this.out_DEPRECATED_PORT_SYSTEM.length ; outPort++)
            if (isDef(this.out_DEPRECATED_PORT_SYSTEM[outPort]))
               for (var k = this.out_DEPRECATED_PORT_SYSTEM[outPort].length - 1 ; k >= 0 ; k--) {
                  var link = this.out_DEPRECATED_PORT_SYSTEM[outPort][k];
         ...
         if (inLink != null && outLink != null
            && outLink.b.canAcceptInputOfType(inLink.a.getOutputTypename(), outLink.j))
         {
   ...
   findInLink_DEPRECATED_PORT_SYSTEM : function(inPort) {
      if (this.in_DEPRECATED_PORT_SYSTEM.length > inPort && isDef(this.in_DEPRECATED_PORT_SYSTEM[inPort])) {
         var a = this.in_DEPRECATED_PORT_SYSTEM[inPort].a;
         for (var outPort = 0 ; outPort < a.out_DEPRECATED_PORT_SYSTEM.length ; outPort++)
            if (isDef(a.out_DEPRECATED_PORT_SYSTEM[outPort]))
               for (var k = a.out_DEPRECATED_PORT_SYSTEM[outPort].length - 1 ; k >= 0 ; k--)
                  return a.out_DEPRECATED_PORT_SYSTEM[outPort][k];
      }
      return undefined;
   },
   getAlpha : function() {
...
function FreehandSketch() {
   ...
   this.init = function() {
      this.defineInput("String", function(str) { return str.str; });
      this.defineAlternateInputType("Unknown", function(unk) { return valueToString(unk.val); });
   }
