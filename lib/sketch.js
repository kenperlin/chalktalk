...
console.log("calling this.updateFragmentShader");
	    ...
	    var A = .025, D = .3, S = .9, P = 30;

            // UPDATE MESH COLOR IF NEEDED.

            var ambient  = this.ambient !==undefined ? this.ambient  : [A,A,A];
            var diffuse  = this.diffuse !==undefined ? this.diffuse  : [D,D,D];
            var specular = this.specular!==undefined ? this.specular : [S,S,S,P];

            if (this.meshColorId !== this.colorId) {
               var rgb = paletteRGB[this.colorId];
               this.ambient = [A * rgb[0] / 255, A * rgb[1] / 255, A * rgb[2] / 255];
               this.diffuse = [D * rgb[0] / 255, D * rgb[1] / 255, D * rgb[2] / 255];
               this.meshColorId = this.colorId;
            }
