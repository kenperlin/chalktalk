...
Sketch.prototype = {
   ...
   getOutputTypename: function() {
      if (this.output instanceof OutputPort) {
         ...
         return outSketch.output.typename;
      }
   ...
   //                     name of an already-defined Atypical type.
   // inputPortNumber: The number (>= 0) of the port on this sketch you want to check type
   //                  compatibility for.
   canAcceptInputWithType: function(inputValueTypename, inputPortNumber) {
      if (!this.inputs) {
         // The "inputs" variable only gets created when a sketch is added to the board.
         // Other sketch objects, such as those used for prototypes, don't have inputs,
         // so we ignore those.
   ...
   drawText : function(context) {
            ...
            || this.inputs.hasValue(0)))
      {
         ...
         if (this.inputs.hasValue(0)) {
            // The conversion system should have already made this a string
            str = this.inputs.value(0);
         }
         else {
            str = this.stringifyData(valueOf(this.inValue_DEPRECATED_PORT_SYSTEM[0], time));
         }
   ...
   stringifyData : function(arg) {
      if (arg instanceof AT.Type) {
         if (arg.canConvert("String")) {
            return arg.convert("String").str;
         }
         else if (arg instanceof AT.UnknownType) {
            // This is a value that hasn't been updated to use the type system yet.
            // It's only a thin wrapper around a Javascript object, so convert it and unwrap it.
            return valueToString(arg.val);
         }
         else {
            // This is an Atypical object with no string conversion possible.
            // Return some kind of default string
            return "some value";
         }
      }
      else {
         // This is a non-Atypical value.
...
function FreehandSketch() {
   ...
   this.hasInputText = function() {
      return this.inputs.hasValue(0) || isDef(this.inValue_DEPRECATED_PORT_SYSTEM[0]);
   }
   ...
   this.getInputText = function() {
      if (this.inputs.hasValue(0)) {
         ...
         value = this.inValue_DEPRECATED_PORT_SYSTEM[0];
         return isNumeric(value) ? roundedString(value) : value;
      }
   }
