...
function FreehandSketch() {
   ...
   this.render = function() {
      ...
      if (this.keysStartTime) {
         var p = [0, 0, 0];
         var q = [0, 0, 0];
         var t = (time - this.keysStartTime) % (this.keys.length - 1);
	 var i = 0;
         this.sp[i++] = [ 0, 0, 0 ];
	 var keyIndex0 = floor(t);
	 var keyIndex1 = keyIndex0 + 1;
	 if (keyIndex1 == this.keys.length - 1)
	    keyIndex1 = 0;
	 for (var strokeIndex = 0 ; strokeIndex < this.keys[0].length ; strokeIndex++) {
	    for (var f = 0 ; f < 1 ; f += 1 / 100) {
	       var _p = sample(this.keys[keyIndex0][strokeIndex], f);
	       var _q = sample(this.keys[keyIndex1][strokeIndex], f);
	       _p[0] -= this.tX;
	       _p[1] -= this.tY;
               this.xform(_p, p);
	       _q[0] -= this.tX;
	       _q[1] -= this.tY;
               this.xform(_q, q);
               this.sp[i++] = [ mix(p[0], q[0], t % 1), mix(p[1], q[1], t % 1), f > 0 ];
	    }
	 }
      }
      else {
