...
Sketch.prototype = {
   ...
   computeCursorPoint : function(x, y, z) {
      if (this._cursorPoint === undefined)
   ...
   drawText : function(context) {
      ...
      if (this instanceof FreehandSketch && this.isNullText() && isDef(this.inValue[0])) {
         ...
         if (typeof str == 'string') {
            ...
            context.fillText(str, this.cx() - dx, this.cy() + .5 * fontHeight);
            context.restore();
         }
         return;
      }
...
function FreehandSketch() {
   ...
   this.mouseUp = function(x, y) {
      ...
      // ABSORB ANY OTHER FREEHAND SKETCH THAT THE SKETCH INTERSECTS.

      var B = computeCurveBounds(this.sp, 1);
      for (var I = 0 ; I < nsk() ; I++)
         if ( sk(I) != sk() && isFreehandSketch(sk(I)) &&
...
/*
// This won't work with this.viewForwardMatrix and this.viewInverseMatrix.
// In order to add this feature back in, we would need to account for those
// matrices needing to change properly.
...
*/
   ...
   this.convertToGlyphSketch = function() {
      ...
      if (glyph != null)
         glyph.toSketch();
      sk().glyph = glyph;
      sk().setColorId(glyphSketch.colorId);
   ...
   this._compute_p = function(x, y, z) {
      this._p.set(x, y, z);
      this.pixelToPoint(this._p);
/*
      var _p1 = this.viewInverseMat.transform([this._p.x, this._p.y, this._p.z]);
      var _p2 =      viewForwardMat.transform(_p1);
*/
      var _p2 = viewForwardMat.transform([this._p.x, this._p.y, this._p.z]);

      this._p.set(_p2[0], _p2[1], _p2[2]);
      this.pointToPixel(this._p);
   }
...
________*/
	       this._compute_p(sp[i][0], sp[i][1], 0);
               _g.moveTo(this._p.x, this._p.y);
