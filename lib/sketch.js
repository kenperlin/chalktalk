   ...
   function FreehandSketch() {
	 ...
	 if (! this.isClick) {
	    ...
	    if ( sk(I) != sk() && isFreehandSketch(sk(I)) &&
	         B[0] <= sk(I).xhi && B[2] >= sk(I).xlo &&
	         B[1] <= sk(I).yhi && B[3] >= sk(I).ylo )
               sk().absorb(sk(I--));
	 ...
	 // ADJUST TO BEST CENTER POINT TO ROTATE AND SCALE ABOUT.
         ...
         if (this.sc == 1 || this.tX == 0 && this.tY == 0) {
            ...
            var dx = ((B[0] + B[2]) / 2 - this.tX) / this.sc;
            var dy = ((B[1] + B[3]) / 2 - this.tY) / this.sc;
            this.tX += dx * this.sc;
            this.tY += dy * this.sc;
   ...
   StrokesSketch.prototype = new FreehandSketch;
