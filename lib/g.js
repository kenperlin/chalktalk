...
////////// CONTENT
   ...
   bgDragGestures[-1] = ['pan', function(x, y) {
      ...
      _g.panY += bgAction_dy;
/*
      _g.panX = max(_g.panX, width() * (1 - overviewScale / 2));
      _g.panX = min(_g.panX, width() * (    overviewScale / 2));

      _g.panY = max(_g.panY, height() * (1 - overviewScale / 2));
      _g.panY = min(_g.panY, height() * (    overviewScale / 2));
*/
   ...
   bgUpGestures[0][ 2] = ['toggle'        , function() { sketchPage.isWhiteboard = ! sketchPage.isWhiteboard; }];
   ...
   bgUpGestures[7][-1] = ['images'        , function() { imageLibrary_isVisible = true; }];
...
*/
         ...
         if (! imageLibrary_isVisible && ! isShowingGlyphs)
         if (! isVerticalPan) {
            if (isRightHover) {
               annotateStart();
               _g.save();
	    ...
	    if (overviewAlpha < 1) {
	       var w = width(), h = height();
	       var xmin = width () * (1/2 - overviewScale / 2);
	       var xmax = width () * (1/2 + overviewScale / 2);
	       var ymin = height() * (1/2 - overviewScale / 2);
	       var ymax = height() * (1/2 + overviewScale / 2);
