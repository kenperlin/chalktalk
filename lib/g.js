                  ...
                  for (let i = 0 ; i < S.outLinks.length ; i++) {
                     if (isDef(S.outLinks[i])) {
                        ...
                        for (let k = 0 ; k < S.outLinks[i].length ; k++) {
                           ...
                           if (receiver.USES_DEPRECATED_PORT_SYSTEM
                              && outValue.canConvert(AT.Unknown))
                           {
...
//window.displayListData = data;
   ...
   function addSketch(sketch) {
      ...
      // Generate output ports for sketches that use the old ports system
      if (sketch.USES_DEPRECATED_PORT_SYSTEM) {
         if (typeof sketch.output === "function") {
            let outputFunc = sketch.output;
            sketch.defineOutput(AT.Unknown, outputFunc);
         }
         else {
            sketch.defineOutput(AT.Unknown, function() {
               return sketch.outValue_DEPRECATED_PORT_SYSTEM[0];
            });
         }
      }
   }
