         ...
         if (userCursor.prevWheelActive && wheelMag <= INACTIVE_THRESH) {
            ...
            if (userCursor.wheelRecenteredCount == 1) {
               ...
               case 3: { // copy
                  ...
                  deferred[fn_CT_OBJ_FWBW] = null;
                  deferred[fn_CT_OBJ_SELECTED] = null;
                  selectedCTObject.obj.selectedByUID = -2;
...
*/
      ...
      canvas.onmousemoveNonHost = function(event) {
...
//console.log(clientY);
         var handle = getHandle(this);
         const userRecord = getOrCreateUserRecord(event.uid);
         userRecord.cursor.x = panX(clientX);
         userRecord.cursor.y = panY(clientY);
         userRecord.cursor.z = panZ(clientZ);
         //////////////////////////////////////////////////////////////

         // MOUSE IS BEING MOVED WITHOUT BUTTONS PRESSED.

         if (isDef(handle.mouseMove)) {
            if (sketchAction == 'rotating') {
               userRecord.rotate_travel += len(userRecord.x - userRecord.rotate_x, userRecord.y - userRecord.rotate_y);
               userRecord.rotate_x = userRecord.x;
               userRecord.rotate_y = userRecord.y;
            }
            handle.mouseMove(userRecord.cursor.x, userRecord.cursor.y, userRecord.cursor.z);
         }



         userRecord.lastX = clientX;
         userRecord.lastY = clientY;
         userRecord.lastZ = clientZ;
...
////////// CONTENT
   ...
   bgDragGestures[3] = ['appearance',
      ,
      function(x, y) {
         console.log("appearance change disabled");
         return;
