...
////////////////////////////////////////////////////////////////
   ...
   function initEventHandlers(canvas) {
	 ...
	 return keyCode;
      ...
      canvas.clientTogglePalette = function(event) {
         const uid = event.uid;
         const userRecord = getOrCreateUserRecord(uid);

         if (event.turnOnPalette) {
         ...
         if (userCursor.prevWheelActive && wheelMag <= INACTIVE_THRESH) {
            ...
            if (userCursor.wheelRecenteredCount == 1) {
               ...
               case 4: { // scale
                  ...
                  selectedCTObject.lockedPosition[0] = This().mouseX;
                  selectedCTObject.lockedPosition[1] = This().mouseY;
                  beginSketchActionWithSelection(userRecord, selectedCTObject.obj, actionIndex);
                  break;
               }
               case 5: { // rotation
