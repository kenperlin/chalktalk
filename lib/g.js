...
//window.displayListener = true;
   ...
   function SelectionUserData() {
      // map uid to selection info // stores SelectionUserRecords
      this.uidToRecords = {};
   }
   let selectionUserData = new SelectionUserData();
   ...
   function getOrCreateSelectionUserRecord(uid) {
      let userRecord = selectionUserData.uidToRecords[uid];
      if (!userRecord) {
         userRecord = new SelectionUserRecord();
         selectionUserData.uidToRecords[uid] = userRecord;
      }
      return userRecord;
   }
	...
	// support resolution request from other clients
     ...
     canvas.clientInitialize = function(event) {
      ...
      window.resetSelections = function() {
         ...
         selectionUserData   = new SelectionUserData();
      ...
      canvas.disableSelectionForAllOtherClients = function(arg) {
         ...
         for (let uidIdx = 0; uidIdx < userIDs.length; uidIdx += 1) {
            ...
            const userRecord = getOrCreateSelectionUserRecord(uid);
      ...
      canvas.clientBeginMoveGroupOrSketchFromPage = function(event) {
         ...
         if (group.isAtCursor) {
            ...
            deferred[fn_CT_OBJ_SELECTED] = (function() {
               return function() {
                  return selectedGroupAnimationFn(selectedCTObject);
               };
            })();
      ...
      try {
         ...
         if (selectedCTObject.obj == null || event.timestamp <= selectedCTObject.beginTimestamp) {
            console.log("old end command received t=[" + time + "]");
            return;
         }
         ...
         console.log("deselection completed, t=[" + time + "]");
         selectedCTObject.kind = 0;
         selectedCTObject.obj = null;
...
*/
      ...
      // (Karl) TODO: PROBABLY WRAP IN A renderPages() FUNCTION, BUT NOT UNTIL FINALIZED
      {
         ...
         for (let uid = 0; uid < userIDs.length; uid += 1) {
            const deferredInfo = deferredUserFunctions.uidToFunctions[uid];
            if (deferredInfo) {
