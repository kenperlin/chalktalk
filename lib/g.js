...
// TODO: FIGURE OUT HOW TO LOAD THE NEWLY CREATED SKETCH TYPE INTO CURRENT GLYPHS.
   ...
   function gStart() {
      ...
      + " <canvas id='background' color='" + backgroundColor + "'>"
      + " <div id='code'"
      + "    style='z-index:1;position:absolute;left:0;top:0;'>"
      + " </div>"
   ...
   function endSketchDragAction(x, y) {
      ...
      case 4:
         ...
         if (sk() != outSketch && sk().isMouseOver) {
            ...
            if (canCreateLinkToBackgroundWithType(outSketch.getOutputType())) {
      ...
      if (isDef(window[g.name].animate)) {
        ...
        // beyond reasonable doubt not to be buggy
         ...
         if (isAudiencePopup() || !(isShowingGlyphs && isDef(This().overlay))) {
            ...
            if (sketchDragMode == 4) {
               let i = getIndexOfSketchAtCursor();
               let outputType = outSketch.getOutputType();
                        ...
                        for (var k = 0 ; k < a.outLinks[i].length ; k++) {
                           ...
                           if (!window.isPressed && isMouseNearCurve(link.C)) {
                              ...
                              if (sketchAction !== "translating"
                                 || sk().canAcceptInputOfType(link.a.getOutputType())
                                 || link.b.canAcceptInputOfType(sk().getOutputType()))
                              {
                                 link.draw(true);
                              }
                           }
