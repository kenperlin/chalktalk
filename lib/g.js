   ...
   function initEventHandlers(canvas) {
	...
	// support resolution request from other clients
	  ...
	  canvas.createSketchPage = function(event){
		  ...
		  sketchBook.setPage(pageIndex+1);//creation
		  setPage(pageIndex+1);//update related info
...
*/
	  ...
	  if (window.forceSetPageAtTime !== undefined && forceSetPageAtTime < time) {
	   ...
	   // let sketchPageCount = sketchPages.length;
	   // let curPageIdx = pageIndex;
	   // for (let sketchPageIdx = 0; sketchPageIdx < sketchPageCount; sketchPageIdx++) {
		  //  //setPage(sketchPageIdx);

    //      if (curPageIdx == sketchPageIdx) {
    //         continue;
    //      }


		  //  setPageWOChangingState(sketchPageIdx, curPageIdx);

		  //  restOfTick(g);
	   // }

    //   setPageWOChangingState(curPageIdx, curPageIdx);
      ...
      // (Karl) TODO: PROBABLY WRAP IN A renderPages() FUNCTION, BUT NOT UNTIL FINALIZED
      {
         ...
         for (let sketchPageIdx = 0; sketchPageIdx < sketchPageCount; sketchPageIdx++) {
            // RENDER THE ACTIVE PAGE SEPARATELY OUTSIDE THE LOOP
            // (BUT WILL PROBABLY NEED A SEPARATE FUNCTION FOR THE INACTIVE PAGES TO AVOID RENDERING NON-SKETCH CONTENT)
            if (sketchPageIdx == curPageIdx) {
               continue;
            }
            renderPage(g, sketchPageIdx);
         }
