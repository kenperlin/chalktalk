...
///////////////// SUPPORT FOR MULTIPLE WEB CLIENTS ///////////////////
   ...
   function broadcastEvent(type, x, y) {
      server.broadcastEvent(localWebClientName + ',' + type + ',' + x + ',' + y);
   }
...
////////////////////////////////////////////////////////////////
   ...
   function initEventHandlers(canvas) {
      ...
      server.onBroadcast = function(event) {
         ...
         currentWebClient = webClientList.clients[data.client];
...
/////////////////////////////////////////////////////////////////////////////
   ...
   var tick = function(g) {
      ...
      if (webClientList.size() > 1) {
         annotateStart();
         var i = 0;
	 var x = paletteX(-1) + 2 * paletteR(-1);
	 var y = paletteY(0) - paletteR(-1) * 1.5;
	 var s = 24;
         for (var name in webClientList.clients) {
	    console.log(name);
	    var rgb = paletteRGB[i++];
	    color( 'rgba(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ',0.4)' );
	    fillRect(x, y, s*.9, s*.9);
	    x += s;
	 }
         annotateEnd();
      }
