...
*/
   ...
   function _g_beginPath()      { draw_beginPath(); }
   function _g_fill()           { draw_fill(); }
   function _g_fillText(s,x,y)  { draw_fillText(s, x, y); }
   function _g_lineTo(x,y)      { _g_sketchTo([x, y, 0], 1); }
   function _g_moveTo(x,y)      { _g_sketchTo([x, y, 0], 0); }
   function _g_stroke()         { draw_stroke(); }

   function _g_sketchTo(_p, isLine) {
      var x, y, z, xx, yy, p;

      if (! isDrawingSketch) {
         draw_pathTo(_p, isLine);
         return;
      }
...
//////////////////////////////////////////////////////////////////////////////////
   ...
   function draw_beginPath() {
      _path = [];
   }

   function draw_pathTo(p, isLine) {
      _path.push([ p[0], p[1], p[2], isLine ]);
   }

   function draw_stroke() { draw_stroke_or_fill(0); }
   function draw_fill() { draw_stroke_or_fill(1); }
   function draw_stroke_or_fill(isFill) {
      var x, y, z, i;
      function drawTo(x, y, isLine) {
         if (! isLine)
	    _g.moveTo(x, y);
         else
	    _g.lineTo(x, y);
      }
      function stroke_or_fill(isFill) {
         if (isFill)
            _g.fill();
         else
            _g.stroke();
      ...
      if (isAnimatingView) {
	    ...
	    drawTo(_path[i][0] / 2 - dz * _path[i][2],
                   _path[i][1] / 2 + dh / 2,
                   _path[i][3]);
         stroke_or_fill(isFill);

         _g.beginPath();
         for (i = 0 ; i < _path.length ; i++)
	    drawTo(_path[i][0] / 2 + dz * _path[i][2] + dw,
                   _path[i][1] / 2 + dh / 2,
                   _path[i][3]);
         stroke_or_fill(isFill);
         _g.lineWidth *= 2;
         ...
         _g.beginPath();
         for (i = 0 ; i < _path.length ; i++)
            drawTo(_path[i][0], _path[i][1], _path[i][3]);
         stroke_or_fill(isFill);
      }
   }

   function draw_fillText(str, x, y) {
      if (! isAnimatingView) {
         _g.fillText(str, x, y);
	 return;
      }
      var f = _g.font;
      _g.font = (parseFloat(f) / 2) + f.substring(f.indexOf('p'), f.length);
      x = x / 2;
      y = y / 2 + height() / 4;
      _g.fillText(str, x, y);
      x += width() / 2;
...
/////////////////////////////////////////////////////////////////////////////
   ...
   var tick = function(g) {
      ...
      if (isAnimatingView) {
         ...
         viewForwardMat.rotateY(sin(2 * time));
         viewForwardMat.translate(-width()/2, -height()/2, 0);
      }
