   ...
   function initEventHandlers(canvas) {
	...
	// support resolution request from other clients
     ...
     canvas.clientInitialize = function(event) {
      ...
      canvas.sendNum = function(hdr, num) {
         console.log("sending a number");
         var data = new Uint16Array(4 + 2), header = hdr, i;
         for (i = 0; i < 4; i += 1) {
            data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
         }
      ...
      canvas.clientBeginMoveGroupOrSketchFromPage = function(event) {
         ...
         if (sketch == null) {
            ...
            selectedCTObject.type = 0; // none

            canvas.sendNum(header, 0);
            return;
         }
      ...
      canvas.clientEndMoveGroupOrSketchFromPage = function(event) {
         ...
         case 1: // sketch
            moveSketchToPageAtIndex(selectedCTObject.obj, event.dstPageIdx);
            break;
         case 2: // group
            moveGroupToPageAtIndex(selectedCTObject.obj, event.dstPageIdx);
            break;
         default: // none
            break;
         }
...
*/
      ...
      // (Karl) TODO: PROBABLY WRAP IN A renderPages() FUNCTION, BUT NOT UNTIL FINALIZED
      {
         ...
         if (deferred.isReady) {
            deferred.createAndSetSketchPage();

            deferred.isReady = false;
            deferred.createAndSetSketchPage = null;
         }
      }
...
//          }
         ...
         if (group.sketchPageIndex != actualPageIdx) {
            console.log("This should run as soon as I switch a page");
            group.clear();
            //group.sketchPageIndex = actualPageIdx;
         }
         //group.update(This().elapsed);
