         ...
         if (sketchAction != null) {
            ...
            case 'translating':
               ...
               if (linkAtCursor != null
                  && (sk().canAcceptInputOfType(linkAtCursor.a.getOutputTypename())
                     || linkAtCursor.b.canAcceptInputOfType(sk().getOutputTypename())))
               {
                  var s = linkAtCursor.s;
                  linkAtCursor.remove();
                  var j = 0;
                  while (sk().in_DEPRECATED_PORT_SYSTEM[j] !== undefined)
                     j++;

                  // Connect the left side, if it's compatible.
                  if (sk().canAcceptInputOfType(linkAtCursor.a.getOutputTypename())) {
                  ...
                  if (linkAtCursor.b.canAcceptInputOfType(sk().getOutputTypename())) {
...
/////////////////////////////////////////////////////////////////////
   ...
   function computeLinkCurvature(link, C) {
      link.s = computeCurvature(link.a.portXY_DEPRECATED_PORT_SYSTEM(link.i), C, link.b.portXY_DEPRECATED_PORT_SYSTEM(link.j));
   ...
   function drawPortData(sketch, port, dataValues, isAlwaysDrawing) {
      ...
      var xy = sketch.portXY_DEPRECATED_PORT_SYSTEM(port);
   ...
   // Draws a line for a possible/in-progress sketch link (while it's being drawn).
   // 
   // fromSketch: The sketch you're outputting from. Should have a linkCurve variable set, to
   //             define the rough shape of the curve.
   // terminatorDecoration: (optional) If specified, adds a terminator to the line (e.g. an
   //                       arrowhead.) If not specified, leaves the end of the line bare.
   //                       Can be one of these values:
   var POSSIBLE_LINK_ARROW_HEAD = 1; // Draws an arrowhead at the end of the curve.
   var POSSIBLE_LINK_NO_LINK = 2; // Draws an "X" at the end of the curve.
   function drawPossibleLink(fromSketch, terminatorDecoration) {
      lineWidth(dataLineWidth);
      color(fadedColor(0.5));
      if (fromSketch.linkCurve != null) {
         let C = fromSketch.linkCurve;
         // linkCurve has been tracking mouse position while the user's been dragging the link
         // around. Think of it as a scribble that here gets smoothed into a nice curve.
         window._debug_ = true;
         let curve = createCurvedLine(C[0], C[C.length-1], computeCurvature(C));
         drawCurve(curve);
         if (terminatorDecoration === POSSIBLE_LINK_ARROW_HEAD) {
            let a = curve.length >= 2 ? curve[curve.length-2] : curve[curve.length-1];
            let b = curve[curve.length-1];
            arrowHead(a[0], a[1], b[0], b[1], 10);
         }
         else if (terminatorDecoration === POSSIBLE_LINK_NO_LINK) {
            // Make an X with two arrow heads
            let curveEnd = curve[curve.length-1];
            let dir = [1, 0];
            arrowHead(curveEnd[0] + dir[0], curveEnd[1] + dir[1],
               curveEnd[0], curveEnd[1], 15);
            arrowHead(curveEnd[0] - dir[0], curveEnd[1] - dir[1],
               curveEnd[0], curveEnd[1], 15);
         }
   ...
   function findNearestInPort(sketch) {
      ...
      if (sketch != null) {
         ...
         for (var i = 0 ; i < sketch.portName_DEPRECATED_PORT_SYSTEM.length ; i++)
            if (sketch.portName_DEPRECATED_PORT_SYSTEM[i] !== 'out')
               inPortCount++;

         if (inPortCount > 0)
            inPort = findNearestPortAtCursor(sketch, sketch.in_DEPRECATED_PORT_SYSTEM, true);
         else
            inPort = firstUndefinedArrayIndex(sketch.in_DEPRECATED_PORT_SYSTEM);
      }
   ...
   function findNearestOutPort(sketch) {
      if (sketch.portName_DEPRECATED_PORT_SYSTEM.length == 0)
         return 0;
   ...
   function findNearestPortAtCursor(sketch, slots, isOnlyInPorts) {
      ...
      var n = -1, ddMin = 10000;
      for (var i = 0 ; i < sketch.portName_DEPRECATED_PORT_SYSTEM.length ; i++) {
         if (isOnlyInPorts && sketch.portName_DEPRECATED_PORT_SYSTEM[i] == "out")
            continue;
         if ((slots === undefined) || slots[i] == null) {
            var xy = sketch.portXY_DEPRECATED_PORT_SYSTEM(i);
   ...
   function findOutPortAtCursor(sketch) {
      if (sketch instanceof NumericSketch ||
          sketch instanceof FreehandSketch &&
                 (! sketch.isNullText() || isDef(sketch.inValue_DEPRECATED_PORT_SYSTEM[0])))
         return -1;

      var x = This().mouseX;
      var y = This().mouseY;
      for (var i = 0 ; i < sketch.portName_DEPRECATED_PORT_SYSTEM.length ; i++)
         if (sketch.defaultValue_DEPRECATED_PORT_SYSTEM[i] !== undefined && sketch.outValue_DEPRECATED_PORT_SYSTEM[i] !== undefined) {
            var xy = sketch.portXY_DEPRECATED_PORT_SYSTEM(i);
...
var sketchDragY = 0;
   ...
   function startSketchDragAction(x, y) {
      ...
      case 4:
         ...
         if (outPort == -1) {
            outPort = outSketch.outPortIndex_DEPRECATED_PORT_SYSTEM(true);
         }
         // When creating a link, we build up a list of mouse positions, a curve that influences
         // the shape of the link arrow after it's drawn.
   ...
   function endSketchDragAction(x, y) {
      var code, dir, fileData, name, typeName;
      ...
      case 4:
         ...
         if (sk() != outSketch && sk().isMouseOver) {
            ...
            // (If it has an output port and can convert to supported background types.)
            if (canCreateLinkToBackgroundWithType(outSketch.getOutputTypename())) {
         ...
         if (sketchPage.createLink(inSketch, inPort, outSketch, outPort)) {
            let i = outSketch.out_DEPRECATED_PORT_SYSTEM[outPort].length - 1;
            outSketch.out_DEPRECATED_PORT_SYSTEM[outPort][i].s = computeCurvature(
               outSketch.linkCurve);
         }
   ...
   function tryToSelectSketchAtCursor() {
      let i = getIndexOfSketchAtCursor();
      if (i >= 0) {
         selectSketch(i);
      }
   }

   // Returns -1 if there is no sketch at the cursor
   function getIndexOfSketchAtCursor() {
      ...
      if (isDef(window[g.name].animate)) {
        ...
        // beyond reasonable doubt not to be buggy
         ...
         if (isAudiencePopup() || !(isShowingGlyphs && isDef(This().overlay))) {
            ...
            if (sketchDragMode == 4) {
               let i = getIndexOfSketchAtCursor();
               let outputType = outSketch.getOutputTypename();
               if (isMouseOverBackground || i < 0) {
                  if (canCreateLinkToBackgroundWithType(outputType))
                  {
                     drawPossibleLink(outSketch, POSSIBLE_LINK_ARROW_HEAD);
                  }
                  else {
                     drawPossibleLink(outSketch, POSSIBLE_LINK_NO_LINK);
                  }
               }
               else {
                  let destinationSketch = sk(i);
                  let destinationPort = findNearestInPort(destinationSketch);

                  if (destinationSketch === outSketch) {
                     // Draw link with no arrowhead when linking to yourself
                     drawPossibleLink(outSketch);
                  }
                  else if (outputType !== null
                     && destinationSketch.canAcceptInputOfType(outputType, destinationPort))
                  {
                     drawPossibleLink(outSketch, POSSIBLE_LINK_ARROW_HEAD);
                  }
                  else {
            ...
            for (var I = 0 ; I < nsk() ; I++) {
               if (sk(I).parent == null) {
                  var a = sk(I);
                  for (var i = 0 ; i < a.out_DEPRECATED_PORT_SYSTEM.length ; i++) {
                     if (isDef(a.out_DEPRECATED_PORT_SYSTEM[i])) {
                        for (var k = 0 ; k < a.out_DEPRECATED_PORT_SYSTEM[i].length ; k++) {
                           var link = a.out_DEPRECATED_PORT_SYSTEM[i][k];
                           link.draw(true);
                           
                           if (!window.isPressed && isMouseNearCurve(link.C)) {
                              linkAtCursor = link;
                              // Draw links thicker if you move your mouse near them.
                              //
                              // But if you're translating, first check whether the sketch you're
                              // moving is compatible with at least one side of the link you'd be
                              // inserting it into, so as to not give misleading feedback.
                              if (sketchAction !== "translating"
                                 || sk().canAcceptInputOfType(link.a.getOutputTypename())
                                 || link.b.canAcceptInputOfType(sk().getOutputTypename()))
                              {
                                 link.draw(true);
                              }
                           }
         ...
         // Update all output values for all sketches that use the new port system
         for (var I = 0 ; I < nsk() ; I++) {
            if (sk(I).output instanceof OutputPort) {
               sk(I).output._updateValue();
            }
         }
         
         // Clear all input values for all sketches
         for (var I = 0 ; I < nsk() ; I++) {
            sk(I).inputs._clear();
         }
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            ...
            if (S.out_DEPRECATED_PORT_SYSTEM.length > 0 || S.containsTextAssignment()) {

               if (S.output instanceof OutputPort) {
                  // If we're using the new port system, propagate the value using this instead.

                  // TODO: either replace this "out" or undeprecate it.
                  for (let i = 0 ; i < S.out_DEPRECATED_PORT_SYSTEM.length ; i++) {
                     if (isDef(S.out_DEPRECATED_PORT_SYSTEM[i])) {
                        let outValue = S.output.val;
                        
                        // Skip objects with undefined or incorrect output values
                        if (!outValue || !(outValue instanceof AT.Type)) {
                           continue;
                        }

                        for (let k = 0 ; k < S.out_DEPRECATED_PORT_SYSTEM[i].length ; k++) {
                           let link = S.out_DEPRECATED_PORT_SYSTEM[i][k];
                           let receiver = link.b;

                           if (receiver.inputs.hasPort(link.j)) {
                              receiver.inputs.port(link.j)._receiveValue(outValue);
                           }

                           // For objects that don't yet use the new port system, propagate to the
                           // old one by converting to Unknown (if possible) and then unwrapping.
                           if (outValue.canConvert("Unknown")) {
                              let unknown = outValue.convert("Unknown");
                              receiver.inValue_DEPRECATED_PORT_SYSTEM[link.j] = unknown.val;
                           }
                        }
                     }
                  }
                  ...
                  if (typeof S.output == 'function') {
                     S.outValue_DEPRECATED_PORT_SYSTEM[0] = S.output();
                  }

                  // ELSE IF SKETCH HAS TEXT, EVALUATE IT.
                  // IF THERE IS ANY RESULT, PASS IT TO OUTPUT.

                  else if (! S.isNullText()) {
                     S.evalResult = S.evalCode(S.text);
                     if (S.evalResult != null && isDef(S.out_DEPRECATED_PORT_SYSTEM[0]))
                        S.outValue_DEPRECATED_PORT_SYSTEM[0] = S.evalResult;
                  }

                  // PROPAGATE VALUES ALONG LINKS.

                  for (var i = 0 ; i < S.out_DEPRECATED_PORT_SYSTEM.length ; i++) {
                     if (isDef(S.out_DEPRECATED_PORT_SYSTEM[i])) {
                        var outValue = isDef(S.outValue_DEPRECATED_PORT_SYSTEM[i]) ? S.outValue_DEPRECATED_PORT_SYSTEM[i] : "0";
                        for (var k = 0 ; k < S.out_DEPRECATED_PORT_SYSTEM[i].length ; k++) {
                           var link = S.out_DEPRECATED_PORT_SYSTEM[i][k];
                           link.b.inValue_DEPRECATED_PORT_SYSTEM[link.j] = outValue;

                           // For objects that use the new port system, propagate to their input
                           // ports as well by converting to an Unknown type.
                           if (link.b.inputs.hasPort(link.j)) {
                              link.b.inputs.port(link.j)._receiveValue(new AT.Unknown(outValue));
                           }
                        }
                     }
                  }
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            var S = sk(I);
            S.inValues_DEPRECATED_PORT_SYSTEM = [];
            for (var i = -1 ; i < S.in_DEPRECATED_PORT_SYSTEM.length ; i++) {
               var val = S.inValue_DEPRECATED_PORT_SYSTEM[i];
               if (isDef(val)) {
                  if (Array.isArray(val))
                     for (var k = 0 ; k < val.length ; k++)
                        S.inValues_DEPRECATED_PORT_SYSTEM.push(val[k]);
                  else
                     S.inValues_DEPRECATED_PORT_SYSTEM.push(val);
               }
            }
            ...
            if (S.isNullText())
               S.outValue_DEPRECATED_PORT_SYSTEM[0] = S.inValues_DEPRECATED_PORT_SYSTEM;
         }
         ...
         if (showingLiveDataMode > 0 || isShowingLiveDataAtPort) {
            ...
            if (showingLiveDataMode >= 1 || isShowingLiveDataAtPort) {
               ...
               if (outSketch != null && outPort >= 0 && ! (outSketch instanceof NumericSketch)) {
                  var val = outSketch.outValue_DEPRECATED_PORT_SYSTEM[outPort];
            ...
            if (showingLiveDataMode >= 2)
               for (var I = 0 ; I < sketchPage.sketches.length ; I++) {
                  ...
                  for (var i = 0 ; i < s.portName_DEPRECATED_PORT_SYSTEM.length ; i++)
                     if (s.outValue_DEPRECATED_PORT_SYSTEM[i] !== undefined && s.inValue_DEPRECATED_PORT_SYSTEM[i] === undefined) {
                        var xy = s.portXY_DEPRECATED_PORT_SYSTEM(i);
                        var val = s.outValue_DEPRECATED_PORT_SYSTEM[i];
...
//window.displayListData = data;
   ...
   // Returns whether or not we can create a link to "the background" (i.e. a FreehandSketch)
   // through a link of the given type.
   function canCreateLinkToBackgroundWithType(typename) {
      // Since we'd be linking to a FreehandSketch, check compatibility with the types it supports.
      return typeof(typename) === "string"
         && (  AT.canConvert(typename, "String") // String and Unknown are the only two 
            || AT.canConvert(typename, "Unknown")) // input types FreehandSketch supports.
   }
   ...
   function copySketch(s) {
      ...
      sk().portLocation_DEPRECATED_PORT_SYSTEM = cloneArray(s.portLocation_DEPRECATED_PORT_SYSTEM);
...
////////////////////////////////////////////////////////////
   ...
   function GeometrySketch() {
      ...
      this.render = function(elapsed) {
         ...
         if (this.inValue_DEPRECATED_PORT_SYSTEM[0] !== undefined) this.setUniform("x", this.inValue_DEPRECATED_PORT_SYSTEM[0]);
         if (this.inValue_DEPRECATED_PORT_SYSTEM[1] !== undefined) this.setUniform("y", this.inValue_DEPRECATED_PORT_SYSTEM[1]);
         if (this.inValue_DEPRECATED_PORT_SYSTEM[2] !== undefined) this.setUniform("z", this.inValue_DEPRECATED_PORT_SYSTEM[2]);
