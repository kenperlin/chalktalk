...
*/
   ...
   function _g_beginPath() { _g.beginPath();          }
   function _g_moveTo(x,y) { _g_pathTo([x, y, 0], 0); }
   function _g_lineTo(x,y) { _g_pathTo([x, y, 0], 1); }
   function _g_stroke()    { _g.stroke();             }


   function _g_pathTo(_p, isLine) {
      var x, y, z, xx, yy, p;

      if (! isDrawingSketch) {
         drawLineWrapper(_p, isLine);
	 return;
      }
	 ...
	 yy = y;
      ...
      p = [ sketch.adjustX(x), sketch.adjustY(y), z ];
   ...
   // ACTUAL LINE DRAWING GOES THROUGH HERE.

   function drawLineWrapper(p, isLine) {
      if (! isLine)
         _g.moveTo(p[0], p[1]);
      else
         _g.lineTo(p[0], p[1]);
