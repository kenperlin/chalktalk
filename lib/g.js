...
*/
      ...
      function broadcastMouseEvent(type, event) {
         server.broadcastEvent(JSON.stringify({
            "eventType": type,
            "event": {
               fake: 1,
      ...
      canvas.onmousedown = function(event) {
         ...
         if (!event.fake)
            broadcastMouseEvent("onmousedown", event);
      ...
      canvas.onmouseup = function(event) {
         ...
         if (!event.fake)
            broadcastMouseEvent("onmouseup", event);
...
*/
      ...
      canvas.onmousemove = function(event) {
         ...
         if (!event.fake)
            broadcastMouseEvent("onmousemove", event);
   ...
   function _g_lineTo(x,y)      { _g_sketchTo([x, y, _g_z], 1); }
   function _g_moveTo(x,y)      { _g_sketchTo([x, y, _g_z], 0); }
...
/////////////////////////////////////////////////////////////////////////////
   ...
   var tick = function(g) {
      ...
      if (isPhone() && time > 5 && ! window._isBird) {
	 window._isBird = true;
	 addSketch(new bird_sketch());
	 finishSketch();
	 sk().choice.setState(1);
	 sk().scale(2);
	 isCTObject = true;
      }
