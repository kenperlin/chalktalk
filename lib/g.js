...
var sketchDragY = 0;
   ...
   var displayList,holodeckData=[0];
   var eventClientId = -1;
...
//console.log("data" + data);
		 ...
		 if(window.holodeckListener)(function() {
            var data = new Uint16Array(4 + holodeckData.length), header = 'Holodeck', i;
			for (i = 0 ; i < 4 ; i++)
				data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
			for (i = 0 ; i < data.length ; i++)
				data[i + 4] = holodeckData[i];
            server.socket.send(data.buffer);
         })();
