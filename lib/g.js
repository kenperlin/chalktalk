...
//-------------------------------------------------------------------------------
   ...
   function sg(type, selection) {
      ...
      if (sk().computeStatistics != null && sk().sketchTrace != null)
...
////////// SUPPORT CODE
   ...
   var bgDragGestures = [];
   var bgClickGestures = [];
   (function() { var i; for (i = -1 ; i < 8 ; i++) bgDragGestures[i] = []; })();
   (function() { var i; for (i = -1 ; i < 8 ; i++) bgClickGestures[i] = []; })();
   ...
   function bgClickGesture(x, y) {
      if (bgClickGestures[bgAction_dir1][bgAction_dir2] !== undefined)
         if (bgClickGestures[bgAction_dir1][bgAction_dir2][1] !== undefined)
            (bgClickGestures[bgAction_dir1][bgAction_dir2][1])();
   }
...
////////// CONTENT
   ...
   bgClickGestures[-1][0] = ['pan'          ];
   bgClickGestures[0][-1] = ['prev\nimage'  , function() { sketchPage.previousImage(); }];
   bgClickGestures[0][ 2] = ['toggle'       , function() { imageLibrary_isShowingImage = ! imageLibrary_isShowingImage; }];
   bgClickGestures[1][-1] = ['lock'         , function() { sketchPage.isGlyphable = ! sketchPage.isGlyphable; }];
   bgClickGestures[2][-1] = ['select\ncolor', function() { palette.isPieMenu = true; }];
   bgClickGestures[3][-1] = ['show\nglyphs' , function() { isShowingGlyphs = true; }];
   bgClickGestures[4][-1] = ['next\nimage'  , function() { sketchPage.nextImage(); }];
   bgClickGestures[5][-1] = ['draw\ntext'   , function() { toggleTextMode(); }];
   bgClickGestures[6][-1] = ['zoomed\nview' , function() { isOverviewMode = true; }];
   bgClickGestures[7][-1] = ['images'       , function() { imageLibrary_isShowingLibrary = true; }];
...
/////////////////////////////////////////////////////////////////////////////
   ...
   function sketchClickActionName(dir, sketch) {
      ...
      if (sketch !== undefined && ! isSimpleSketch(sketch))
         switch (dir) {
         case 1: name = 'unparse' ; break;
         case 5: name = 'cmd'     ; break;
         case 7: name = 'see code'; break;
         }
      return name;
   }
   ...
   function doSketchClickAction(x, y) {
      ...
      case 1:
         if (isSimpleSketch(sk()) && ! (sk() instanceof NumericSketch ))
            sk()._isGlyphable = ! sk()._isGlyphable;
         else if (sk().glyph !== undefined) {
            sk().fadeAway = 1;
            sk().glyph.toSimpleSketch((sk().xhi + sk().xlo) / 2,
                                      (sk().yhi + sk().ylo) / 2,
				      max(sk().xhi - sk().xlo, sk().yhi - sk().ylo) - 2 * sketchPadding);
         }
         break;
      case 2:
...
*/
      ...
      if (isDef(window[g.name].animate)) {
	    ...
	    :                                            backgroundColor == 'white' ? 'cell' : 'crosshair'
         ...
         else if ( bgClickCount == 0 &&
	           ! palette.isPieMenu &&
	           ! group.isCreating &&
	           ! group.contains(sketchPage.x, sketchPage.y) &&
	           (! sketchPage.isGlyphable || isk() && isHover() && ! sk().isGlyphable()) ) {
	    annotateStart();
	    color(scrimColor(0.5, isk() && isHover() ? sk().colorId : sketchPage.colorId));
	    fillOval(sketchPage.x - 10, sketchPage.y - 10, 20, 20);
	    annotateEnd();
	 }
