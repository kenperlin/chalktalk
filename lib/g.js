...
*/
     ...
     //    ctPath.flush();
      ...
      if (window.displayListener)
         (function() {
            {
               var data = new Uint16Array(4 + displayList.length + window.moreData), header = 'CTdata01', i;
               if (data.length != window.prevAmt) {
                  console.log(data.length + ":bytes=" + (data.length * 2));
                  window.prevAmt = data.length;
               }
         	    for (i = 0 ; i < 4 ; i++)
         	       data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
