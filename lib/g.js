...
*/
   ...
   function endSketchDragAction(x, y) {
      switch (sketchDragMode) {
      case 0:
         var dir = pieMenuIndex(x - this._sketchDragX, y - this._sketchDragY);
         if (isFreehandSketch(sk())) {
            switch (dir) {
	       ...
	       if (isCodeWidget) {
	          ...
	          console.log('uploaded new code for sketch ' + sk().typeName);
               }
	       break;
            }
	 }
	 ...
	 if (isFreehandSketch(sk()) && sk().text.length > 0) {
            var dir = pieMenuIndex(x - this._sketchDragX, y - this._sketchDragY, 8);
	    switch (dir) {
	    case 1:
	       ...
	       sketchFileData[typeName] = [ name + '.js' , code ];
...
*/
   ...
   function doSketchClickAction(x, y) {
      ...
      case 0:
         ...
         break;
      case 1:
         if (isFreehandSketch(sk()) && ! (sk() instanceof NumericSketch )) {

	    // CONVERT A TEXT CHARACTER INTO THE SIMPLE SKETCH THAT WOULD GENERATE IT.
	    ...
	    if (sk().text.length > 0) {
	       ...
	       setTextMode(false);
            ...
            // CONVERT A PROCEDURAL SKETCH INTO THE SIMPLE SKETCH THAT WOULD GENERATE IT.

            sk().fade();
            sk().glyph.toFreehandSketch((sk().xhi + sk().xlo) / 2,
                                      (sk().yhi + sk().ylo) / 2,
                                      max(sk().xhi - sk().xlo, sk().yhi - sk().ylo) - 2 * sketchPadding);
         }
         break;
...
*/
            ...
            if (lsRequest.responseText.indexOf('collections') >= 0)
               ...
               server.get('state/collections', function(value) {
                  ...
                  for (var i = 0 ; i < collections.length ; i++)
                     addGlyph(collections[i].id, collections[i].strokes);
               });
         }
         lsRequest.send();
      }
      ...
      isExpertMode = letterPressed != 'spc';
...
*/
         ...
         // WHEN SKETCH OR SKETCHPAGE IS LOCKED, SHOW A SMALL COLORED "DRAWING" DISK AT CURSOR.
                   ...
                   (! sketchPage.isGlyphable || isk() && isHover() && isFreehandSketch(sk()) && ! sk().isGlyphable()) ) {
            annotateStart();
