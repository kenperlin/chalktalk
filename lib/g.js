...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   function saveSketchState(s, i, sketches) {
      ...
      if (s instanceof GeometrySketch) {
         ...
         addSketchState(sketch, sketch, i, sketches);
         finishDrawingUnfinishedSketch(sketch);
         return;
      }

      addSketchState(s.clone(), s, i, sketches);

      for (var prop in s) {
         if (!(s[prop] instanceof Object)) {
            sketches[i][prop] = s[prop];
         }
         else if (s[prop] instanceof Array) {
            sketches[i][prop] = cloneArray(s[prop]);
         }
      }

      sketches[i].id = s.id;

      sketches[i]._curves = cloneArray(s._curves);

      sketches[i].portLocation = cloneArray(s.portLocation);

      if (s.code != null) {
         sketches[i].code = cloneArray(s.code);
      }

      if (sketches[i].initCopy !== undefined) {
         sketches[i].initCopy();
      }

      if (sketches[i].createMesh) {
         sketches[i].mesh = undefined;
      }
      sketches[i].sketchProgress = 1;
      sketches[i].sketchState = 'finished';

      sketches[i].copyStretchValues(s);

      sketches[i]._faders = (new Faders()).copy(s._faders);

      sketches[i].tX = s.tx();
      sketches[i].tY = s.ty();
      sketches[i].textX = s.textX;
      sketches[i].textY = s.textY;
   }

   function addSketchState(sketch, sketchNonClone, i, sketches) {
      ...
      sketches[i].arrows = [];
      sketches[i].children = [];
      if (sketches[i].colorId === undefined)
         sketches[i].setColorId(sketchPage.colorId);
      sketches[i].defaultValue = [];
      sketches[i]._faders.clear();
      sketches[i].id = sketch.id;
      sketches[i].in = [];

      /*
      for (var i = 0; i < sketchNonClone.in.length; i++) {
         sketches[i].in.push(sketchNonClone.in[i]);
      }
      */
      sketches[i].inValue = [];
      sketches[i].isDrawingEnabled = false;
      sketches[i].out = [];
      sketches[i].outValue = [];
      sketches[i].portBounds = [];
      sketches[i].portLocation = [];
      sketches[i].sketchState = 'finished';
      sketches[i].viewForwardMat.copy(viewForwardMat);
      sketches[i].viewInverseMat.copy(viewInverseMat);
      sketches[i].zoom = sketchPage.zoom;
      /*
      sketches[i].inValue = cloneArray(sketch.inValue);
      sketches[i].inValues = cloneArray(sketch.inValues);
      sketches[i].xyz = cloneArray(sketch.xyz);
      sketches[i].isDrawingEnabled = false;
      sketches[i].out = cloneArray(sketch.out);
      sketches[i].outValue = cloneArray(sketch.outValue);
      sketches[i].portBounds = cloneArray(sketch.portBounds);
      sketches[i].portLocation = [];
      sketches[i].sketchState = "finished";
      sketches[i].arrows = cloneArray(sketch.arrows);
      */
      sketches[i].viewForwardMat.copy(viewForwardMat);
      sketches[i].viewInverseMat.copy(viewInverseMat);
      sketches[i].zoom = sketchPage.zoom;
      if (sketches[i] instanceof Sketch2D) {
         sketches[i].x2D = This().mouseX;
         sketches[i].y2D = This().mouseY;
      }
   }
