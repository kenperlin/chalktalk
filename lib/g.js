...
/////////////////////////////////////////////////////////////////////
   ...
   function computeLinkCurvature(link, C) {
      link.s = computeCurvature(link.a.portXY_DEPRECATED_PORT_SYSTEM(0), C, link.b.portXY_DEPRECATED_PORT_SYSTEM(link.j));
...
var sketchDragY = 0;
   ...
   function endSketchDragAction(x, y) {
      ...
      case 4:
         ...
         let newLink = sketchPage.createLink(inSketch, inPort, outSketch);
         if (isDef(newLink)) {
            newLink.s = computeCurvature(outSketch.linkCurve);
         }
...
*/
      ...
      if (isDef(window[g.name].animate)) {
        ...
        // beyond reasonable doubt not to be buggy
         ...
         if (isAudiencePopup() || !(isShowingGlyphs && isDef(This().overlay))) {
            ...
            for (var I = 0 ; I < nsk() ; I++) {
               if (sk(I).parent == null) {
                  var a = sk(I);
                  if (isDef(a.output)) {
                     for (var k = 0 ; k < a.output.links.length ; k++) {
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            if (sk(I).output instanceof OutputPort && sk(I).output.links.length > 0) {
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            ...
            if (S.output instanceof OutputPort
               && (S.output.links.length > 0 || S.containsTextAssignment()))
            {
               ...
               for (let k = 0 ; k < S.output.links.length ; k++) {
                  let link = S.output.links[k];
                  let receiver = link.b;
