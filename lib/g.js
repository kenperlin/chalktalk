...
//////////////////////////////////////////////////////////////////////////////////
   ...
   function isWebgl() { return window.strokeShader !== undefined; }
...
//    + " <input id='soundfileinput' type='file' style='visibility:hidden' /> </input>"
      ...
      webgl_context = webgl_canvas.getContext('experimental-webgl');
      //fillShader   = new FillShader  (webgl_context);
      //strokeShader = new StrokeShader(webgl_context);
   ...
   function startCanvas(name) {
      ...
      else if (name == 'video_canvas' || name == 'slide' || name == 'webgl_canvas') {
         return;
      }
...
*/
      ...
      if (isDef(window[g.name].animate)) {
            ...
            if (! isVerticalPan) {
               ...
               if (isBottomHover) {
                  ...
                  for (var pn = 0; pn < sketchPages.length; pn++) {
                     ...
                     _g_ext(pn, [(pn - centerRatio) * numberSpacing, h - _g.panY - margin * 0.5]);
                  }
         ...
         if (speakerNotes) {
            annotateStart();
            _g.fillStyle = _g.strokeStyle = scrimColor(0.5);
	    _g.font = '10pix Arial';
	    _g.fillText(speakerNotes, 5 - _g.panX, h - 5 - _g.panY);
            annotateEnd();
         }
	 ...
	 if (isShowingTime) {
	    ...
	    _g.font = '12pt Arial';
	    _g.fillText(timeString, w - textWidth(timeString) - 6 - _g.panX, h - 6 - _g.panY);
            annotateEnd();
	 }
      ...
      displayStrokes.endFrame();

      if (displayStrokes.isStereo) {
         function stereoMessage(message, y) {
            _g.fillText(message, w*.25 - textWidth(message)/2, y);
            _g.fillText(message, w*.75 - textWidth(message)/2, y);
	 }
         _g.fillStyle = scrimColor(.5);
         _g.font = '12px Arial';
	 stereoMessage('[ ] keys adjust interocular = '  + roundedString(displayStrokes.interocular), h / 4);
	 stereoMessage('< > keys adjust focal length = ' + roundedString(displayStrokes.focalLength), h / 4 + 24);
      }

      // When piano plays middle C, create a walking bird.

      if (! window.isMidiBird)
      ...
      // When piano plays high C, create a flapping bird.

      if (! window.isMidiRocket)
         for (key in midi.downKeys)
            if (key == 60 + 2 * 12) {
