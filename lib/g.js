...
////////////////////////////////////////////////////////////////
   ...
   function initEventHandlers(canvas) {
	...
	// support resolution request from other clients
      canvas.onRequestForResolution = function(event) {
      ...
      canvas.onRequestForSketchPageIndex = function(event) {
         console.log("getting current sketch page index");

         var data = new Uint16Array(4 + 2), header = 'CTpIdx01', i;
         for (i = 0; i < 4; i += 1) {
            data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
         }
	  ...
	  // support create sketchPage
	   canvas.createSketchPage = function(event) {
		   console.log("create a new sketchPage");
		   ...
		   var data = new Uint16Array(4 + 2), header = 'CTPage01', i;
         for (i = 0; i < 4; i += 1) {
            data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
         }
	   ...
	   canvas.setSketchPage = function(event) {
         console.log("in Chalktalk, setting the sketch page: " + event.index);
         if (event.index > sketchPages.length - 1) {
            canvas.createSketchPage(null);
         }

         sketchBook.setPage(event.index);
    		setPage(event.index);

         canvas.onRequestForSketchPageIndex(null);
	   };

      canvas.toggleSketchCopyMode = function(event) {
         console.log("in Chalktalk, toggling sketch copy mode: " + event.toggleState);
         // only toggle if the state has actually changed
         if (event.toggleState != MR_sketchCopyModeData.toggleState) {
            MR_sketchCopyModeData.toggleState = !MR_sketchCopyModeData.toggleState;
         }
      };
      ...
      // (Karl) TODO: PROBABLY WRAP IN A renderPages() FUNCTION, BUT NOT UNTIL FINALIZED
      {
         ...
         // RENDER INACTIVE PAGES
         {
            ...
            for (; sketchPageIdx < curPageIdx; sketchPageIdx++) {
               renderInactivePage(g, sketchPageIdx);
            }
...
*/
   ...
   function restOfTickForInactivePage(g){
...
////////////////////////////////////////////////////////////
   ...
   function renderInactivePage(g, pageIdx) {
      setPageRenderOnly(pageIdx);
      restOfTickForInactivePage(g);
   }
