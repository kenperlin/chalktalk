...
///////////////// SUPPORT FOR MULTIPLE WEB CLIENTS ///////////////////
   ...
   function broadcastEvent(type, x, y) {
      ...
      var message = server.myClientId + ',' + type;
   ...
   function parseBroadcastEvent(dataString) {
   ...
   var clientSketchIndex = [];

   function useEventSketchIndex() {
      clientSketchIndex[server.myClientId] = sketchPage.index;
      sketchPage.index = clientSketchIndex[eventClientId];
   }

   function restoreLocalSketchIndex() {
      clientSketchIndex[eventClientId] = sketchPage.index;
      sketchPage.index = clientSketchIndex[server.myClientId];
   }
   ...
   function initEventHandlers(canvas) {
      ...
      canvas.onmousedown = function(event, x, y) {
         if (x === undefined) {
            ...
            broadcastEvent('mousedown', event.clientX - clientRect.left,
                                        event.clientY - clientRect.top);
            isClientMouseDown[server.myClientId] = true;
            return;
         }
      ...
      canvas.onmouseup = function(event, x, y) {
         if (x === undefined) {
            var clientRect = event.target.getBoundingClientRect();
            broadcastEvent('mouseup', event.clientX - clientRect.left,
                                      event.clientY - clientRect.top);
            isClientMouseDown[server.myClientId] = false;
            return;
         }
      ...
      canvas.onmousemove = function(event, x, y) {
         if (x === undefined) {
            enableTouchEvent(event);
            var clientRect = event.target.getBoundingClientRect();
            broadcastEvent('mousemove', event.clientX - clientRect.left,
                                        event.clientY - clientRect.top);
            return;
         }
         ...
         if (isPanning) {
            if (! isVerticalPan) _g.panX = min(0, _g.panX + clientX - _g.lastX);
            if (  isVerticalPan) _g.panY = min(0, _g.panY + clientY - _g.lastY);
         }
         ...
         if (isFinishedDrawing() /*&& letterPressed == '\0'*/
                                 && (! sketchPage.isPressed || palette.dragXY != null)
                                 && sketchAction == null)
   ...
   function _g_sketchTo(x, y, isLine) {
      ...
      if (sketch.sketchTrace != null) {
         if (sketch.sketchState != 'finished' && sketch.glyphTransition < 0.5)
            buildTrace(sketch.trace, x, y, isLine);
         else {
      ...
      var cx = x;
      var cy = y;
      var isPreview = sketch.sketchState == 'in progress';

      if (_g.inSketch && _g.suppressSketching <= 0)
         sketch.sp.push([x, y, isLine]);
      ...
      if (_g.inSketch && sketch.sketchState == 'in progress'
                      && _g.suppressSketching <= 0) {
         sketch.dSum += d;
         var t = sketch.sketchProgress;
         if (t < sketch.dSum / sketch.sketchLength) {
            var dSave = d;
            d = t * sketch.sketchLength - (sketch.dSum - dSave);
            cx = xPrev + dx * d / dSave;
            cy = yPrev + dy * d / dSave;
            if (d > 0) {
               cursorX = cx;
      ...
      if (! isLine) {
         var sx = snx(sketch, cx, cy);
         var sy = sny(sketch, cx, cy);
         _g.moveTo(sx, sy);
      }
         ...
         for (var i = 20 ; i < d ; i += 20) {
            ...
            var sx = snx(sketch, xx, yy);
            var sy = sny(sketch, xx, yy);
            _g.lineTo(sx, sy);
         }
         var sx = snx(sketch, cx, cy);
         var sy = sny(sketch, cx, cy);
...
////////// SUPPORT CODE
   ...
   var bgDragGestures = [];
   var bgUpGestures = [];
   (function() { var i; for (i = -1 ; i < 8 ; i++) bgUpGestures[i] = []; })();
   ...
   function bgUpGesture() {
      if (bgUpGestures[bgAction_dir1][bgAction_dir2] !== undefined)
         (bgUpGestures[bgAction_dir1][bgAction_dir2][1])();
   }

////////// CONTENT

   bgDragGestures[4] = ['change\nfont scale', function(x, y) {
      _font_scale_factor *= 1 + 0.01 * (y - bgAction_yDown);
      _font_scale_factor = min(900, _font_scale_factor);
      bgAction_yDown = y;
   }];

   bgDragGestures[-1] = ['pan', function(x, y) {
      _g.panX += bgAction_dx;
      _g.panY += bgAction_dy;
      group.translate(-bgAction_dx, -bgAction_dy);
      sketchPage.panHistoryAdd();
   }];

   bgDragGestures[6] = ['pan\nhistory', function(x, y) {
      sketchPage.panHistoryScrub(bgAction_dx / 20);
   }];

   bgUpGestures[0][-1] = ['prev\nimage'  , function() { sketchPage.previousImage(); }];
   bgUpGestures[0][ 2] = ['toggle'       , function() { imageLibrary_isShowingImage = ! imageLibrary_isShowingImage; }];
   bgUpGestures[1][-1] = ['create\ngroup', function() { group.create(); }];
   bgUpGestures[2][-1] = ['select\ncolor', function() { palette.isPieMenu = true; }];
   bgUpGestures[3][-1] = ['show\nglyphs' , function() { isShowingGlyphs = true; }];
   bgUpGestures[4][-1] = ['next\nimage'  , function() { sketchPage.nextImage(); }];
   bgUpGestures[5][-1] = ['draw\ntext'   , function() { toggleTextMode(); }];
   bgUpGestures[6][-1] = ['zoomed\nview' , function() { isOverviewMode = true; }];
   bgUpGestures[7][-1] = ['images'       , function() { imageLibrary_isShowingLibrary = true; }];
...
/////////////////////////////////////////////////////////////////////////////
   ...
   function defineServerOnBroadcast() {
      server.onBroadcast = function(event) {
         eventQueue.unshift(event.data);
      }
   }

   function runEvent(dataString) {
      var data = parseBroadcastEvent(dataString),
          sender = data.sender,
          type = data.type,
          isAnyMouseDown, i;

      eventClientId = parseInt(data.sender);
      ...
      case 'nClients':
         server.nClients = data.x;
         server.myClientId = data.y;
         break;
      ...
      case 'mousedown':
         isClientMouseDown[eventClientId] = true;
         break;

      case 'mouseup':
         isClientMouseDown[eventClientId] = false;
         break;

      case 'requestFocus':
         isAnyMouseDown = false;
         ...
         if (isAnyMouseDown)
            console.log(eventClientId + ' is not ok to request focus');
         else
            console.log(eventClientId + ' is ok to request focus');
         break;
      }

      if (type.indexOf('mouse') == 0)
         events_canvas['on' + type](new MouseEvent(type), parseInt(data.x), parseInt(data.y));
      else if (type.indexOf('key') == 0)
         events_canvas['on' + type](new KeyboardEvent(type), parseInt(data.x));

      eventClientId = server.myClientId;
   }
...
*/
      ...
      if (isDef(window[g.name].animate)) {
	    ...
	    :                                            'crosshair'
         ...
         if (! imageLibrary_isShowingLibrary && ! isShowingGlyphs)
         if (! isVerticalPan) {
            if (isRightHover) {
               annotateStart();
               _g.save();
               ...
               for (var pn = 0; pn < nPages ; pn++) {
                  ...
                  var numberY = - _g.panY + pn * numberSpacing;
	 ...
	 // WHEN ZOOMED OUT, DRAW OVERVIEW OF ENTIRE WORLD.
         ...
         (function() {
	    var a, b;
            overviewAlpha = isOverviewMode ? min(1, overviewAlpha + This().elapsed * 2)
                                           : max(0, overviewAlpha - This().elapsed);
            if (isShowingOverview || overviewAlpha > 0) {
	       if (isOverviewMode) {
	          a = overviewAlpha;
		  b = 1;
	       }
	       else {
		  a = overviewAlpha < .5 ? 0 : 2 * overviewAlpha - 1;
	          b = overviewAlpha < .5 ? 2 * overviewAlpha : 1;
	       }
	       var alpha = sCurve(a);
	       var blend = sCurve(b);
               function xInOverview(x) {
	          return mix(x, (x - w/2) / overviewScale + w/2 - _g.panX, alpha);
	       }
               function yInOverview(y) {
	          return mix(y, (y - h/2) / overviewScale + h/2 - _g.panY, alpha);
	       }
	       ...
	       color(scrimColor(.25 * blend));
               lineWidth(5 / mix(1, overviewScale, alpha));
	       var xmin = width () * (1 - overviewScale) / 2;
	       var xmax = width () * (1 + overviewScale) / 2;
	       var ymin = height() * (1 - overviewScale) / 2;
	       var ymax = height() * (1 + overviewScale) / 2;
	 ...
	 // NOT IN OVERVIEW MODE.

         (function() {

	    // SHOW EDGES OF THE WORLD.

	    if (overviewAlpha < 1 && sketchPage.travel > clickSize()) {
	       var w = width(), h = height();
	       var xmin = w * (1 - overviewScale) / 2;
	       var xmax = w * (1 + overviewScale) / 2;
	       var ymin = h * (1 - overviewScale) / 2;
	       var ymax = h * (1 + overviewScale) / 2;
	       color(scrimColor(.25 * (1 - overviewAlpha)));
	       ...
	       // WHILE PANNING, SHOW GRID.

	       if (sketchPage.isPressed && bgClickCount == 1 && bgAction_dir1 == -1) {
	          lineWidth(5);
	          for (var x = xmin + w ; x < xmax ; x += w)
		     line(x, ymin, x, ymax);
	          for (var y = ymin + h ; y < ymax ; y += h)
