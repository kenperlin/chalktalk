...
//window.displayListData = data;
   ...
   function canCreateLinkToBackgroundWithType(type) {
      ...
      // Since we'd be linking to a FreehandSketch, check compatibility with the types it
      // supports.
      for (let i = 0; i < FreehandSketch.inputTypes.length; i++) {
         if (AT.canConvert(type, FreehandSketch.inputTypes[i])) {
            return true;
         }
      }
      return false;
