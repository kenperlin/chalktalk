...
//    + " <input id='soundfileinput' type='file' style='visibility:hidden' /> </input>"
      ...
      // zhenyi
      window.cubeTest = new CT.Cube();
      window.cubeTest.translate(0,0,-10).scale(0.2);

      window.cubeHelper = new CT.Cube();

      ctScene.add(window.cubeHelper);
      ctScene.add(window.cubeTest);

      window.matrixHelper = new CT.Sphere();
      //socket = io('http://192.168.1.38:8080/client'); // NYU
      socket = io('https://holojam.herokuapp.com/client')
      ...
      socket.on('update', function (data) {
         ...
         for (var i = 0 ; i < live_objects.length ; i++) {
            ...
            if (obj.label == 'VR1') {
               ...
               //console.log("before:" + ctScene._viewMatrix);
               //ctScene.setViewMatrix(matrixHelper._matrix);
               //console.log("matrix to be assigned:" + CT.matrixInverse(matrixHelper._matrix));
               
               //console.log("after:" + ctScene._viewMatrix);
            }
         if (obj.label == 'VR2') {
               window.cubeHelper.setPQ([obj.x,obj.y,obj.z,obj.qx,obj.qy,obj.qz,obj.qw]);
               window.cubeHelper.scale(0.2);
               //window.ctObject.add(cubeHelper);
               //console.log("VR2:" + window.cubeHelper._matrix);
               //ctScene.setViewMatrix(matrixHelper._matrix);
               //console.log(ctScene._viewMatrix);
            }
         }
      });
   }
