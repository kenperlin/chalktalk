...
///////////////// SUPPORT FOR MULTIPLE WEB CLIENTS ///////////////////
   ...
   function broadcastEvent(type, x, y) {
      server.broadcastEvent(webClientName + ',' + type + ',' + x + ',' + y);
   }

   function parseBroadcastEvent(event) {
      var data = event.data.split(',');
      touchWebClient(data[0]);
   ...
   function WebClient() {
      var index = -1, panX, panY;
   }

   function touchWebClient(name) {
      if (webClients[name] === undefined) {
         webClients[name] = new WebClient();
      }
   }

   var webClients = {};

   var webClientName = createRandomName();

   touchWebClient(webClientName);

   var localWebClient = webClients[webClientName];

   var currentWebClient = localWebClient;

   function saveLocalWebClientIndex() {
      localWebClient.index = sketchPage.index;
      sketchPage.index = currentWebClient.index;
   }

   function restoreLocalWebClientIndex() {
      currentWebClient.index = sketchPage.index;
      sketchPage.index = localWebClient.index;
   }
...
////////////////////////////////////////////////////////////////
   ...
   function initEventHandlers(canvas) {
      ...
      server.onBroadcast = function(event) {
         ...
         currentWebClient = webClients[data.client];

         if (type.indexOf('mouse') == 0)
            canvas['on' + type](new MouseEvent(type), parseInt(data.x),
                                                      parseInt(data.y));
         else if (type.indexOf('key') == 0)
            canvas['on' + type](new KeyboardEvent(type), parseInt(data.x));
