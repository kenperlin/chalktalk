...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   function saveSketchState(s) {
      console.log("saving individual sketch state");
      ...
      if (s instanceof GeometrySketch) {
         ...
         sketch.sp0 = cloneArray(sketch.sp0);
         sketch.sp = cloneArray(sketch.sp);
         ...
         sketch.rX = s.rX;
         sketch.rY = s.rY;
         sketch.shaderCount = s.shaderCount;
         sketch.sketchProgress = s.sketchProgress;
         sketch.sketchState = s.sketchState;
         sketch.sx = s.sx;
         sketch.sy = s.sy;
         sketch.update = s.update;
         addSketchState(sketch);
         finishDrawingUnfinishedSketch(sketch);
         return;
      }

      addSketchState(s.clone());

      for (var prop in s)
         if (! (s[prop] instanceof Object))
            cache[cache.length - 1][prop] = s[prop];
         else if (s[prop] instanceof Array)
            cache[cache.length - 1][prop] = cloneArray(s[prop]);

      cache[cache.length - 1].id = s.id;

      cache[cache.length - 1]._curves = cloneArray(s._curves);

      cache[cache.length - 1].portLocation = cloneArray(s.portLocation);

      if (s.code != null)
         cache[cache.length - 1].code = cloneArray(s.code);

      if (cache[cache.length - 1].initCopy !== undefined)
         cache[cache.length - 1].initCopy();

      if (cache[cache.length - 1].createMesh)
         cache[cache.length - 1].mesh = undefined;
      cache[cache.length - 1].sketchProgress = 1;
      cache[cache.length - 1].sketchState = 'finished';

      cache[cache.length - 1].copyStretchValues(s);

      cache[cache.length - 1]._faders = (new Faders()).copy(s._faders);

      cache[cache.length - 1].tX = s.tx();
      cache[cache.length - 1].tY = s.ty();
      cache[cache.length - 1].textX = s.textX;
      cache[cache.length - 1].textY = s.textY;
   }

   function addSketchState(sketch) {
      ...
      cache.push(sketch); ////////////////////////////////////////
      cache[cache.length - 1].arrows = [];
      cache[cache.length - 1].children = [];
      if (cache[cache.length - 1].colorId === undefined)
         cache[cache.length - 1].setColorId(sketchPage.colorId);
      cache[cache.length - 1].defaultValue = [];
      cache[cache.length - 1]._faders.clear();
      cache[cache.length - 1].id = sketch.id;
      cache[cache.length - 1].in = [];
      for (var i = 0; i < cache[cache.length - 1].in.length; i++) {
         cache[cache.length - 1].in.push(new SketchLink(sketch.in.a, sketch.in.i, sketch.in.b, sketch.in.j, sketch.in.s));
         cache[cache.length - 1].in[i].s = sketch.in.s;  
      }

      cache[cache.length - 1].inValue = cloneArray(sketch.inValue);
      cache[cache.length - 1].inValues = cloneArray(sketch.inValues);
      cache[cache.length - 1].xyz = cloneArray(sketch.xyz);
      cache[cache.length - 1].isDrawingEnabled = false;
      cache[cache.length - 1].out = cloneArray(sketch.out);
      cache[cache.length - 1].outValue = cloneArray(sketch.outValue);
      cache[cache.length - 1].portBounds = cloneArray(sketch.portBounds);
      cache[cache.length - 1].portLocation = [];
      cache[cache.length - 1].sketchState = "finished";
      cache[cache.length - 1].arrows = cloneArray(sketch.arrows);
      cache[cache.length - 1].viewForwardMat.copy(viewForwardMat);
      cache[cache.length - 1].viewInverseMat.copy(viewInverseMat);
      cache[cache.length - 1].zoom = sketchPage.zoom;
      if (cache[cache.length - 1] instanceof Sketch2D) {
         cache[cache.length - 1].x2D = This().mouseX;
         cache[cache.length - 1].y2D = This().mouseY;
      }
   }
