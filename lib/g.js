...
////////// CONTENT
   ...
   bgClickGestures[0] = ['prev image' , function() { sketchPage.previousImage(); }];
   bgClickGestures[1] = ['lock'     , function() { sketchPage.isGlyphable = ! sketchPage.isGlyphable; }];
   bgClickGestures[2] = ['color'    , function() { palette.isPieMenu = true; }];
   bgClickGestures[3] = ['glyphs'   , function() { isShowingGlyphs = true; }];
   bgClickGestures[4] = ['next image' , function() { sketchPage.nextImage(); }];
   bgClickGestures[5] = ['draw text', function() { toggleTextMode(); }];
...
/////////////////////////////////////////////////////////////////////////////
   ...
   function endSketchDragAction(x, y) {
      switch (sketchDragMode) {
      case 0:
         var action = pieMenuIndex(x - this._sketchDragX, y - this._sketchDragY);
	 switch (action) {
	 case 0: console.log('load collection from sketch'); break;
	 case 1: console.log('assign collection to sketch'); break;
	 case 2: console.log('show all sketches/collections'); break;
	 case 3: console.log('unassign sketch'); break;
	 }
         break;
      case 3:
   ...
   function sketchClickActionName(dir, sketch) {
      ...
      if (sketch !== undefined)
         if (isSimpleSketch(sketch))
            switch (dir) {
	    case 1: if (! sketch.isGlyphable()) name = 'unlock'; break;
            }
         else
            switch (dir) {
      ...
      if (isDef(window[g.name].animate)) {
	    ...
	    :                                            backgroundColor == 'white' ? 'cell' : 'crosshair'
         ...
         // WHEN SKETCH OR SKETCHPAGE IS LOCKED, SHOW A SMALL COLORED "DRAWING" DISK AT CURSOR.
	           ...
	           (! sketchPage.isGlyphable || isk() && isHover() && isSimpleSketch(sk()) && ! sk().isGlyphable()) ) {
	    annotateStart();
