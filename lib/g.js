...
/////////////////////////////////////////////////////////////////////
   ...
   function findNearestInPort(sketch) {
      ...
      if (sketch != null) {
         ...
         if (inPortCount > 0)
            inPort = findNearestPortAtCursor(sketch, sketch.inLinks, true);
         else
            inPort = firstUndefinedArrayIndex(sketch.inLinks);
      }
   ...
   function endSketchDragAction(x, y) {
      ...
      case 4:
         ...
         if (sketchPage.createLink(inSketch, inPort, outSketch, outPort)) {
            let i = outSketch.outLinks[outPort].length - 1;
            outSketch.outLinks[outPort][i].s = computeCurvature(
               outSketch.linkCurve);
         }
      ...
      if (isDef(window[g.name].animate)) {
        ...
        // beyond reasonable doubt not to be buggy
            ...
            for (var I = 0 ; I < nsk() ; I++) {
               if (sk(I).parent == null) {
                  ...
                  for (var i = 0 ; i < a.outLinks.length ; i++) {
                     if (isDef(a.outLinks[i])) {
                        for (var k = 0 ; k < a.outLinks[i].length ; k++) {
                           var link = a.outLinks[i][k];
                           link.draw(true);
               ...
               if (S.output instanceof OutputPort) {
                  ...
                  for (let i = 0 ; i < S.outLinks.length ; i++) {
                     if (isDef(S.outLinks[i])) {
                        ...
                        for (let k = 0 ; k < S.outLinks[i].length ; k++) {
                           let link = S.outLinks[i][k];
                           let receiver = link.b;
                           ...
                           if (outValue.canConvert(AT.Unknown)) {
                              let unknown = outValue.convert(AT.Unknown);
                              receiver.inValue_DEPRECATED_PORT_SYSTEM[link.j] = unknown.value;
                           }
                        }
                     }
                  }
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            ...
            for (var i = -1 ; i < S.inLinks.length ; i++) {
...
//window.displayListData = data;
   ...
   function canCreateLinkToBackgroundWithType(type) {
      // Since we'd be linking to a FreehandSketch, check compatibility with the types it supports.
      return AT.typeIsDefined(type) && (
               AT.canConvert(type, AT.String) // String and Unknown are the only two 
            || AT.canConvert(type, AT.Unknown)); // input types FreehandSketch supports.
   }
