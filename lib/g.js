...
*/
      ...
      if (window.checkForAbsorb) {
         ...
         // TODO:
         // function findCandidates(glyph, glyphs) {
         //    for (const other of)
         // }

         function findCandidateSketches(glyphs, maxCandidates = 3) {

            const strokes = strokesGlyph.data;
            if (strokes.length == 0 || strokes[0].length < 2)
               return null;
            ...
            var bestScore = strokesGlyph.WORST_SCORE;
            // POSSIBLY SHOW ONLY TOP N SCORERS? (3?)
            var worstScore = bestScore;
            var candidates = [];
            ...
            for (var i = 0 ; i < glyphs.length ; i++) {
               // EXCLUDE GLYPHS WITH NUMBER OF STROKES 
               // EQUAL TO CURRENT FREE-HAND SKETCH, TOO LATE TO CHANGE
               if (strokesGlyph.data.length ===  glyphs[i].data.length) {
                  continue;
               }
               var score = strokesGlyph.compareExistingStrokes(glyphs[i]);
               if (score == strokesGlyph.WORST_SCORE) {
                  continue;
               }

               // if (score >= bestScore) {
               //    bestScore = score;
               //    candidates.push(glyphs[i]);
               // }
               // else if (candidates.length > 0 && score >= worstScore) {
               //    candidates.push
               // }
               candidates.push({glyph : glyphs[i], score : score});
         ...
         const candidates = findCandidateSketches(glyphs);
         if (candidates !== null) {
            console.log("TODO: GRAPHICS")
            const MAX_TO_DISPLAY = 3;
            for (let i = candidates.length - 1; i >= 0 && i >= candidates.length - MAX_TO_DISPLAY; i--) {
               console.log(candidates[i].glyph.indexName);
            }
            sk().candidates = candidates;
         }
