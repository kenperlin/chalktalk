...
// WHEN WE DON'T WANT TO RENDER A FRAME, WE CAN USE A FAKE CONTEXT.
   ...
   var whiteBackgroundRGB = [255,255,255];
   ...
   var isCasualStyle = false;
   var isCatchingRenderExceptions = true;
   var isDarkVideo = false;
   ...
   function sketchActionEnd() {
      ...
      // IF A SKETCH IS DROPPED INTO ANOTHER SKETCH:

      for (let i = 0 ; i < nsk() ; i++) {
         if (sk(i) != sk() && sk(i).isMouseOver) {
	    if (sk(i).catchSketch)
	       sk(i).catchSketch(sk());
	    if (sk().throwSketch)
	       sk().throwSketch(sk(i));
      ...
      // IF A FREEHAND SKETCH IS DROPPED INTO THE CURRENT GROUP:

      if ( sketchAction == 'translating' &&
           sk() instanceof FreehandSketch &&
           group.contains(mouseMoveClientX, mouseMoveClientY)) {
/*
         TO DO: IMPLEMENT A FEATURE WHEREBY EVERYTHING IN THE GROUP
	 TURNS INTO A "META-SKETCH", WHICH IS SAVED TO THE SERVER.
	 
	 THIS META-SKETCH ACTS LIKE A SINGLE SKETCH, AND IS INVOKED
	 BY DRAWING THE FREEHAND SKETCH THAT WAS DROPPED ONTO THE GROUP.

	 INPUT LINKS INTO THE GROUP AND THE FIRST OUTPUT LINK (IF ANY)
	 FROM THE GROUP BECOME THE LINKS FROM/TO THIS META-SKETCH.

	 THIS INSTANCE OF THE FREEHAND SKETCH IS DELETED, AND
	 SO IS THE GROUP AND ALL OF THE SKETCHES IN IT.
*/
         group.fade();
         sk().fade();
      }
...
////////////////////////////////////////////////////////////////
   ...
   function resizeWindows() {
      ...
      slide_canvas.width = width();
      slide_canvas.height = height();
   ...
   function initEventHandlers(canvas) {
      ...
      function broadcastEvent(type, event) {
	    ...
	    event : type.indexOf('mouse') > 0
	            ? { button: event.button, clientX: event.clientX, clientY: event.clientY, uid : event.uid }
	            : { keyCode: event.keyCode, uid : event.uid }
         }));
      }
	 ...
	 return keyCode;
      ...
      canvas.onkeydown = function(event) {
         ...
         if (!CT_CONFIG.IGNORE_REMOTE_INPUT) {
      ...
      canvas.onmousedown = function(event, isRemote) {
         ...
         if (event.isMR) {
            if (CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID >= 0 &&
               event.uid != CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID) {
               return;
            }
         }
         else if (CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID >= 0 &&
            CT_CONFIG.INSTANCE_ID != CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID) {
            return;
         }
         else if (!isRemote) {
            event.uid = CT_CONFIG.INSTANCE_ID;
         }
         else {
            if (CT_CONFIG.IGNORE_REMOTE_INPUT) {
            ...
            console.log("remote event received from ID=[" + event.uid + "]");
         }

         setActiveUser(event.uid);
         const userRecord = getActiveUser().record;
         updateUserCursor(userRecord, event.clientX, event.clientY, def(event.clientZ));

         handleBroadcastEvent(event, "onmousedown");

         if (!COLOR_WAS_INIT) {
            COLOR_WAS_INIT = true;
            if (CT_CONFIG.INSTANCE_ID > 0) {
               sketchPage.colorId = palette.rgb.length - (CT_CONFIG.INSTANCE_ID % palette.rgb.length) - 1;
            }
         }
         ...
         if (event.button && event.button != 3 && event.button != 1)
            return;
      ...
      canvas.onmouseup = function(event, isRemote) {
         if (event.isMR) {
            if (CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID >= 0 &&
               event.uid != CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID) {
               return;
...
*/
      ...
      // MOUSE IS MOVED.
	var prevEventButton = -1;
   canvas.onmousemove = function(event, isRemote) {
      if (event.isMR) {
         if (CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID >= 0 &&
            event.uid != CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID) {
            return;
         }
         canvas.clientHandleSketchAction(event);
      }
      else if (CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID >= 0 &&
         CT_CONFIG.INSTANCE_ID != CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID) {
         return;
      ...
      handleBroadcastEvent(event, "onmousemove");
		 ...
		 // HEHE
		prevEventButton = grabControlFromUnity(event, prevEventButton);
   ...
   function _g_sketchTo(_p, isLine) {
      if (_p === undefined)
		...
		var x = _p[0], y = _p[1], z = def(_p[2], _g_z) + ((window.PUSHED_sketch_zOffset) ? (window.PUSHED_sketch_zOffset) : 0), xx, yy, p;
      if (! isDrawingSketch) {
         p = [ x, y, z ];
         if (! isScreenView)
...
// TODO: FIGURE OUT HOW TO LOAD THE NEWLY CREATED SKETCH TYPE INTO CURRENT GLYPHS.
   ...
   function loadSketches(onesketch) {
      try {
         var lsRequest = new XMLHttpRequest();
         ...
         lsRequest.onloadend = function () {
            if (lsRequest.responseText != "") {
               ...
               for (var n = 0; n < ls.length; n++) {
                  var filename = ls[n];
		  if (onesketch && filename != onesketch)
		     continue;
   ...
   function addTypeNameToSketchCode(text, typeName, defaultLabel) {
      let i = text.indexOf('function');
      if (i > 0)
      ...
      if (text.substring(0, i).trim() == 'function') {
         text = typeName + ' = function() {\n' +
                '\'use strict\';\n' +
		(defaultLabel ? 'this.label = \'' + defaultLabel + '\';\n' : '') +
   ...
   function importSketch(filename, remainingRequests) {
	 ...
	 // IF FILE STARTS WITH 'name = function...', USE name AS A FAULT LABEL.

         let defaultLabel = null;
	 j = text.indexOf('= function');
	 if (j > 0 && j < 20)
	    defaultLabel = text.substring(0, j).trim();

         text = addTypeNameToSketchCode(text, typeName, defaultLabel);
         ...
         // OTHERWISE LOAD THE NEW SKETCH TYPE.
            ...
            sketchFileData[typeName] = [ filename, responseText ];
	    if (window._load_sketch) {
               var i = responseText.indexOf('{');
               var j = responseText.lastIndexOf('}');
               try {
	          let sketch = window._load_sketch;
                  sketch._temporaryFunction = new Function(responseText.substring(i + 1, j));

		  let noReloadNames = sketch._noReload, noReloadItems = {};
		  if (noReloadNames)
		     for (let i = 0 ; i < noReloadNames.length ; i++) {
		        let name = noReloadNames[i];
		        noReloadItems[name] = sketch[name];
                     }
                  sketch._temporaryFunction();
		  if (noReloadNames)
		     for (let i = 0 ; i < noReloadNames.length ; i++) {
		        let name = noReloadNames[i];
		        sketch[name] = noReloadItems[name];
                     }

               } catch (e) { console.log(e); }
	       delete window._load_sketch;
            }
         }
   ...
   function gStart() {
      ...
      + " <canvas id='load_image_canvas' tabindex=1 width=2048 height=2048"
      + "    style='z-index:1;position:absolute;left:2048;top:-20;'>"
      + " </canvas>"

      + " <canvas id='video_analysis_canvas' tabindex=1 width=480 height=360"
      + "    style='z-index:1;position:absolute;left:-480;top:-20;'>"
      + " </canvas>"

      + " <canvas id='reflection_map_canvas' tabindex=1 width=256 height=256"
      + "    style='z-index:1;position:absolute;left:-256;top:-256;'>"
      + " </canvas>"

      + " <canvas id='background_canvas' color='" + backgroundColor + "'></canvas>"

      + " <canvas id='video_canvas' tabindex=1"
      + "    style='z-index:1;position:absolute;left:0;top:0;'>"
      + " </canvas>"

      + " <canvas id='slide_canvas' tabindex=1"
      + "    style='z-index:1;position:absolute;left:0;top:0;'>"
      + " </canvas>"
      +
      (isShowingRenderer
      ...
      + " <canvas id='test_canvas' tabindex=1"
      + "    style='z-index:1;position:absolute;left:0;top:0;'>"
      + " </canvas>"
      ...
      + " <div id=gifs_div "
      + "style='z-index:1;position:absolute;left:0;top:0;width:100%;vertical-align:middle'"
      + " ><!!></div> "
...
//    + " <input id='soundfileinput' type='file' style='visibility:hidden' /> </input>"
      ...
      background_canvas.style.backgroundColor = backgroundColor;

      test_canvas.width = width();
      test_canvas.height = height();
...
//    lightTracker = new LightTracker(video_analysis_canvas);
      lightTracker = new SpotTracker(video_analysis_canvas);
      ...
      events_canvas.browserSetID = function(_event) {
         const UID = _event.uid;

         console.log("setting instance ID to: " + UID);

         CT_CONFIG.INSTANCE_ID = _event.uid;

         CT_CONFIG.IS_HOST = _event.isHost;

         getOrCreateUserRecord(CT_CONFIG.INSTANCE_ID, true);

         // temp for demo only for now, change default color depending on ID
         // defaultPenColor = CT.colorsMap.get(
         //    CT.colorsArray[CT.colorsArray.length - (UID % CT.colorsArray.length) - 1]
         // );
      }
	  //
	   initUnityHandlers(events_canvas);
      initSketchCanvas();
   ...
   var COLOR_WAS_INIT = false;

   function setGif(name) {
      gifs_div.innerHTML = name == '' ? '<!!>' :
         '<img style="display:block;margin-left:auto;margin-right:auto;height:100%;" src=gifs/' + name + '.gif>';
   }
...
(function(c){var b,d,e,f,g,h=c.body,a=c.createElement("div");a.innerHTML='<span style="'+["position:absolute","width:auto","font-size:128px","left:-99999px"].join(" !important;")+'">'+Array(100).join("wi")+"</span>";a=a.firstChild;b=function(b){a.style.fontFamily=b;h.appendChild(a);g=a.clientWidth;h.removeChild(a);return g};d=b("monospace");e=b("serif");f=b("sans-serif");window.isFontAvailable=function(a){return d!==b(a+",monospace")||f!==b(a+",sans-serif")||e!==b(a+",serif")}})(document);
   ...
   function setBackgroundColor(color) {
      background_canvas.style.backgroundColor = color;
...
/////////////////////////////////////////////////////////////////////
   ...
   function drawPossibleLink(s, x, y, z = 0) {
      if (! s)
         return;
...
////////// CONTENT
   ...
   let bgDragLinkSketch = null;

   bgDragGestures[4] = ['link',
      function(x, y) {
         if (bgDragLinkSketch == null) {
	    let s = new NumericSketch();
	    addSketch(s);
	    s.initNumeric('0', bgAction_xDown, bgAction_yDown);
	    s.textCursor = s.text.length;
            s.linkCurve = [];
	    bgDragLinkSketch = s;
	 }
	 bgDragLinkSketch.linkCurve.push([x,y]);
      },
      function(x, y) {
         let s = bgDragLinkSketch;
         if (bgAction_sketch) {
	    new SketchLink(bgDragLinkSketch, 0, bgAction_sketch, bgAction_sketch.in.length);
            s.out[0][0].s = computeCurvature(s.linkCurve);
            delete s.linkCurve;
         }
         else
	    deleteSketch(s);
         bgDragLinkSketch = null;
      }
   ];
   ...
   bgDragGestures[7] = ['prev/next',
      ...
      function(x, y) {
         switch (bgAction_dir2) {
         case 0: imageLibrary_previousImage(); break;
         case 2: imageLibrary_toggleLevel(); break;
         case 4: imageLibrary_nextImage(); break;
         }
      }
   ];
...
var sketchDragY = 0;
   ...
   function doSketchDragAction(x, y) {
      ...
      case 1:
         break;
      case 6:
         console.log('2D rotate by ' + (x - sketchDragX));
	 sketchDragX = x;
         break;
      case 2:
         sk().arrowDrag(x, y);
   ...
   function endSketchDragAction(x, y) {
      ...
      case 2:
         ...
         break;
      case 3:
         let propsSketch = new props_sketch();
	 propsSketch.sketchTrace = [
	    [[x-3,y-.3],[x+3,y-.3],[x+3,y+.3]],
	    [[x+3,y+.3],[x-3,y+.3],[x-3,y-.3]],
	 ];
	 let b = sk();
	 addSketch(propsSketch);
	 new SketchLink(sk(), 0, b, 0, 0);
         break;
      case 4:
         tryToSelectSketchAtCursor();
   ...
   function skNext() { sketchPage.index = ! isk() ? 0 : (sketchPage.index + 1) % nsk(); }
   ...
   const defaultTimestamp = new Uint16Array(
      new Float32Array([-1]).buffer
   );

   const tick = function(g) {
      if (window.displayListener) {
         displayList = [0];
		   displayListMesh = [0, defaultTimestamp[0], defaultTimestamp[1]];
	   }
/*
...
*/
      ...
      if (isDef(window[g.name].animate)) {
         ...
         let bgContext = background_canvas.getContext('2d');
	 if (window.bgImage)
	    bgContext.drawImage(bgImage, 0, 0, width(), height());
	 else {
            background_canvas.color = backgroundColor;
         ...
         for (var I = 0 ; I < nsk() ; I++) {
            ...
            const skI = sk(I);
            skI.xlo = xlo - sketchPadding;
            skI.ylo = ylo - sketchPadding;
            skI.xhi = xhi + sketchPadding;
            skI.yhi = yhi + sketchPadding;

            skI.isMouseOver = skI.parent == null &&
                                skI.fadeAway == 0 &&
                                This().mouseX >= skI.xlo &&
                                This().mouseX <  skI.xhi &&
                                This().mouseY >= skI.ylo &&
            ...
            if (skI.isMouseOver) {
               ...
               Sketch.currentSketchMouseOver = skI;
            }
        ...
        // beyond reasonable doubt not to be buggy
         ...
         if (isAudiencePopup() || !(isShowingGlyphs && isDef(This().overlay))) {
            ...
            else if (bgDragLinkSketch)
               drawPossibleLink(bgDragLinkSketch, sketchPage.mx, sketchPage.my);
         ...
         if (! isShowingHelp) {
            if (isSketchInProgress()) {
               for (let uidIdx = 0; uidIdx < userIDs.length; uidIdx += 1) {
                  ...
                  const userRecord = idToUser(uID);
                  drawCrosshair(userRecord.cursor.x, userRecord.cursor.y);
               }
            }
         }
         ...
         if (isShowingGlyphs && ! isShowingHelp && glyphChart.shouldDraw)
            glyphChart.draw();
         ...
         if (isFog) {
            let w = width() + 1000;
            for (let i = 0 ; i < 10 ; i++) {
               let x = 500 * (noise(  .5, .1 * time, 10 * i + .5) - 1);
               let y = 500 * (noise(10.5, .1 * time, 10 * i + .5) - 1);
               if (! window.fogImage) {
                  window.fogImage = new Image();
                  fogImage.src = "images/smoke_0.png";
               }
               _g.drawImage(fogImage, x, y, w, w);
            }
         }

	 if (window.isCapturingReflectionMap) {
	    videoLayer.captureReflectionMap();
	    //_g.drawImage(CT.reflectionMap, 0, 0);
         }
...
console.log(errorMessage);
      ...
      for (let i = 0; i < ctScene._objects.length ; i++)
         ctScene._objects[i].draw();
   }
...
////////////////////////////////////////////////////////////
   ...
   function setPage(index) {
      ...
      if (isAudiencePopup())
         audiencePopup.document.getElementById('slide_canvas').innerHTML =
   ...
   function insertTemplate(template, slide_canvas) {
      ...
      slide_canvas.innerHTML = parent.innerHTML;
   }

   function resizePadding() {
      var paddingHeight = (height() - slide_canvas.clientHeight) / 2;
      slide_canvas.getElementsByClassName('padding')[0].setAttribute('height', paddingHeight);
   }
   ...
   // GIVEN AN IMAGE SOURCE, LOAD THE DATA FOR THE IMAGE AS image.data

   function loadImageData(file) {
      let image = new Image();
      image.onload = () => {
         let context = load_image_canvas.getContext('2d');
	 context.drawImage(image, 0, 0);
         image.data = context.getImageData(0, 0, image.width, image.height).data;
	 //server.writeFile('ground_data.txt', JSON.stringify(image.data));
      }
      image.src = file;
      return image;
   }
