...
/////////////////////////////////////////////////////////////////////////////
   ...
   var tick = function(g) {
      ...
      if (window.didTest === undefined) {
/*
         server.set("wenboDataCompressed", wenboDataCompressed[0]);
         server.get("wenboDataCompressed", function(value) {
	    console.log("wenboDataCompressed value[1] = " + value[1][0]);
	 });
*/
/*
         server.getSensorData(function(data) {
	    data = uncompressData(data, 80 * 48);
	    console.log("data length = " + data.length);
	    for (var row = 0 ; row < 48 ; row++) {
	       str = '';
	       for (var col = 0 ; col < 80 ; col++)
	          str += String.fromCharCode(64 + min(25, data[80 * row + col]));
	       console.log(str);
            }
	 });
*/
...
*/
      ...
      if (isDef(window[g.name].animate)) {
         ...
         if (speakerNotes) {
            annotateStart();
            _g_fillStyle = _g.strokeStyle = scrimColor(0.5);
	    _g.font = '10pix Arial';
	 ...
	 if (isShowingTime) {
      ...
      // When piano plays high C, create and animate a rocket ship.
