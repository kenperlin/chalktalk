...
// WHEN WE DON'T WANT TO RENDER A FRAME, WE CAN USE A FAKE CONTEXT.
   ...
   let beginOrFinishMovingBackwardsOrForwards = {
      timestamp : 0,
      zOffsetPrev : 0,
      zOffset : 0
   };
...
////////////////////////////////////////////////////////////////
   ...
   function initEventHandlers(canvas) {
	...
	// support resolution request from other clients
     ...
     canvas.clientInitialize = function(event) {
         var data = new Uint16Array(4 + 8), header = 'CTInit01', i;
         for (i = 0; i < 4; i += 1) {
            data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
         }
      ...
      window.resetSelections = function() {
         ...
         selectedCTObject.obj = {zOffset : function() { console.log("WARNING: Should not be called!"); }};
         selectedCTObject.srcPageIdx = 0;

         beginOrFinishMovingBackwardsOrForwards.zOffsetPrev = 0;
         beginOrFinishMovingBackwardsOrForwards.zOffset = 0;
      ...
      canvas.clientBeginOrFinishMovingBackwardsOrForwards = function(event) {
         ...
         if (selectedCTObject.kind == 0 || deferred.functions.length < 1) {
            console.log("SHOULD NOT BE POSSIBLE TO DO THIS COMMAND IF NOTHING IS SELECTED");
            canvas.sendTimeAndNum(header, 0); 
            return;
         }

         if (event.timestamp <= beginOrFinishMovingBackwardsOrForwards.timestamp) {
            console.log("old command received");
            canvas.sendTimeAndNum(header, 0);
            return;   
         }
         beginOrFinishMovingBackwardsOrForwards.timestamp = event.timestamp;

         // console.log("ct clientBeginOrFinishMovingBackwardsOrForwards()");

         switch (event.option) {
         case 0: // end
            console.log("ending");
            deferred.functions.pop();
            break;
         case 1: // begin towards you
            console.log("towards");

            if (deferred.functions.length > 1) {
               deferred.functions.pop();
            }
            ...
            deferred.functions.push(
               (function(){
                  let t = time;
                  const selected = selectedCTObject.obj;
                  ...
                  return function() {
                     ...
                     beginOrFinishMovingBackwardsOrForwards.zOffset -= dt;
                  };
               })()
            );
      ...
      canvas.sendTimeAndNum = function(hdr, num) {
         console.log("sending a number");
      ...
      const mrAllowSamePageMove = true;
      ...
      canvas.clientBeginMoveGroupOrSketchFromPage = function(event) {
         console.log("in clientBeginMoveGroupOrSketchFromPage()");
         const header = 'CTBrdon?';

         if (event.timestamp <= selectedCTObject.beginTimestamp) {
            console.log("old begin command received");
            canvas.sendTimeAndNum(header, 0); 
            return;
         }
         ...
         if (group.isAtCursor) {
            ...
            canvas.sendTimeAndNum(header, 2); 

            beginOrFinishMovingBackwardsOrForwards.zOffset = selectedCTObject.obj.zOffset();
               ...
               (function() {
                  return function() { 
                     ...
                     if (srcPageIdx != pageIndex) {
                        ...
                        window.PUSHED_sketch_zOffset = beginOrFinishMovingBackwardsOrForwards.zOffset;
         ...
         if (group.sketches.indexOf(sketch) != -1) {
            console.log("found sketch in group, selecting group");
         ...
         deferred.functions.push(
            (function() {
               return function() {
                  ...
                  // TODO still trying to decide whether I should allow placement of object on same board
                  if (srcPageIdx != pageIndex) {
                     const mx = This().mouseX;
                     const my = This().mouseY;
                     const cx = sketch.cx();
         ...
         canvas.sendTimeAndNum(header, 1);

         //moveSketchToPageAtIndex(sketch, (pageIndex + 1) % sketchPages.length);
      }

      canvas.clientEndMoveGroupOrSketchFromPage = function(event) {
         console.log("in clientEndMoveGroupOrSketchFromPage()");
         const header = 'CTBrdoff';

         if (event.timestamp <= selectedCTObject.beginTimestamp) {
            console.log("old end command received");
            return;
         }
         if (event.dstPageIdx == selectedCTObject.srcPageIdx) {
            if (mrAllowSamePageMove) {
               switch (selectedCTObject.kind) {
               case 1: // sketch    

                  selectedCTObject.obj._zOff = beginOrFinishMovingBackwardsOrForwards.zOffset;              
               ...
               case 2: // group                  
                  ...
                  const dz = beginOrFinishMovingBackwardsOrForwards.zOffset - selectedCTObject.obj._zOff;
                  selectedCTObject.obj._zOff = beginOrFinishMovingBackwardsOrForwards.zOffset;
               ...
               console.log("canceling move command");
               if (selectedCTObject.kind == 1 || selectedCTObject.kind == 2) {
                  deferred.functions.pop();
               }
               selectedCTObject.kind = 0;
            }
...
*/
      ...
      // (Karl) TODO: PROBABLY WRAP IN A renderPages() FUNCTION, BUT NOT UNTIL FINALIZED
      {
         // RENDER ACTIVE PAGE

         //sketchBook.setPage(pageIndex);
         ...
         //overviewShowGridWhilePanning = true;
         ...
         // RENDER INACTIVE PAGES
         {
            ...
            for (; sketchPageIdx < curPageIdx; sketchPageIdx++) {
               renderInactivePage(g, sketchPageIdx, curPageIdx);
            }
         ...
         for (let f = 0; f < deferred.functions.length; f += 1) {
            deferred.functions[f]();
         }
      }
     ...
     //    ctPath.flush();
      ...
      if (window.displayListener)
         (function() {
            ...
            {
               // mesh data info
               const data = new Uint16Array(4 + displayListMesh.length);
               const header = 'CTmesh01';
               let i = 0;
               for (i = 0 ; i < 4 ; i++)
                  data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
               for (i = 0 ; i < data.length ; i++)
                  data[i + 4] = displayListMesh[i];
...
let MUAHAHA = false;
   ...
   function consoleSetPage(idx) { 
      sketchPage = sketchBook.setPage(idx);  
      setPage(idx); 
   }
