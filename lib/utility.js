	    ...
	    if (event.data instanceof ArrayBuffer)
	       return;
            ...
            if (obj.eventType) {
               ...
               // mark the message as remote (true) since it came
               // from a different source
               (events_canvas[obj.eventType])(obj.event, /*isRemote = */true);
               return;
            }
      ...
      if (nDigits !== undefined && nd > 0) {
         ...
         if (nDigits > 0)
            while (str.length - i < nd + 1)
               str += "0";
         else if (str.length > 2 && str.substring(0, 2) == '0.')
...
// HANDLE PLAYING AN AUDIO SIGNAL:
   ...
   let audioNode = null, audioIndex = 0, audioStereo = 0;

   let setAudioStereo = s => audioStereo = s;

   let setAudioSignal = f => {
      if (audioNode == null) {
      ...
      if (audioNode != null) {
         audioNode.onaudioprocess = function(event) {
            ...
            var signal0 = output.getChannelData(0);
            var signal1 = output.getChannelData(1);
            ...
            if (f === undefined)
               ...
               for (var i = 0 ; i < output.length ; i++)
                  signal0[i] = f[audioIndex++ % f.length];
            else
               for (var i = 0 ; i < output.length ; i++)
                  signal0[i] = f(audioIndex++ / output.sampleRate);

            for (var i = 0 ; i < output.length ; i++)
	       signal1[i] = signal0[i];

	    let t = .5 + .5 * audioStereo;
            for (var i = 0 ; i < output.length ; i++) {
	       signal0[i] *= t;
	       signal1[i] *= 1 - t;
            }
...
// MATH CONSTANTS AND FUNCTIONS
   ...
   function add(a, b) { return a.length>2 ? [a[0]+b[0],a[1]+b[1],a[2]+b[2]] : [a[0]+b[0],a[1]+b[1]]; }
   ...
   function clamp(value,lo,hi) { return max(lo, min(hi, value)); }
   function copy(src) { let j = 0, dst = []; while (j < src.length) dst.push(src[j++]); return dst; }
   function copyFrom(dst, src, n, j, k) { for (let i = 0 ; i < n ; i++) dst[def(j) + i] = src[def(k) + i]; }
   function cos(t) { return Math.cos(t); }
   function cotan(t) { return Math.cotan(t); }
   function createBoxCorners(lo, hi) {
   ...
   function evalBezier(t, a, b, c, d) {
      if (Array.isArray(a)) { d=a[3],c=a[2],b=a[1],a=a[0]; }
      return mix(mix(mix(a, b, t), mix(b, c, t), t), mix(mix(b, c, t), mix(c, d, t), t), t);
   }
   function evalPolynomial(t, p) {
      let y = 0;
      if (Array.isArray(p))
   ...
   function sCurve(t, lo, hi) {
      if (lo !== undefined && hi !== undefined)
         return mix(lo, hi, sCurve((t - lo) / (hi - lo)));
   ...
   function serializeOutput(anim) {
      let H = 'CTanim01', data = new Uint16Array(4 + 6 * anim.length), i = 0;
      for (i = 0 ; i < 4 ; i++) data[i] = H.charCodeAt(i<<1) << 8 | H.charCodeAt(i<<1 | 1);
      for (let n = 0 ; n < anim.length ; n++) {
         let mat = anim[n]._m(), quat = CT.matrixToQ(mat);
	 let sgn = quat[3] < 0 ? -1 : 1;
         for (let j = 0 ; j < 3 ; j++) data[i++] = floor(65535 * (.5 + mat[j + 12] / 64));
         for (let j = 0 ; j < 3 ; j++) data[i++] = floor(65535 * (.5 + .5 * sgn * quat[j]));
      }
      return data.buffer;
   }
   ...
   function subtract(a, b) { let c=[] ; for (let i=0 ; i<a.length ; i++) c.push(a[i]-b[i]); return c; }
   function tan(t) { return Math.tan(t); }
   function transitionAngleTo(a, b, e) {
      while (a - b > PI) b += TAU;
      while (b - a > PI) b -= TAU;
      return transitionTo(a, b, e);
   }
   function transitionTo(a, b, e) { return a + max(-e, min(e, b - a)); }
...
// ARRAY UTILITIES.
   ...
   function index(src, I) {
      if (Array.isArray(I)) {
         let dst = [];
         for (let n = 0 ; n < I.length ; n++)
            dst.push(index(src, I[n]));
         return dst;
      }
      else
         return src[I];
   ...
   function slice(src, index) {
      ...
      if (Array.isArray(index))
	    ...
	    for (let k = 0 ; k < index.length ; k++)
               dst[n].push(src[n][index[k]]);
         }
      else
         for (let n = 0 ; n < src.length ; n++)
            dst.push(src[n][index]);
      return dst;
   }
   ...
   function smoothCurve(curve) {
      for (let n = 1 ; n <= curve.length - 2 ; n++)
         for (let j = 0 ; j < curve[n].length ; j++)
            curve[n][j] = mix(curve[n][j], curve[n-1][j], .5);
      for (let n = curve.length - 2 ; n >= 1 ; n--)
         for (let j = 0 ; j < curve[n].length ; j++)
            curve[n][j] = mix(curve[n][j], curve[n+1][j], .5);
   }
...
// 2D GEOMETRY UTILITIES.
   ...
   function Rectangle(x, y, width, height) {
      ...
      this.contains = (x, y) => x >= this.left()   && x < this.right() &&
   ...
   */
   ...
   function makeAngleSpline(angles, dst, ptsPerKey) {
      let keys = [];
      for (let n = 0 ; n < angles.length ; n++)
         keys.push([n / (angles.length-1), cos(angles[n]), sin(angles[n])]);
      let spline = makeSpline(keys, dst, ptsPerKey);
      angles = [];
      for (let n = 0 ; n < spline.length ; n++)
         angles.push(atan2(spline[n][2], spline[n][1]));
      return angles;
   }

   function makeScalarSpline(values, dst, ptsPerKey) {
      let keys = [];
      for (let n = 0 ; n < values.length ; n++)
         keys.push([n / (values.length-1), values[n]]);
      let spline = makeSpline(keys, dst, ptsPerKey);
      values = [];
      for (let n = 0 ; n < spline.length ; n++)
         values.push(spline[n][1]);
      return values;
   }
   ...
   function resampleCurve(src, count, _dst) {
      ...
      dst.push(copy(src[src.length-1]));
...
// VARIOUS MANIPULATIONS OF HTML ELEMENTS.
   ...
   let getPropMenuChoice = s => { 
      let i = s.indexOf(':'),
          n = i < 0 ? 0 : parseInt(s.substring(i + 1, s.length));
      return s.substring(0, i < 0 ? s.length : i).split(',')[n];
   }

   let propMenuStr = (choices, name) => { 
      let i = choices.length;
      while (--i > 0 && choices[i] != name)
         ;
      return choices.join() + ':' + i;
   }
