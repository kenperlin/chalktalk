...
// GET AND SET STATE DATA VIA THE PERSISTENT SERVER.
   ...
   function Server() {
      ...
      this.get = function(key, fn, onErr) {
         var request = new XMLHttpRequest();
      ...
      this.connectSocket = function() {
         ...
         this.socket.onmessage = function(event) {
            if (!displayStrokes.isStereo) return;
...
// TYPES AND FORMAT CONVERSIONS.
   ...
   function parseRGBA(s) {
      ...
      if (s.indexOf('#') == 0)
         for (var i = 0 ; i < 3 ; i++)
            dst[i] = (parseHexCode(s.charCodeAt(2*i+1)) << 4 | parseHexCode(s.charCodeAt(2*i+2))) / 255;
      else {
         var s = s.substring(s.indexOf('rgba') == 0 ? 5 : 4, s.indexOf(')')).split(',');
         for (var i = 0 ; i < 3 ; i++)
            dst[i] = parseInt(s[i]) / 255;
         if (s.length > 3)
...
// ARRAY UTILITIES.
   ...
   function contact(a,b,c,d,e,f,g,h,i,j) {
      var dst = a;
...
// 2D GEOMETRY UTILITIES.
   ...
   */
   ...
   function createRoundRect(x, y, w, h, r) {
      var c = [], d = 2 * r;
      c = c.concat(makeOval(x    ,y+h-d, d,d, 8,  PI/2,PI    )); c = c.concat([[x    ,y+h-r],[x    ,y  +r]]);
      c = c.concat(makeOval(x    ,y    , d,d, 8,  PI  ,3*PI/2)); c = c.concat([[x  +r,y    ],[x+w-r,y    ]]);
      c = c.concat(makeOval(x+w-d,y    , d,d, 8, -PI/2,0     )); c = c.concat([[x+w  ,y  +r],[x+w  ,y+h-r]]);
      c = c.concat(makeOval(x+w-d,y+h-d, d,d, 8,  0   ,PI/2  )); c = c.concat([[x+w-r,y+h  ],[x  +r,y+h  ]]);
      return c;
   }
   ...
   // CREATE A RAGGED LINE.

   function createRaggedLine(ax, ay, bx, by) {
      var dx = bx - ax, dy = by - ay, d = len(dx, dy);
      dx /= d;
      dy /= d;
      var dst = [];
      for (var t = 0 ; t < 1 ; t += 4/d) {
         //var s = 6 * noise(t*d/8+ax+ay+.5);
         var s = 2 * sin(3 * (t*d/8+ax+ay));
         dst.push([ mix(ax, bx, t) + s * dy,
                    mix(ay, by, t) - s * dx ]);
      }
      return dst;
   }
