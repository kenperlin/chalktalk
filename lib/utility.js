...
// MATH CONSTANTS AND FUNCTIONS
   ...
   function labelPrint(label, toPrint) {
      console.log(label);
      console.log(toPrint);
   }
...
// ARRAY UTILITIES.
   ...
   function cloneObject(object, ignoredArraysDuringCopy) {
         ...
         for (var prop in obj) {
            ...
            if (check.isGetterOrSetter) {
               ...
               if (ignoredArraysDuringCopy.indexOf(prop) > -1) {
                  ...
                  dst[prop] = deepCloneArray(obj[prop]);
               }
            }
               ...
               if (idx >= 0) {
                  ...
                  if (descriptor === undefined || descriptor.writable) {
                     // MUST BE SHALLOW COPY, DIFFERENTIATE BETWEEN MESH / NON-MESH
                     if (srcRefs[0].createMesh) {
                        dst[prop] = obj[prop];
                     }
                     else {
                        dst[prop] = deepCloneObject(obj[prop]);
                     }
                  }
               }
            }
      ...
      function deepCloneArray(src, dst) {
         if (src === undefined) {
            return undefined;
         }
         if (dst === undefined) {
         ...
         for (var i = 0 ; i < src.length ; i++) {
            if (src[i] instanceof Array) {
               dst[i] = deepCloneArray(src[i], dst[i]);
            }
