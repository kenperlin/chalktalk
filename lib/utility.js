      ...
      this.connectSocket = function() {
	    ...
	    if (obj.code) {
	       ...
	       return;
            ...
            var update = that.ProtoParsers.Update.decode(event.data);
...
// CHARACTER CONSTANTS AND CONVERSIONS.
   ...
   function charCodeToString(key) {
      ...
      if (isControlPressed) {
         switch (key) {
         case  49: return EXP_1;   // SUPERSCRIPT 1
   ...
   */
   ...
   var convertCurveRelativeToAbsolute = function(src) {
      ...
      for (i = 0 ; i < src.length ; i++)
	    ...
	    if (i == 0)
	       ...
	       if (! (src[i] instanceof Array))
	          dst.push(src[i++]);
	       dst.push([src[0][0] + src[i][0], src[0][1] + src[i][1]]);
            }
         }
      return dst;
   }
   ...
   function findCurveInflections(stroke, nInflections) {
      ...
      while (nInflections--) {
         ...
         // CLEAR OUT THIS AREA OF CURVE, TO AVOID ADDING IT OR ITS NEIGHBORS
   ...
   function curvesComputeBounds(src) {
      var xlo = 10000, ylo = xlo, xhi = -xlo, yhi = -ylo;
