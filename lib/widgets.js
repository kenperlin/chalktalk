...
/////////////////////////////////// HELP MENU //////////////////////////////////
   ...
   function drawHelpMenu() {
	 ...
	 utext('1', x, y, .5, .5);
         ...
         var dx = [r,r,0,-r,-r,-r,0,r];
         var dy = [0,r,r,r,0,-r,-r,-r];
         for (i = -1 ; i < 8 ; i++) {
	    (function() {
	       if (Object.keys(bgUpGestures[i]).length > 0) {
		     ...
		     if (j == -1)
		        ...
		        arrow(cx, cy, cx - .6 * dx[j], cy + .6 * dy[j]);
/*
			if (abs(dx[i]) == r)
		           utext(data[0], cx - .6 * dx[i], cy + .7 * dy[j], .5, .5);
                        else
		           utext(data[0], cx - .7 * dx[j], cy + .5 * dy[i], .5, .5);
*/
		        utext(data[0], cx - dx[j], cy + dy[j], .5, .5);
	             }
	          }
