...
/////////////////////////////////// HELP MENU //////////////////////////////////
   ...
   function drawHelpMenu() {
      var clickOp, dragOp, cx, cy, r, rx, ry, i, c, s, t, x, y, rc, rs;

      if (isk()) {

         clickOp = 'delete,unparse,move,copy,scale,cmd,rotate,undraw/see code'.split(',');
	 ...
	 fillRect(sk().xlo, sk().ylo, sk().xhi, sk().yhi);

         //color(overlayColor);
	 //drawOval(cx - r * .9, cy - r * .9, r * 1.8, r * 1.8, 40);
         color(overlayScrim);
	 fillOval(cx - r * .9, cy - r * .9, r * 1.8, r * 1.8, 40);
         color(overlayColor);
         ...
         for (var i = 0 ; i < 8 ; i++) {
            ...
            if (dragOp[i].length > 0) {
               utext(clickOp[i], x, y, .5, 1.3, defaultFont);
               utext('(' + dragOp[i] + ')', x, y, .5, -.3, defaultFont);
            }
            else
               utext(clickOp[i], x, y, .5, .5, defaultFont);
