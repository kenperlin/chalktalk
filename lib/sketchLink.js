...
SketchLink.prototype = {
   ...
   draw : function(isVisible) {
      ...
      if (! isEqualArray(status, this.status)) {
         this.status = status;
         this.C = createCurve([ax,ay], [bx,by], s);
      ...
      if (isVisible) {
         ...
         case 0: // Label the link as 'x','y' or 'z'.
            ...
            color(dataColor);
            //drawOval(this.c[0] - ts, this.c[1] - ts, 2*ts, 2*ts);
	    var fontHeight = floor(ts * 16 / 13);
            utext("xyzXYZ".substring(j, j+1), this.c[0], this.c[1] + (j==1?-3:-1.5), .5, .5, fontHeight + 'pt Arial');
            break;
   ...
   removeFromInSketch : function() {
      ...
      if (isFreehandSketch(b) && b.isNullText() && b.sp0.length <= 1)
         deleteSketch(b);
   },
