...
function SketchLink(fromSketch, outputPortIndex, toSketch, inputPortIndex, curvature) {
   ...
   if (outputPortIndex >= fromSketch.out_DEPRECATED_PORT_SYSTEM.length
      || fromSketch.out_DEPRECATED_PORT_SYSTEM[outputPortIndex] === undefined)
   {
      fromSketch.out_DEPRECATED_PORT_SYSTEM[outputPortIndex] = [];
   }
   fromSketch.out_DEPRECATED_PORT_SYSTEM[outputPortIndex].push(this);

   // Set this to be the in link for sketch toSketch, port inputPortIndex.

   // TODO: either replace this "in" or undeprecate it
   toSketch.in_DEPRECATED_PORT_SYSTEM[inputPortIndex] = this;
}
