...
function SketchLink(fromSketch, outputPortIndex, toSketch, inputPortIndex, curvature) {
   ...
   if (outputPortIndex >= fromSketch.outLinks.length
      || fromSketch.outLinks[outputPortIndex] === undefined)
   {
      fromSketch.outLinks[outputPortIndex] = [];
   }
   fromSketch.outLinks[outputPortIndex].push(this);

   // Set this to be the in link for sketch toSketch, port inputPortIndex.
   toSketch.inLinks[inputPortIndex] = this;
}
...
SketchLink.prototype = {
   ...
   draw : function(isVisible) {
	 ...
	 if (n >= 2)
         ...
         case 0: // Label the link as 'x','y' or 'z'.
            ...
            if (b.inLinks.length > 1 || b.inLabel_DEPRECATED_PORT_SYSTEM.length > 0) {
	       var label, dy;
   ...
   removeFromOutSketch : function() {
      ...
      for (var k = 0 ; k < a.outLinks[i].length ; k++)
         if (a.outLinks[i][k] == this) {
            a.outLinks[i].splice(k, 1);
            if (a.outLinks[i].length == 0)
