...
function idToUser(uid) {
	return userMap.uidToRecords[uid];
}
function setActiveUser(uid) {
	// console.log("set active user: uid=[" + uid + "]");
	// console.log(userMap.uidToRecords);
	UserRecord.ACTIVE.uid = uid;
	UserRecord.ACTIVE.record = userMap.uidToRecords[uid];
}
function getActiveUser() {
	return UserRecord.ACTIVE;
}
function updateUserCursor(userRecord, x, y, z) {
	userRecord.cursor.x = x;
    userRecord.cursor.y = y;
    userRecord.cursor.z = z;
}
