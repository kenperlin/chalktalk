	 ...
	 if (isHover())
      ...
      if (mode > 0) {
	    ...
	    if (sk().stretchValues !== undefined)
         ...
         fillRoundRect(sk().xlo, sk().ylo, sk().xhi - sk().xlo,
                       sk().yhi - sk().ylo, sketchPadding);
         _g_setZ(0);
	 ...
	 if (sk().onClick !== undefined && typeof sk().onClick !== 'function')
	    ...
	    if (mode == 1) {
	       ...
	       if (txt !== undefined) {
            ...
            else if (sk().onSwipe[i] !== undefined && sk().onSwipe[i] != null) {
               rc = r * c * t * .75;
               rs = r * s * t * .75;
               utext(sk().onSwipe[i][0], cx + rc, cy - rs, .5, .5);
