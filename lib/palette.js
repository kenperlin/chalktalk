   ...
   function drawPalette(context) {
      ...
      for (var n = 0 ; n < nc ; n++) {
         if (sketchPage.paletteColorDragXY != null && n == paletteColorId)
	    continue;
         var r = paletteR(n);
         context.fillStyle = palette[n];
         fillOval(paletteX(n) - r, paletteY(n) - r, 2 * r, 2 * r);
      }
      annotateEnd(context);
   }
...
// POSITION AND SIZE OF THE COLOR PALETTE ON THE UPPER LEFT OF THE SKETCH PAGE.
   ...
   function paletteWidth() {
      if (isPieMenuPalette)
         return 2 * palettePieRadius;
      return 32;
   }
   function paletteHeight() {
      if (isPieMenuPalette)
         return 2 * palettePieRadius;
      return paletteY(palette.length) - 10;
   }
   function paletteX(i) {
      if (isPieMenuPalette)
         return bgAction_xDown + palettePieRadius * sin(TAU * i / palette.length);
      return 20 - _g.panX;
   }
   function paletteY(i) {
      if (isPieMenuPalette)
         return bgAction_yDown - palettePieRadius * cos(TAU * i / palette.length);
