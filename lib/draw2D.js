   ...
   function startCurve(c) {
      ...
      if (c.length >= 2)
	    ...
	    if (c[i] instanceof Array)
	       ...
	       ovalTo(c[i-1], c[i], c[++i]);
   }

   function ovalTo(p, n, q) {
      var angle, i, ni = 8 * abs(n);

      for (i = 1 ; i <= ni ; i++) {
         angle = abs(n) * PI/2 * i / ni;
         if (n > 0)
	    _g_lineTo(mix(q[0], p[0], cos(angle)), mix(p[1], q[1], sin(angle)));
	 else
	    _g_lineTo(mix(p[0], q[0], sin(angle)), mix(q[1], p[1], cos(angle)));
      }
   }
