...
Group.prototype = {
   ...
   copy : function() {
      ...
      for (let j = 0 ; j < sketches.length ; j++) {
         let s = this.sketches[j];
         for (let port = 0; port < s.inputs.numPorts(); port++) {
            let i = this.sketches.indexOf(link.a)
            if (sketches.inputs.hasLink(port) && i >= 0) {
               let link = s.inputs.link(port);
               new SketchLink(sketches[i], sketches[j], link.j, link.s);
            }
         }
      }
   ...
   _drawShape : function(alpha) {
      ...
      if (this.xhi != this.xlo || this.yhi != this.ylo) {
         color(fadedColor((backgroundColor == 'black' ? .1 : .6) * (this.fadeAway ? this.fadeAway : 1)));
