...
function GlyphChart() {
   ...
   this.iSelectedTag = 0;
   this.latestTagHoveredOver = null;
   
   this.strokeCountMask = 0;
   
   this.tags = [''];
   // BITMAP OF ON / OFF STATES FOR EACH TAG
   this.tagStates = {};
...
GlyphChart.prototype = {
   ...
   tagBounds : function(i) {
      ...
      var ht = this.tagBoundHeight;
      var x = width() - (txtLen * 10) - _g.panY;
      // THIS COMMENTED CODE WOULD CYCLE THE BOX AROUND TO THE NEXT COLUMN,
      // DISABLED FOR NOW FOR TAGS, LIKELY TO RE-ACTIVATED LATER
      var y = ((i /*% this.glyphsPerCol*/) * height()) / (height() / this.tagBoundHeight) + ht * .1 - _g.panY;

      return [ x, y , x + min(this.tagBoundWidth, txtLen * 10) - _g.panY, y + ht * .85 ];
   },
   ...
   drawTagList : function() {
      ...
      function drawTag(i) {
         ...
         var x = gX + (height() / that.tagBoundHeight) * .1;
         var y = gY;
         var t = (that.t - i - 0.55) * 2.5 + i + .5;
         var isSelected = mx >= b[0] && my >= b[1] && mx < b[2] && my < b[3];

         if (isSelected && i > 0) {
            that.latestTagHoveredOver = that.tags[i];
         }

         // WHETHER THIS TAG HAS BEEN CLICKED ON THE CURRENT FRAME
         var isClicked = that.isSelectingTag && i == that.iSelectedTag;
         ...
         if (isClicked) {
            ...
            // TOGGLE INDIVIDUAL STATE
               ...
               console.log("TOGGLE");
               const prevState = that.tagStates[tag];
               that.tagStates[tag] = !prevState;
               that.numActiveTags += (prevState == true) ? -1 : 1;
            }
         }
         ...
         var gR = that.gR;
         var sc = height() / 2000 * 10 / that.tagBoundHeight;
         var n, d, j;
         ...
         _g.strokeStyle = _g.fillStyle = isHighlighted ? defaultPenColor : that.color();
   ...
   drawGlyph : function(i, cx, cy) {
      ...
      var emphasizedByTag = false;
      if (glyph.tags.length > 0 && this.numActiveTags > 0) {
         function glyphIsEmphasized(chart) {
            for (let i = 0; i < glyph.tags.length; i++) {
               if (chart.tagStates[glyph.tags[i]] == true) {
                  return true;
               }
            }
            return false;
         }
         emphasizedByTag = glyphIsEmphasized(this);
      }

      const maskNZ = (this.strokeCountMask & 1 << nn) != 0;

      var isHighlighted = isSelected ||
                         (emphasizedByTag && !maskNZ) ||
                         (maskNZ && (emphasizedByTag || this.numActiveTags == 0)) ||
                         (this.latestTagHoveredOver != null && glyph.tags.indexOf(this.latestTagHoveredOver) >= 0);
