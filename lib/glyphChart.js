...
GlyphChart.prototype = {
   ...
   // WORK-IN-PROGRESS
   tags : ["all", "creature", "math"],
   tagBoundWidth : 125,
   tagBoundHeight : 50,
   tagBounds : function(i) {
      var ht = this.tagBoundHeight;
      var x = width() - this.tagBoundWidth - _g.panY;
      // THIS COMMENTED CODE WOULD CYCLE THE BOX AROUND TO THE NEXT COLUMN, DISABLED FOR NOW FOR TAGS, LIKELY TO RE-ACTIVATED LATER
      var y = ((i /*% this.glyphsPerCol*/) * height()) / (height() / (this.tagBoundHeight - 1)) + ht * .1 - _g.panY;
      return [ x, y, x + this.tagBoundWidth - _g.panY, y + ht * .85 ];
   },
   drawTagList : function() {
      ...
      //fillRoundRect(width() - this.tagBoundWidth - _g.panY, 0, width() + 200 - _g.panY, height(), 0.5);

      const that = this;

      function drawTag(i, txt) {
         var b = that.tagBounds(i); // logic in tagBounds(i) is wrong, needs to be adjusted
         var gX = b[0], gY = b[1], gW = b[2]-b[0], gH = b[3]-b[1]; // PROBABLY WRONG / NEEDS TO BE ADJUSTED
