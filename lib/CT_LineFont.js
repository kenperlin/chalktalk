...
CT.lineFont = (function() {
   ...
   lineFont._meshCache = [];
   lineFont.charMesh = function(charCode) {
      if (isDef(lineFont._meshCache[charCode])) {
         return lineFont._meshCache[charCode];
      }

      let curves = lineFont[0][charCode];
      if (!isDef(curves)) {
         return undefined;
      }
      let currMesh = [];
      ...
      for (let c = 0; c < curves.length; c++) {
         for (let v = 0; v < curves[c].length; v++) {
            currMesh.push(curves[c][v]);
            if (v > 0) {
               currMesh.edges.push([numVertices + v-1, numVertices + v]);
            }
         }
         numVertices += curves[c].length;
      }
      lineFont._meshCache[charCode] = currMesh;
      return currMesh;
   }
   return lineFont;
})();
