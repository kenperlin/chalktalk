...
function enqueueSendMeshData(type, child, sketch) {
   ...
   if (!record.sent) {
      ...
      if (record.resendTo.length > 0) {
         let iter = record.resendTo.entries();
         ...
         for (let i = 0; i < userIDs.length; i += 1) {
            if (record.redendTo.has(userIDs[i])) {
               continue;
            }

            console.log("UPDATING [" + shapeID + "] to [" + userIDs[i] + "]");
            if (!sketch._scaleX){
               _sendMeshData(
                  0, type /* temp */, shapeID, userIDs[i] /* temp 0, need to increment for each sub mesh */, 
                color,
                  //trans[0], trans[1], trans[2] + 3.733605607889321 + sketch.zOffset(),
                trans[0], trans[1], sketch.zOffset(),
                  rot[0], rot[1], rot[2], scale,
