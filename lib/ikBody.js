...
function IKBody(ik_data) {
   ...
   this.linkMaterial   = phongMaterial().setAmbient(.4,.0,.0).setDiffuse(.4,.0,.0).setSpecular(.3,.3,.3,7);
...
IKBody.prototype = {
   ...
   loadData : function(ik_data) {
	 ...
	 this._d2s = function(j, frame) {
      ...
      var qWL = (new THREE.Quaternion()).copy(this.getQ(IK.WRIST_L, 120)).inverse();
      var qWR = (new THREE.Quaternion()).copy(this.getQ(IK.WRIST_R, 120)).inverse();

      var tmp = new THREE.Quaternion().set(0,-.15,0,1).normalize();
      qWL.multiply(tmp);
   ...
   computeNodeData : function() {
      ...
      nodeP[IK.BELLY     ] = relVec(IK.CHEST     , 0   , -0.2 , 0);
      nodeP[IK.PELVIS    ] = relVec(IK.BELLY     , 0   , -0.2 , 0);
   ...
   modelBody : function() {
	 ...
	 case IK.NECK:
	    ...
	    var neckShape = new THREE.Mesh(openCylinderGeometry(), this.linkMaterial);
	    neckShape.position.set(0, -.12/.12, 0);
	    neckShape.scale.set(0.2, .8, 0.2);
	    head.add(neckShape);
      ...
      if (this.graph === undefined) {
         ...
         function IKBodyResponder() {
            ...
            this.simulate = function() {
               ...
               this.graph.nodes[IK.BELLY ].p.offset( 0, 0, -.07, this.ikBody.getQ(IK.NECK));
            }
         }
   ...
   renderNode : function(node, j) {
      ...
      node.g.setMaterial(this.mode < 3 ? this.linkMaterial : this.jointMaterial);
