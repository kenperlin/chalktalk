...
IKBody.prototype = {
   ...
   loadData : function(ik_data) {
	 ...
	 this._d2s = function(j, frame) {
      ...
      var qWL = (new THREE.Quaternion()).set(0,-.15,.2,1).normalize();
      var qWR = (new THREE.Quaternion()).set(.2,0,-.05,1).normalize();
      ...
      if (this.graph === undefined) {
         ...
         function IKBodyResponder() {
            ...
            function adjust(p, x, y, z, q) {
               p.add(tmp.set(x,y,z).applyQuaternion(q));
            }
            ...
            this.simulate = function() {
               ...
               adjust(this.graph.nodes[IK.HIP_L ].p, .1, 0, 0, this.ikBody.getQ(IK.NECK));
               adjust(this.graph.nodes[IK.HIP_R ].p,-.1, 0, 0, this.ikBody.getQ(IK.NECK));

               adjust(this.graph.nodes[IK.KNEE_L].p, 0, 0, .02, this.ikBody.getQ(IK.ANKLE_L));
               adjust(this.graph.nodes[IK.KNEE_R].p, 0, 0, .02, this.ikBody.getQ(IK.ANKLE_R));

               // FORCE SPINE TO CURVE BACKWARD, RATHER THAN FORWARD.

               adjust(this.graph.nodes[IK.PELVIS].p, 0, .1, -.1, this.ikBody.getQ(IK.NECK));
               adjust(this.graph.nodes[IK.BELLY].p, 0, 0, -.07, this.ikBody.getQ(IK.NECK));
            }
         }
         IKBodyResponder.prototype = new GraphResponder;

         this.graph = new VisibleGraph();
         var responder = new IKBodyResponder();
	 responder.ikBody = this;
         this.graph.setResponder(responder);
      ...
      for (var j = 0 ; j < this.graph.links.length ; j++)
         this.renderLink(this.graph.links[j]);
   ...
   renderNode : function(node, j) {
      ...
      if (node.g === undefined) {
         node.g = this.graph.newNodeMesh(this.jointMaterial, 0.04);
         this.mesh.add(node.g);
      }
      var s = this.mode >= 3 ? 0.04 : this.mode == 2 ? .03 : this.mode == 1 ? j<=9 ? .03 : 0 : j <= 4 ? .03 : 0;
      node.g.scale.set(s,s,s);
      node.g.setMaterial(this.mode < 3 ? this.boneMaterial : this.jointMaterial);
   ...
   renderLink : function(link) {
      if (link.g === undefined) {
         ...
         link.g = this.graph.newLinkMesh(link.w < 1 ? this.tendonMaterial : this.boneMaterial, r);
