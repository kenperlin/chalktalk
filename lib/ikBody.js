...
IKBody.prototype = {
   ...
   modelBody : function() {
	 ...
	 case 2:
	    ...
	    foot.position.set(0,-.1,.09);
	    foot.scale.set(.06,.06,.15);

            var sole = new THREE.Mesh(globeGeometry(16,5), material);
	    sole.position.set(0,-.1,.09);
	    sole.scale.set(.06,.01,.15);
   ...
   blink   : function(isBlinking) {
      var head = this.body.children[0].children[1];
      for (var e = 0 ; e < 2 ; e++)
         head.children[e].scale.y = isBlinking ? 0 : .2;
      return isBlinking;
   },
