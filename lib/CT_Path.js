...
CT.ShapePath = function() {
   ...
   this._defaultVertexShader =
      ...
      ,'attribute vec2 aPos;'
      ,'uniform mat4  uCamMatrix, uPosMatrix;'
      ,'uniform vec4  uPath[1000];'
      ,'uniform vec3  uCentroid;'
      ,'uniform float uFill, uNpts, uRadius0, uRadius1, uEye, uAspect;'
      ,'varying float vClipX;'
      ,'vec3 P(int i) { return (uCamMatrix * uPosMatrix * uPath[i]).xyz; }'
      ,'void main() {'
      ,'   vec3 p0;'
      ,'   vec4 pos;'
      ,'   if (uFill > 0.5) {'
      ,'      p0 = mix(uCentroid, uPath[int(aPos.y)].xyz, aPos.x);'
      ,'      pos = uCamMatrix * uPosMatrix * vec4(p0, 1.);'
      ,'   }'
      ,'   else {'
      ...
      ,'      }'
      ,'      pos = vec4(p0 + vec3(radius * dp * uPath[n].w, 0.), 1.);'
      ,'   }'
