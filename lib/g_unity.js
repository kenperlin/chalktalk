...
// HEHE DESELCTING THE OBJECT
	...
	const meshRequestTimestampMap = new Map();
	canvas.clientRequestMeshAsset = function(event) {
		const uid = event.uid;
		const prevTime = meshRequestTimestampMap.get(uid);
		if ((prevTime != undefined) && (event.timestamp <= prevTime)) {
			return;
		}
		meshRequestTimestampMap.set(uid, event.timestamp);
	...
	canvas.clientMeshAssetDeletionAck = function(event) {
		const uid = event.uid;
		const mid = event.mid;

		console.log("in clientMeshAssetDeletionAck()");

		if (!CT.modelInfo.idMap.has(mid)) {
			console.log("does not have the element for some reason");
			return;
		}

		const modelRecord = CT.modelInfo.get(mid);
		const deletionMap = CT.modelInfo.deletionMap;
		if (!deletionMap.has(mid)) {
			console.log("deletion map does not have the element for some reason");
			CT.modelInfo.delete(mid);
			return;
		}

		const uidsThatNeedToDelete = deletionMap.get(mid);
		if (uidsThatNeedToDelete.has(uid)) {
			console.log("removing uid from deletion set since the user confirmed deletion");
			uidsThatNeedToDelete.delete(uid);

			if (uidsThatNeedToDelete.size == 0) {
				console.log("removing model data permanently since all users confirmed deletion");
				deletionMap.delete(mid);
				CT.modelInfo.idMap.delete(mid);

				console.log("user:[" + uid + "] successfully deleted mesh:[" + mid + "]");
			}
		}
	}
}
