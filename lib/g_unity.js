...
function sendIDAndTimeAndFPNum(hdr, fpNum, id) {
	var data = new Uint16Array(4 + 1 + 4), header = hdr, i;
	for (i = 0; i < 4; i += 1) {
		data[i] = header.charCodeAt(i<<1) << 8 | header.charCodeAt(i<<1 | 1);
	}
...
function initUnityHandlers(canvas) {
	...
	canvas.clientEnablePalette = function(event) {
		...
		if (CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID >= 0 &&
           uid != CT_CONFIG.SINGLE_USER_PER_HOST_MODE_UID) {
           return;
        }
	...
	canvas.clientSetSketchPage = function(event) {
        ...
        // console.log("SET SKETCH PAGE, uid=[" + uid + "]");
	...
	canvas.clientAddUserID = function(event) {
		...
		if (uidIdx < 0) {
			...
			getOrCreateUserRecord(event.uid);
		}
	};
