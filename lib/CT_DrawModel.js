...
function enqueueSendMeshData(child, sketch, vtxCount, vertices, triIdxCount, triangleIndices) {
   meshDataSendCallbackQueue.push(function() {
      ...
      _sendMeshData(
         0, 0 /* temp */, sketch.id, 0 /* temp 0, need to increment for each sub mesh */, 
         trans[0], trans[1], sketch.zOffset(),
         rot[0], rot[1], rot[2], scale,
         vtxCount, vertices, triIdxCount, triangleIndices
      );
   ...
   if (! child || child._draw_hash != hash) {
      ...
      if (window.displayListener) {
         ...
         if (S) {
            ...
            enqueueSendMeshData(child, S, a.length, a, b.length, b);
         }
      }
   }
   else if (child) {
