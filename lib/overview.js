...
function overview_update() {
   ...
   if (isShowingOverview || overview_alpha > 0) {
      ...
      for (var I = 0 ; I < nsk() ; I++)
         if (! group.containsSketch(sk(I)))
            for (var i = 0 ; i < sk(I).out_DEPRECATED_PORT_SYSTEM.length ; i++)
               if (isDef(sk(I).out_DEPRECATED_PORT_SYSTEM[i]))
                  for (var k = 0 ; k < sk(I).out_DEPRECATED_PORT_SYSTEM[i].length ; k++)
                     sk(I).out_DEPRECATED_PORT_SYSTEM[i][k].drawTransformed(xInOverview, yInOverview);
