...
let overviewShowBoundaryWhilePanning = true;
let overviewShowGridWhilePanning = true;
...
function overview_update() {
   ...
   // (KTR temp disable)
   if (false && overview_alpha < 1 && sketchPage.isPressed
                          && bgClickCount == 1
                          && bgAction_dir1 == -1
      ...
      const xmin = w * (1 - overview_scale) / 2;
      const xmax = w * (1 + overview_scale) / 2;
      const ymin = h * (1 - overview_scale) / 2;
      const ymax = h * (1 + overview_scale) / 2;

      if (overviewShowBoundaryWhilePanning) {
      ...
      // SHOW GRID.
      if (overviewShowGridWhilePanning) {
         for (var x = xmin + w ; x < xmax ; x += w)
            line(x, ymin, x, ymax);
         for (var y = ymin + h ; y < ymax ; y += h)
            line(xmin, y, ymax, y);
