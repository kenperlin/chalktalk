...
InputPortList.prototype._setLink = function(i, link) {
   if (this.hasPort(i)) {
      ...
      while (this.numPorts() <= i) {
         // Create the port with the primary supported type
         let primaryType = this._dynamicPortTemplate.acceptedTypes[0];
         let newPort = new InputPort(primaryType,
            this._dynamicPortTemplate.preprocessingFunctions[primaryType.name]);

         // Then add all the other supported types
         for (let i = 1; i < this._dynamicPortTemplate.acceptedTypes.length; i++) {
            let acceptedType = this._dynamicPortTemplate.acceptedTypes[i];
            newPort.addAcceptedType(acceptedType,
               this._dynamicPortTemplate.preprocessingFunctions[acceptedType.name]);
         }
