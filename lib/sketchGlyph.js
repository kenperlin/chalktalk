...
function SketchGlyph(name, src) {
   ...
   //var isPendulum = false;
   this.compare = function(other) {
      if (this.data.length != other.data.length)
         return this.WORST_SCORE;

      if(other.name == "sg('pendulum_sketch','pendulum')")

        server.broadcastObject(msg);

        //server.broadcast("hue");
        //isPendulum = true;

        // for (var n = 0 ; n < this.data.length ; n++){
        //     //for (var i = 0 ; i < this.data[n].length ; i++) {
        //   console.log("pendulum x:" + other.data[n][0][0] + " y:" + other.data[n][0][1]);
        //   console.log("my x:" + this.data[n][0][0] + " y:" + this.data[n][0][1]);
        // }
