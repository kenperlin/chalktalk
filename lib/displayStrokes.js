...
DisplayStrokes.prototype = {
   ...
   _stroke_or_fill : function(isFill) {
      let path = this.path;
      if (isCasualStyle && isk() && ! sk().isFreehandSketch()) {
         let p = [];
         for (let n = 0 ; n < path.length ; n++)
	    p.push([path[n][0], path[n][1], path[n][2], path[n][3] ? .0001 : 0]);
	 p = resampleCurve(p, 100);
	 let dx = sk().tX * sk().xyz[2];
	 let dy = sk().tY * sk().xyz[2];
         for (let n = 0 ; n < p.length ; n++) {
	    p[n][0] += 5 * noise((p[n][0] - dx) / 40, (p[n][1] - dy) / 40, 100 + sk().id);
	    p[n][1] += 5 * noise((p[n][0] - dx) / 40, (p[n][1] - dy) / 40, 200 + sk().id);
	 }
         for (let n = 0 ; n < p.length ; n++)
	    p[n][3] = p[n][3] != 0;
	 path = p;
      ...
      for (var i = 0 ; i < path.length ; i++)
         this._actual_moveTo_or_lineTo(path[i]);
   ...
   _actual_moveTo_or_lineTo : function(p) {
      ...
      let lw = w * def(this.widthFactor, 1) * (sketchPage.isFuzzyLines() ? 4 : 1);
   ...
   _actual_stroke_or_fill : function(isFill, isText) {
      ...
      if (! window._isExtendBounds && buffer.length > 1) {
         ...
         const OFF = (window.PUSHED_sketch_zOffset) ? (window.PUSHED_sketch_zOffset) : 0;
         if (OFF != 0.0) {
         ...
         if (xlo <= 1.05 && ylo <= aspect && xhi >= -1.05 && yhi >= -aspect) {
            ...
            if (window.displayListener && ! isText && !DISPLAY_LIST_CONFIG.IGNORE)
		 ...
		 else if (!this._doCullingForDisplayList && window.displayListener && !isText && !!DISPLAY_LIST_CONFIG.IGNORE) {
            var rgba = this._computeRGBA(isFill);
            _sendStrokeToDisplayListener(this.width(), this.height(),rgba, isFill, buffer);
         }  
      }     
