...
/*
   ...
   _moveTo_or_lineTo : function(x, y, isLine) {
      if (! isLine)
         this._points = [];
      this._points.push(  ( x - width()  / 2 ) / (width() / 2) );
      this._points.push( -( y - height() / 2 ) / (width() / 2) );
      this._points.push(0);
   },
   ...
   _stroke_or_fill : function(isFill) {
      if (this._points.length > 1) {
         ...
         var rgba = [1, 1, 1, _g.globalAlpha];
	 var style = _g.strokeStyle;
	 if (style.indexOf('#') == 0)
	    for (var i = 0 ; i < 3 ; i++)
	       rgba[i] = parseHexCode(style.charCodeAt(2*i+1)) << 4 | parseHexCode(style.charCodeAt(2*i+2));
	 else {
	    var iRparen = style.indexOf(')');
	    var isAlpha = style.indexOf('rgba(') >= 0;
	    var s = style.substring(isAlpha ? 5 : 4, iRparen).split(',');
	    for (var i = 0 ; i < 3 ; i++)
	       rgba[i] = parseInt(s[i]);
            if (s.length > 3)
	       rgba[3] *= parseFloat(s[3]);
         }

         strokeShader.drawStroke(this._points, rgba, _g.lineWidth / (width() / 2));
      }
   },
*/
}
