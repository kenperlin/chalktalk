...
DisplayStrokes.prototype = {
   ...
   text : function(str, p, mat) {
      ...
      // If there is a matrix then rotate about p, dividing out scale so matrix only rotates.

      var m;
      if (mat)
         m = new Rotation(mat._m());

      var id = isk() ? sk().id + sk().stringId++ : 0;

      var c, curve, curves, i, j, n, pt, sx, sy, save_lineWidth, save_strokeStyle, x, y, z;
         ...
         if (font[ch]) {
            ...
            for (i = 0; i < curves.length; i++) {
               ...
               for (j = 0; j < curve.length; j++) {
                  pt = curve[j];
                  if (m) {
                     pt = [x + sx * (pt[0] + .2 * pt[1] * italic), y + sy * pt[1], z, j > 0];
      ...
      if (window.displayListener) {
         let rgba = this._computeRGBA(false);
         this._sendTextToDisplayListener(rgba, x, y, sy, str); 
      }
   ...
   _sendTextToDisplayListener : function(rgba, x, y, s, text) {
      ...
      this._sendDisplayObjectHeaderToDisplayListener(12 + Math.ceiling(text.length / 2), rgba, x, y, s);
   ...
   _sendStrokeToDisplayListener : function(rgba, isFill, buffer) {
      ...
      this._sendDisplayObjectHeaderToDisplayListener(12 + buffer.length, rgba);
   ...
   _computeRGBA : function(isFill) {
      ...
      return mix( backgroundColor == 'black' ? [0,0,0,1] : [1,1,1,1],
