...
DisplayStrokes.prototype = {
   ...
   _sendTextToDisplayListener : function(rgba, x, y, s, text) {
      console.log("_sendTextToDisplayListener\t" + x + " "+ y + " " + s);
	  // zhenyi: use factor 1000
      this._sendObjectHeaderToDisplayListener(12 + Math.ceil(text.length / 2), rgba, x, y, s/1000);
   ...
   _sendObjectHeaderToDisplayListener : function(size, rgba, x, y, s) {
      ...
      if (s) {
		 ...
		 console.log("header:\t" + x + " " + this._convertToFixedPoint(x));
         for (let q = 0 ; q < 3 ; q++)
            displayList.push(0);
