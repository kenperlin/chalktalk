...
DisplayStrokes.prototype = {
   ...
   text : function(str, p) {
      var c, curve, curves, i, j, n, pt, s, save_lineWidth, save_strokeStyle, x, y;

      save_lineWidth = _g.lineWidth;
      save_strokeStyle = _g.strokeStyle;

      s = parseFloat(this.g.font) * 0.25;

      _g.lineWidth = max(1, s / 5);
	 ...
	 if (ch >= 32 && ch < 128) {
	    ...
	    for (i = 0 ; i < curves.length ; i++) {
	       ...
	       for (j = 0 ; j < curve.length ; j++) {
	          pt = curve[j];
                  this._actual_moveTo_or_lineTo([x + s * pt[0], y - s * pt[1], 0, j > 0]);
	       }
