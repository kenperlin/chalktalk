...
function ChromaKeyedVideo()
{
  ...
  this.analyzeVideoFrame = function() {
      var vac = video_analysis_canvas, vw = vac.width, vh = vac.height;
      var r, g, b, data, x, y, z, ctx, i, xSum, ySum, area;
	 ...
	 x = .5 - x;
	 y = (.5 - y) * vh / vw;
	 z = 1 / Math.sqrt(area / (vw * vh));
	 x *= 100 / z;
	 y *= 100 / z;
	 console.log( roundedString(x, 3) + ' ' +
	              roundedString(y, 3) + ' ' +
	              roundedString(z, 3) );
      }
   }
  ...
  this.createVertexBuffer = function(gl)
  {
    ...
    var w = this.width();
    var verts = [-w, .85-2*w, 0,   w, .85-2*w, 0,
                 -w, .85    , 0,   w, .85    , 0];
