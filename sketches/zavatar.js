...
function() {
   ...
   var nameList = ['HEAD CHEST BELLY PELVIS',
   ...
   for (var n = 0 ; n < nameList.length ; n++) {
      var names = nameList[n].split(' ');
      for (var i = 0 ; i < names.length ; i++)
         window[names[i]] = index++;
   }

   // STICKMAN'S JOINTS.

   this.graph.addNode(  0,1.45, 0, 1);  //  HEAD
   this.graph.addNode(  0,  1 , 0   );  //  CHEST
   this.graph.addNode(  0, .65, 0   );  //  BELLY
   this.graph.addNode(  0, .3 , 0   );  //  PELVIS

   this.graph.addNode(-.2, 0.2 , 0   );  //  R_HIP
   this.graph.addNode(-.3,-0.5 , 0.1 );  //  R_KNEE
   this.graph.addNode(-.4,-1.2 , 0   );  //  R_ANKLE

   this.graph.addNode( .2, 0.2 , 0   );  //  L_HIP
   this.graph.addNode( .3,-0.5 , 0.1 );  //  L_KNEE
   this.graph.addNode( .4,-1.2 , 0   );  //  L_ANKLE

   this.graph.addNode( -.25, 1.0, 0 );  //  R_SHOULDER
   this.graph.addNode( -.65, 0.9, 0 );  //  R_ELBOW
   this.graph.addNode(-1.05, 1.0, 0 );  //  R_WRIST

   this.graph.addNode(  .25, 1.0, 0 );  //  L_SHOULDER
   this.graph.addNode(  .65, 0.9, 0 );  //  L_ELBOW
   this.graph.addNode( 1.05, 1.0, 0 );  //  L_WRIST

   // STICKMAN'S LIMBS.

   this.graph.addLink(HEAD, CHEST);
   this.graph.addLink(CHEST, BELLY);
   this.graph.addLink(BELLY, PELVIS);

   this.graph.addLink(PELVIS, R_HIP);
   this.graph.addLink(R_HIP, R_KNEE);
   this.graph.addLink(R_KNEE, R_ANKLE);

   this.graph.addLink(PELVIS, L_HIP);
   this.graph.addLink(L_HIP, L_KNEE);
   this.graph.addLink(L_KNEE, L_ANKLE);

   this.graph.addLink(CHEST, R_SHOULDER);
   this.graph.addLink(R_SHOULDER, R_ELBOW);
   this.graph.addLink(R_ELBOW, R_WRIST);

   this.graph.addLink(CHEST, L_SHOULDER);
   this.graph.addLink(L_SHOULDER, L_ELBOW);
...
/*
   this.graph.addLink(HEAD,       R_SHOULDER,  0.3);
   this.graph.addLink(HEAD,       L_SHOULDER,  0.3);
   this.graph.addLink(R_SHOULDER, L_SHOULDER,  0.3);

   this.graph.addLink(PELVIS, R_HIP,  0.3);
   this.graph.addLink(PELVIS, L_HIP,  0.3);
   this.graph.addLink(R_HIP,  L_HIP,  0.3);
*/
/*
...
*/
   ...
   var toDraw = [[HEAD,PELVIS], [PELVIS,L_ANKLE], [PELVIS,R_ANKLE], [CHEST,L_WRIST], [CHEST,R_WRIST]];
...
*/
   ...
   this.renderNode = function(node) {
      if (this.mesh === undefined)
         return;
      node.r = this.graph.R.defaultNodeRadius * (node.type == 1 ? 4 : 2);;
      if (node.g === undefined)
         this.mesh.add(node.g = this.graph.newNodeMesh(this.myMaterial, node.r));
   ...
   this.renderLink = function(link) {
      if (this.mesh === undefined)
         return;
      if (link.g === undefined)
         this.mesh.add(link.g = this.graph.newLinkMesh(this.myMaterial, this.graph.R.defaultNodeRadius * Math.sqrt(link.w)));
