...
var Holojam = function(
  ...
  this.ucAddresses = unicastAddresses;
  if (!this.ucAddresses) {
    this.ucAddresses = [];
  }
  // Multicast
  ...
  if(receiving){
    udp.on('message', (buffer, info) => {
      if (this.ucAddresses.indexOf(info.address) == -1) {
        console.log("Adding " + info.address + " to unicast list.");
        this.ucAddresses.push(info.address);
      }
  ...
  const UpdateUnicastAddressses = (addresses) => {
    this.ucAddresses = addresses;
  };

  this.SendToAddress = (nugget, address) => {
    let buffer = this.Encode(nugget);
    EmitToAddress(nugget, buffer, address);
    return buffer;
  }

  const EmitToAddress = (nugget, buffer, address) => {
    //Unicast.
    udp.send(buffer, 0, buffer.length, sendPort, address, (error, bytes) => {
      if (error) console.log("EmitToAddress Error: " + err);
    });
  }
  ...
  const Emit = (nugget, buffer) => {
    if(!sending) return;
    //Unicast.
    this.ucAddresses.forEach((address) => {
      udp.send(buffer, 0, buffer.length,
        sendPort, address,
        (error, bytes) => { 
          if(error) console.log("Unicast Error: " + error + " bytes:" + bytes); 
      });
    });

    //Multicast.
    //console.log("Trying to multicast to " + sendAddress + ":" + sendPort);
    udp.send(buffer, 0, buffer.length, sendPort, sendAddress, (err, bytes) => {
  ...
  this.SendBuffer = function(buffer) {
    Emit(null, buffer);
    return null;
  }
