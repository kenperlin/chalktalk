...
,'      vec2 light_amp[numLights];'
,'      float diff_amp[numLights];'
,'      float spec_amp[numLights];'
,'      light_color[0] = vec3(1., 1., 1.);'
,'      light_color[1] = vec3(.5, 0., 0.);'
,'      light_pt[0] = vec3(50.0, 40.0, 50.0);'
,'      light_pt[1] = vec3(-50.0, -50.0, 0.0);'
,'      light_amp[0] = vec2(0.45, 0.45);'
,'      light_amp[1] = vec2(0.25, 0.0);'
,'      //'
...
,'      vec3   viewdir, lightdir, refldir;'
,'      float  d_amp, s_amp;'
,'      for (int i = 0; i < numLights; i++) {'
,'         diff_amp[i] = max(0.2, dot(vNormal, normalize((light_pt[i]-vPosition))));'
,'         viewdir = normalize(eyept - vPosition);'
,'         lightdir = normalize(light_pt[i] - vPosition);'
,'         refldir = normalize(reflect(lightdir, vNormal));'
,'         spec_amp[i] = pow(max(0., dot(viewdir, refldir)), specpow);'
,'         sum_amps += diff_amp[i] + spec_amp[i];'
,'      }'
,'      pupil_size = pupil_size - (cos(mtime)*.035) - (sum_amps * .025);'
...
,'      for (int i = 0; i < numLights; i++) {'
,'         out_color += (color * light_color[i] * diff_amp[i] * light_amp[i].x) + (light_color[i] * spec_amp[i] * light_amp[i].y);'
,'      }'
